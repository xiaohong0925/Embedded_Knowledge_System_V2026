...
> 最近更新：2026-08-25（§3 新增节组级前置；§14 新增第 8 条写作自检循环：每篇成文后对照写作标准 7 条+事实/风格/补图/构建/自查表逐项验收；§15 新增复检修改批量上限：一节一闭环）
...
- **内容为主题服务**（2026-08-25 用户重申）：每个知识点都要能回答"它为本节主问题/章主线贡献什么"，讲不清贡献的删掉或挪走；每个设计性要求（为什么读这段、为什么做这步）都在文内给出理据，不让读者自行脑补。**大纲的章级结构（如第二部四段式）是章级叙事意图，不是节级标题模板**——禁止把"问题场景/机制深入"等作为字面小标题逐节重复 stamp（2026-08-25 第 8 章 8.1 批误用教训，用户判定"枯燥且过于模板化"）；节内行文按第 14 节「机制两问」自然展开。
...
## 12.5 第 7 章复检结论存档（2026-08-26 闭环）
...
- **返工首例已闭环（2026-08-18）**：B-E.15.5 数通仪器仪表整机总线架构（134→416 行），验证了返工流程可行性——含一次外部 AI 稿合并经验：对方产品对标内容必须逐条核实后再吸收（其 M9019A 带宽数字有误、Spirent 温控细节无法验证，均已剔除/修正）。

## 14. 扩展篇写作标准（2026-08-17 确立，全书扩展篇适用）
