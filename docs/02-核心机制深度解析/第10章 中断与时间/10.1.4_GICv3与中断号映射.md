...
GICv3 的变化一句话说透：把 Distributor 拆成 Distributor + 每核一个的 Redistributor，PPI/SGI 的配置本地化，消除了多核争抢全局寄存器的竞争热点。真正面向未来的是 LPI——基于消息的中断（INTID 8192 起），打破了 1020 的中断号上限，配合 ITS（Interrupt Translation Service）硬件翻译 DeviceID → INTID，PCIe MSI-X 由此获得原生支持。设备树侧的变化很小：`#interrupt-cells = <3>`，类型字段只剩 SPI/PPI，SGI 和 LPI 都不由 DT 描述。

选型上没有悬念：大型 SoC、多核服务器、PCIe 丰富的系统用 GICv3；早期嵌入式和低端平台上的 GICv2 依然随处可见，两套都要认。而 hwirq → irq_domain → virq 的映射把硬件差异挡在了驱动视野之外——这个硬件无关的抽象正是下一节的主角，级联中断控制器也靠它才能挂进体系。
