...
——这些需求依赖的硬件时钟各不相同，Linux 时间子系统用一套**五层架构**把它们统一起来：<br>
每层职责单一、层间通过明确接口协作，ARM Generic Timer、x86 TSC、HPET 等各种硬件都能接入同一个框架。<br>
...
```c
/* 注册流程要点（简化示意，kernel/time/clocksource.c） */
clocksource_register_hz(cs, freq)        /* include/linux/clocksource.h 内联 helper */
...
五层架构的核心思想是解耦：clocksource 只管"读时间"，clockevent 只管"设闹钟"，timekeeping 在中间维护 wall/monotonic 两条时间轴，tick device 与 hrtimer 构建其上。<BR>
这套分层让硬件更换只影响底层驱动，上层框架不动；也让 tick 模式切换被限制在 tick device 一层，为 10.6 的 tickless 埋下接口基础。

工程上记住两条：选源看 rating、watchdog 会自动降级不稳定的源；算超时一律用 monotonic time，别碰会跳变的 wall time。
...
| 核心代码 | `kernel/time/timekeeping.c` | timekeeping 主逻辑，`struct timekeeper` 定义在 include/linux/timekeeper_internal.h |
