...
至于"只剩一个任务"这个条件，由调度器侧维护：运行队列上 `nr_running > 1` 时，调度器通过 `nohz_full_kick` 向该 CPU 发 IPI 恢复 tick。
...
10.5.5 的音频爆音案例就是典型：48kHz 音频驱动用 hrtimer 每 20.8μs 补一次 DMA，CPU 进 C3/C6 后唤醒要数百微秒，hrtimer 回调迟到、DMA 缓冲区欠载——爆音。
...
设置后，cpuidle governor 不会选择唤醒延迟超过约束的 C-state。<BR>
驱动也可以通过 PM QoS 接口（`/dev/cpu_dma_latency`）按任务动态设置约束——音频播放时收紧、暂停时放开。
