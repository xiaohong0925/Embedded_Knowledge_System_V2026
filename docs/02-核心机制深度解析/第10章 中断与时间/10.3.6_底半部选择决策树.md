...
softirq、tasklet、workqueue、threaded IRQ、NAPI——前面几节把五种底半部机制都讲完了，实际写驱动时到底选哪个？选错了轻则延迟超标，重则内核 oops。<br>
本节覆盖：一棵可直接套用的决策树、五种机制的全维度对比表、每种机制的最小代码骨架，以及三个选错机制导致的真实故障案例。
...
> 💡 拿不准的时候默认选 workqueue：能睡眠、API 简单、行为可预测。性能测试发现延迟不达标，再评估升级到 threaded IRQ 的必要性。

> 💡 选定机制后还有一个随之而来的问题：底半部与进程上下文访问共享数据时的同步。`spin_lock_bh()`、`local_bh_disable()` 这类专门和底半部打交道的原语属于并发主题，统一在第 13 章「并发与同步」展开，本节不再重复。
...
> ⚠️ 带 `IRQF_ONESHOT` 时不要再在处理函数里调 `disable_irq_nosync()`/`enable_irq()`：内核电线程化机制已经保证了"线程执行期间该中断保持屏蔽"，手动开关反而会打乱这个状态配对，多次 enable 失衡还会触发内核警告。
...
**根因**：软中断上下文没有进程实体，一旦睡眠就没有可被调度回来的对象。内核日志里会看到 `BUG: scheduling while atomic`，随后系统状态不再可信。<br>
...
**根因**：`__do_softirq()` 串行执行所有 pending 的 softirq，一个跑得久，其他全部排队。被踢到 ksoftirqd 后情况也不会好转——ksoftirqd 是普通优先级的内核线程，要和其他进程竞争 CPU，重活在哪里执行都会拖累系统。<br>
...
50kHz 意味着每 20μs 来一次中断，而 workqueue 的执行延迟在数十微秒以上且不稳定。结果就是 work 不断积压（同一个 work 重复 schedule 还会被丢弃）、事件统计失真。<br>
...
10.3.7 底半部常见错误与陷阱 —— 剖析底半部编程中最高发的七类错误：`local_bh_disable` 误用、并发竞态、资源泄漏等，每个都配错误代码与修正方案。
