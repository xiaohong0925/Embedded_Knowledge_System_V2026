...
修复：把 I2C 通信挪到底半部（workqueue），顶半部只确认中断来源后立即返回：

```c
/* 修复后的顶半部：确认 + 清标志 + 调度，I2C 读挪进 workqueue */
static irqreturn_t keys_irq_handler(int irq, void *dev_id)
{
    struct my_device *dev = dev_id;

    /* ① 读 GPIO 状态确认（寄存器访问，< 1μs） */
    if (!(readl(dev->gpio_regs + GPIO_STATUS) & dev->pin_mask))
        return IRQ_NONE;

    /* ② 清 GPIO 中断（必须在顶半部） */
    writel(dev->pin_mask, dev->gpio_regs + GPIO_CLEAR);
...
顶半部耗时从 500μs 降到微秒级，中断丢失归零。<BR>
完整的注册模板见 10.2.1，workqueue 等底半部机制的写法与选型见 10.3。
...
顶半部超时的三个后果按杀伤力排序：边沿触发的中断只补记一次，多余边沿直接丢失，数据悄悄不见；电平触发的中断反复置位，堆积成风暴，CPU 被 handler 占满；`irq_exit()` 被推迟则 softirq 全局延迟，tick 中断无法触发时调度器直接失明，系统表现为无规律的卡顿。黄金法则的经验值是顶半部 < 100μs，高速外设 < 10μs——这不是建议，是验收线。

irqsoff tracer 是验收工具：记录最长的中断关闭区间和完整调用链，六步即可定位元凶。用的时候守住一个关键认知——报告里的 latency 是整个关中断区间，不是单个函数的耗时，别拿着总数去冤枉某一个函数。组合拳按irqsoff → function_graph → hwlat 的顺序层层深入：先找区间，再看函数级时间分布，最后排除硬件层面的延迟注入。
...
顶半部不能干什么、干久了会怎样，已经明确。<br>
那具体怎么拆分？底半部有哪些机制可选、各适合什么场景？

下一节 **10.3.1 为什么需要底半部**，从设计动机讲起，再逐个拆解 softirq、tasklet、workqueue。
