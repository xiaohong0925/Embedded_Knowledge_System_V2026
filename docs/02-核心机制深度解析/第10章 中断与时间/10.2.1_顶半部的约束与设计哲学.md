...
## <span class="blue"> 顶半部是什么，以及它必须做的四件事 [I]

先给定义：**顶半部（top half）是 `request_irq()` 注册的那个 handler——中断到达后第一个执行的软件代码，运行在中断上下文**。<br>
它是硬件与内核之间的第一个接触点，后面可以跟底半部（softirq/tasklet/workqueue，10.3 展开）。

顶半部必须做的事只有四类，每一类都有"不做就出事"的硬理由：

| 步骤 | 操作 | 为什么必须在顶半部做 |
|------|------|---------------------|
| ① 确认 | 读设备中断状态寄存器 | 状态寄存器随时会被下一次事件覆盖，底半部跑起来时读到的已是新状态 |
| ② 清除 | 写中断清除/应答寄存器 | 不清则中断线持续有效，handler 一返回立刻再次进入，形成中断风暴 |
| ③ 抢救 | 把硬件 FIFO/寄存器里的数据搬进内存 | FIFO 深度有限，新数据覆盖旧数据；底半部的调度延迟远大于 FIFO 溢出时间 |
| ④ 调度 | 唤醒底半部（`schedule_work()` 等） | 底半部不会自动执行，没人唤醒，活就永远没人干 |

前三类都是"硬件状态不等人"的窗口操作，第四类是交接棒。<br>
除此之外的一切——协议解析、大块数据拷贝、慢速总线通信——都不属于顶半部。

至于为什么要把中断处理拆成两半、底半部有哪些机制可选，那是 10.3.1 的主题。<br>
本节先回答更基础的问题：顶半部的运行环境为什么如此苛刻。

> 💡 ③ 抢救最经典的实例是 8250 串口驱动：hardirq handler（`serial8250_handle_irq()`）里直接读空 RX FIFO 搬进 tty flip buffer（`serial8250_rx_chars()`，一次最多 256 字节）。<br>
> 115200 波特下 16 字节 FIFO 约 1.4ms 就会溢出——等底半部来读早就丢了，溢出由硬件置 `UART_LSR_OE` 位并计入 overrun 统计。<br>
> 注意边界：抢救只针对小且会被覆盖的数据；网卡整包拷贝这类大数据量恰恰是反面教材（10.3.1），现代网卡用 NAPI 把它挪出顶半部。
...
这个"关"不需要 handler 做任何事：ARM64 响应 IRQ 异常时，硬件自动置位 `PSTATE.DAIF` 的 I 位，handler 进来就已经处于屏蔽状态，入口代码只是保持它直到返回。<BR>
唯一的例外是下面 💡 提到的 pseudo-NMI 路径——那时 DAIF.I 会重新打开，普通中断改由 GIC 的 PMR 优先级寄存器挡住。
...
顶半部的三大约束都指向同一个根因：中断上下文没有 `task_struct`。不能睡眠，是因为 `schedule()` 无处保存和恢复上下文；栈受限，是因为处理过程共享被中断进程的内核栈、或使用大小有限的 per-CPU 中断栈；必须快进快出，是因为全程关着本地中断——你在 handler 里多待的每一微秒，都是全系统中断响应的盲区。

`request_irq` 的五个参数（`irq`、`handler`、`flags`、`name`、`dev_id`）里，`flags` 同时编码触发方式和共享标志，`dev_id` 是共享中断下区分各驱动 handler 的唯一凭据。上下文判断宏记四个：`in_irq()` 硬中断、`in_softirq()` 软中断、`in_interrupt()` 两者加 NMI、`in_atomic()` 计数非零——调试"为什么这里不能睡"的问题时先用它们确认自己站在哪。

黄金法则一句话：顶半部只做不得不做的事——读状态、清标志、调度底半部，其余一律下放。
