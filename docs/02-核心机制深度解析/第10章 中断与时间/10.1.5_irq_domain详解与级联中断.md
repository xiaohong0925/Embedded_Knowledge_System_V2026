...
上一节建立了 hwirq→virq 的三层映射概念和 GICv3 架构，但映射层本身还没拆：irq_domain 有哪几种实现？设备树里的 `interrupts` 属性在内核里具体走过哪些函数？主控制器上再挂一个 GPIO expander，中断信号层层传递，内核怎么组织这种级联？<BR>
本节覆盖：irq_domain 三种映射策略的选择依据、设备树中断解析的完整函数链、级联中断控制器的注册与 handler 写法、`/proc/interrupts` 的读法、smp_affinity 中断绑核实操与中断风暴排查。
...
每个中断控制器有自己的本地编号（hwirq），内核需要一个全局统一编号（virq）来管理所有中断。irq_domain 就是两者之间的映射层：它把硬件侧的 hwirq 翻译成内核侧的 virq。
...
**linear domain**是最直白的——内核维护一个数组，下标就是hwirq，存的是对应的virq号。查找一次数组访问搞定。代价呢？如果你的hwirq最大是1023，哪怕只用其中3个，数组也得开1024个槽位。适合"连续密集"的场景。
...
> 💡 linear domain 的数组大小由创建时传入的 `hwirq_max` 决定。GPIO 控制器有 128 路但只用到前 8 路时，可以把 `hwirq_max` 设小一点，省点内存。

### 设备树中断解析链路

你在设备树里写的`interrupts = <23 IRQ_TYPE_EDGE_FALLING>;`是怎么变成内核里的virq的？这条链路很长，但搞懂了排查中断问题就轻松多了。
...
现实硬件里，中断常常要"过两道手"。主GIC管着100多个SPI中断，其中一个SPI接到了GPIO expander（如PCF8574、PCA9555）上。这个GPIO expander自己又是中断控制器，管着8路GPIO中断。这就是**级联中断**——二级控制器的中断信号要借一级控制器的通道往上报。
...
> ⚠️ 级联场景下，`irq_alloc_descs()` 申请 virq 的基数（base）不能和父控制器的 virq 范围重叠。父控制器占了 virq 16~150 时，子控制器申请 irq_desc 若 base=0、cnt=8，就可能和已有系统中断冲突。正确做法是传 base=`-1`，让内核自动分配。

> 🔴 级联 handler 里忘了调用 `generic_handle_irq()` 分发子中断，子设备的中断处理函数就永远得不到执行。更隐蔽的是 handler 里没读清 expander 的状态寄存器：中断线一直被拉高，会引发中断风暴——CPU 被卡死在中断上下文出不来。
...
> ⚠️ 不要执行 `echo 0 > /proc/irq/<n>/smp_affinity` 来"恢复自动分配"——空掩码（不含任何 CPU）会被内核拒绝，返回 `Invalid argument`。恢复默认的正确做法是写入 `/proc/irq/default_smp_affinity` 的值；装了 irqbalance 的系统，恢复默认后由它自动均衡。
...
如果某个中断的计数每秒增加几千甚至上万，风暴源就找到了。下一步用`smp_affinity`把它绑到一个不重要的CPU上，然后抓`ftrace`看调用栈。
...
| 级联handler | 读pending寄存器 → `generic_handle_irq()`分发 → 返回IRQ_HANDLED；忘分发=子中断丢失，忘清状态=中断风暴 |
...
irq_domain解决了中断号的映射与管理，但还有一个关键问题：**中断handler本身应该怎么写？** 直接在中断上下文里做所有事情，还是拆成两半分工？10.2.1 顶半部的约束与设计哲学——为什么中断里不能睡眠、不能调度，以及顶半部的设计哲学。
