...
irq_domain 的选型只看 hwirq 的分布形状：连续号段用 linear（数组 O(1) 查找），稀疏号段用 tree（radix tree），legacy 已淘汰不用碰。中断解析的完整链路是设备树 `interrupts` 属性 → `irq_of_parse_and_map()` → `irq_create_fwspec_mapping()` → `irq_create_mapping()` → 返回 virq，驱动拿到的永远是 virq，硬件编号到此为止。

级联中断的本质是二级控制器借一级控制器的通道上报，两种模式的分工必须分清：Chained 模式用 `gpiochip_set_chained_irqchip()` 注册，在硬中断上下文里 `generic_handle_irq()` 直接分发；Nested 模式用 `devm_request_threaded_irq()`，handler 读 pending 后 `handle_nested_irq()` 在线程上下文分发——I2C GPIO expander 这类"读状态本身要睡眠"的控制器只能走后者。两个错误各对应一种经典故障：忘了分发，子中断永远丢失；忘了清状态，中断风暴立至。

排查侧三件工具按顺序用：`/proc/interrupts` 的每 CPU 计数暴露分布，控制器名列区分级联层级，计数突增就是风暴信号；`echo mask > /proc/irq/<n>/smp_affinity` 手动绑核隔离（空掩码会被拒绝）；`watch` 采样定位嫌疑中断后上 ftrace 跟调用栈找根因。
