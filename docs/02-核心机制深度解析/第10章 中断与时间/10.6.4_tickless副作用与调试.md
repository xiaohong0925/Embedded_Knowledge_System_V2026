...
>
> 第10章的知识拼图到这就齐了。下一节走进综合实战——GPIO 中断延迟超标和音频偶发爆音两个真实排查流程，前者用 ftrace irqsoff 抓到顶半部里的慢操作，后者用 hrtimer 精度测试量化出深 C-state 的唤醒延迟。你会用到本章几乎所有的调试工具，从 ftrace 到 timer_list，从 irq affinity 到 tickless 分析。
