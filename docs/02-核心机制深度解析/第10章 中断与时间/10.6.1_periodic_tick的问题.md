...
periodic tick 的问题不在 CPU 占用本身——几十微秒的中断处理不算什么——而在它把 C-state 锁死在浅层：CPU 刚进深睡就被下一个 tick 拉出来，idle 功耗因此比理论值高 5–10 倍，电池设备的续航直接缩一个量级。

这个开销与负载无关，满载和空载一个价，这正是它不合理的地方。<br>
解决方向不是把 HZ 调小（精度换功耗，两头不全），而是让 tick 按需触发——NO_HZ 系列选项，下一节展开机制。
