...
softirq 和 tasklet 够快，但有一个硬约束——**不能睡眠**。底半部里想调 `kmalloc(GFP_KERNEL)` 申请内存？不行。想拿 mutex？不行。想等一个 DMA 完成？更不行。<br>
workqueue 就是解除这个约束的方案：它把底半部搬到**进程上下文**的内核线程里执行，代价是上下文切换带来的额外延迟，换来的是可以调用任何内核 API 的自由。<br>
本节覆盖：workqueue 的核心数据结构、`queue_work()` 到函数真正执行的完整调用链、worker 线程池的运作方式、三种内置 workqueue 的选择，以及 ordered workqueue 的顺序保证。
...
如果底半部确实需要做这些事，唯一的出路是换执行上下文。workqueue 的做法很直接：**把活交给内核线程去干**。内核线程跑在进程上下文，和普通进程一样可以被调度、可以睡眠、可以阻塞。
...
`work_struct` 是要执行的任务本体，`workqueue_struct` 是任务的调度中心。典型的用法是把 `work_struct` 嵌在自己的设备结构体里，回调里用 `container_of()` 取回外层结构体。
...
注意第一行判断：`test_and_set_bit(WORK_STRUCT_PENDING_BIT, ...)`——**同一个 work 还没执行完时重复 queue，会被静默丢弃**（返回 false）。这和 tasklet 的 STATE_SCHED 防重入是同一类设计。

`__queue_work()` 找到目标 CPU 对应的 `pool_workqueue`（简称 pwq），把 `work_struct` 挂到 pwq 的待处理链表上；如果 pool 里没有正在运行的 worker，就唤醒或创建一个。

worker 线程的主体是 `worker_thread()`（workqueue.c:2729），主循环里不断调用 `process_one_work()`（:2536）：从 pool 链表中取出一个 work，清除 PENDING 位，然后调用 `work->func(work)`。执行到这里时，已经运行在进程上下文——可以睡眠、可以阻塞。
...
workqueue 子系统采用 cmwq（Concurrency Managed Workqueue）设计：**workqueue_struct 本身不拥有线程**，真正拥有线程的是每个 CPU（或 NUMA 节点）上的 `worker_pool`，`pool_workqueue` 是两者之间的桥梁：
...
多个 workqueue 可以共用同一个 worker_pool。pool 根据负载**动态创建和回收** worker 线程：有活没人干就创建，空闲超时（5 分钟）就回收。所以 `ps` 里看到的 kworker 数量是随负载变化的。
...
命名规则（workqueue.c 的 `create_worker()`）：`kworker/<cpu>:<id>` 是绑定 CPU 的 worker，`kworker/u<pool_id>:<id>` 是 unbound 的；名字里的 `H` 后缀表示高优先级 pool。线程名后面的 `-events` 等后缀来自它当前服务的 workqueue 名，是较新内核为方便排查加上的。
...
选择逻辑很直接：追求极致延迟且无需睡眠的场景（主要是内核网络、块层这类子系统）用 softirq；驱动开发的底半部，默认选 workqueue。tasklet 只在维护老代码时遇到。
...
{
	/* destroy_workqueue 会先 drain 所有 pending/running 的 work，
	 * 再销毁 pwq 和释放关联资源 */
	destroy_workqueue(my_wq);
	kfree(work_data);
}
...
> ⚠️ 在 work 回调里长时间 `msleep()` 会占住这个 worker 线程。对 per-CPU 的 `system_wq` 来说，一个长时间睡眠的 work 会拖累同 pool 上排队的其他 work——内核检测到 pool 卡住会创建新 worker 补救，但这有代价。需要长时间等待的任务，应该用专门的线程或独立 workqueue。

> 🔴 资源释放顺序：`destroy_workqueue()` 返回后，所有 work 保证已执行完毕。因此 work 回调里用到的资源（如 `work_data`）必须在 `destroy_workqueue()` **之后**释放；反过来，如果 work 已经 queue 出去但模块直接 `kfree(work_data)`，回调执行时访问的就是已释放内存。
...
v6.6 的真实数值（workqueue.c:397-399）：传 0 时使用默认值 `WQ_DFL_ACTIVE` = **256**；传入值被 `clamp_val()` 限制在 1 到 `WQ_MAX_ACTIVE` = **512** 之间。所以"传 0 不限制"的说法不对——它实际是 256。
...
`alloc_ordered_workqueue()` 内部就是 `WQ_UNBOUND | __WQ_ORDERED` 加 `max_active=1`：全局只有一个执行位置，所有 work 按 FIFO 顺序串行执行。典型用户是需要严格顺序的文件系统元数据操作等场景。

> 💡 排查 workqueue 相关问题时，先 `ps aux | grep kworker` 找到对应 worker 线程，再 `cat /proc/<pid>/stack` 看它当前卡在哪个调用栈上——worker 卡在 D 状态（不可中断睡眠）是 workqueue 类问题的典型症状。
...
> ⚠️ `cancel_work_sync()` 会等待正在执行的 work 完成，**不能在中断上下文调用**；如果 work 回调里又去拿调用者持有的锁，还会死锁。模块卸载路径上用它配合资源释放顺序设计。
...
workqueue 解决了"底半部不能睡眠"的问题，但如果面对的是**网络收包**这种场景——中断频率高达每秒几万甚至几十万次——tasklet 和 workqueue 的调度开销都会成为瓶颈。这时需要另一种思路：高流量时关掉中断改为轮询，流量降下来再恢复中断。这就是下一节 **10.3.5 NAPI中断与轮询的混合模式** 要讲的机制。
