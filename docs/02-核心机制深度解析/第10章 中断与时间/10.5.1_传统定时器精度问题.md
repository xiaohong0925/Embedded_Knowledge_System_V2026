...
上图的 tick 路径里，`update_process_times()`（kernel/time/timer.c:2064）负责更新进程统计与调度记账，`run_timer_softirq()` 随后才检查时间轮——**定时器到期判断只是 tick 待办清单中的一项**。<br>
...
传统定时器的精度天花板是 `1/HZ`，根因是 timer_list 的到期检查只发生在 tick 路径上——jiffies 不前进，就没有任何代码去看定时器一眼。<BR>
提高 HZ 是拿中断频率线性换精度，开销、缓存干扰、功耗同步上升，且 1000Hz 封顶也只有 1ms，够不到微秒级。

真正的出路是让定时器脱离 jiffies，直接编程硬件时钟的比较器——这就是 hrtimer 的方向，也是 10.5.2 分层架构要展开的铺垫。
