...
本节覆盖两部分：一是音频偶发爆音的完整排查流程——这类"细延迟"问题平均值完全正常、偶尔超标几百微秒，看 trace 抓不到，需要用 10.5.5 的 hrtimer 精度测试模块把问题量化出来；二是从 GPIO 延迟（10.7.1）和音频爆音两个案例中提炼的中断与时间子系统通用排错方法论，含系统化排查流程图、排错四步法、工具速查表和症状对照表。
...
10.7.1 的 GPIO 延迟排查和本节的音频爆音排查走完了，提炼一套通用的方法论。下次遇到中断或时间相关问题，按这个流程走，不会漏掉关键环节。<br>
...
10.7.1 的 GPIO 案例是"粗延迟"（800μs 级别），本节的音频案例是"细延迟"（偶发迟到 500μs）。粗延迟用 ftrace 一抓一个准，细延迟需要用精度测试模块来量化。两个思路互补，覆盖绝大多数实际场景。<br>
...
> **10.99 知识图谱与查漏补缺** — 一张图串起中断与时间子系统的全部要点，帮你检查有没有盲区。同时附有自测题和前后章关联图，方便复习和回查。
