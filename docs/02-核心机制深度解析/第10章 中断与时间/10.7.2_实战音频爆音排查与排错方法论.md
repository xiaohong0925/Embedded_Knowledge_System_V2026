...
10.7.1 的 GPIO 案例是"粗延迟"（800μs 级，irqsoff 一抓一个准），本节的音频案例是"细延迟"（平均值正常、偶发迟到数百微秒，必须用精度测试模块量化）<br>
——两类问题两种打法，合起来覆盖中断与时间子系统绝大多数实战场景。

两案共享同一条核心理念：**先测量，再分析，后修复，终验证**——没有数据的优化都是瞎猜。<br>
排错四步法和症状对照表建议放在手边，下次遇到同类问题先查表再动手。
