...
> 所属章节：第11章 设备模型 > 11.5 Overlay动态加载
> 难度：[E→M] | 预计阅读时间：15分钟
...
[图片占位：dtbo编译与加载流程图——上半部分：编译流程（.dts源文件 → dtc -@ -O dtb → .dtbo二进制 → 验证）。下半部分：加载流程（用户cat dtbo > configfs → 内核OF解析 → 符号解析 → 节点合并 → of_reconfig通知 → 新增platform_device → 触发probe）。风格：上下两栏流程图，编译在上加载在下]
