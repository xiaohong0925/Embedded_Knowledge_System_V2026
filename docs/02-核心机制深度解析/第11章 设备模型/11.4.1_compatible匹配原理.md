...
compatible 匹配是双方各出一张表的对赌：设备树节点的 compatible 列表从特定到通用排列，首项精确、末项兜底；驱动的 `of_device_id` 表声明支持名单，空哨兵 `{}` 结尾——表尾缺了它，匹配行为直接未定义。`of_match_node` 内部是评分制：命中节点列表中越靠前的字符串得分越高，所以**决定结果的是节点里的顺序，不是驱动表的顺序**，这一条记反了，排查优先级问题时会完全走反方向。

字符串比较不区分大小写、支持 `*` 通配，但绝不容错拼写差异——和绑定规范逐字符核对是唯一的保险。匹配失败的现场极其安静：设备在、driver 链接不在，无报错无 panic（`ls /sys/bus/<bus>/devices/<dev>/driver` 一看便知）。设备树侧的真相从源头查：`cat /proc/device-tree/.../compatible`。
