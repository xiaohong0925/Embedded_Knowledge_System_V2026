...
> 所属章节：第11章 设备模型 > 11.1 总线-设备-驱动模型
> 
> 难度：[I→M] | 预计阅读时间：12分钟
...
## <span class="blue"> driver_register()到probe()的完整调用链 [I][M] 
...
> 💡 回调返回非0会提前终止遍历，但`__device_attach_driver`匹配成功时返回0，所以**一个驱动可以匹配多个设备**。
...
> ⚠️ match返回0时内核**不打印任何日志**，silent fail。驱动注册了，设备也注册了，但什么都没发生, 这是新手最痛的debug场景。
...
<!-- [图片占位：match()到probe()调用链时序图, 纵向展示driver_register() → bus_add_driver() → driver_attach() → bus_match() → platform_match() → of_driver_match_device() → 匹配成功 → really_probe() → drv->probe()的完整调用链。每个函数用矩形框，箭头标注调用方向和返回值。风格：时序调用链图，纵向排列] -->

![图1：driver_register到probe的完整调用链序列图](images/11-driver_register%20到%20probe%20完整调用链时序图.png)

---

## <span class="blue"> 为什么probe()可能不被调用 [I] 
...
> 💡 先`insmod`设备模块再`insmod`驱动模块。通过`/sys/bus/platform/devices/`查看已注册设备，目录为空就是设备注册的问题。
...
> ⚠️ `driver_register()`返回0只代表"驱动加入了总线队列"，**不代表设备被匹配或probe被调用**。probe没执行时，先看match，再看设备在不在，再看设备是不是被绑走了, 按这个顺序排查，不会走弯路。
...
你已经搞懂了从`driver_register()`到`probe()`的完整调用链。下一节（11.1.4）我们将深入设备模型的基础设施, kobject和kset，理解sysfs目录树是怎么自动生成的。
