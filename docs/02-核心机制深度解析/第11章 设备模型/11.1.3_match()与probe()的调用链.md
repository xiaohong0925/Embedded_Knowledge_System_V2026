...
11.1.2 给出了 `bus_type`、`device`、`device_driver` 的静态结构，本节回答动态问题：驱动调用 `driver_register()` 之后，内核经过哪些函数、在哪一步比对 `match()`、又在哪一步进入 `probe()`。<br>
这是「设备从设备树节点到 probe 成功」主线中最核心的动态路径，也是「驱动注册了但 probe 没执行」这类问题的定位依据。<BR>
本节覆盖：`driver_register()` 到 `probe()` 的完整调用链逐层拆解（附精简源码）、match 失败的静默特性、probe 不执行的四种原因与排查顺序。
...
static int __device_attach_driver(struct device *dev, void *data)
{
...
static int really_probe(struct device *dev, struct device_driver *drv)
{
    ...
    // 调用总线层的 probe（新版本已移除）或驱动自己的 probe
    if (dev->bus->probe) {
...
int device_attach(struct device *dev)
{
    ...
    // ★ 遍历总线的 klist_drivers，对每个驱动执行 __device_attach
...
## <span class="blue"> 附：platform_match() 源码 —— match 到底怎么比 [I]

以 platform 总线为例，match 的优先级为：OF（设备树 compatible）> `id_table` > 设备名字符串。
...
static int platform_match(struct device *dev, struct device_driver *drv)
{
    ...
    /* 1. OF 匹配：设备树 compatible vs of_match_table，优先级最高 */
    if (of_driver_match_device(dev, drv))
        return 1;

    /* 2. id_table 匹配（ACPI 或老平台） */
    if (pdrv->id_table)
...
这个 `platform_match()` 就是 `bus_type` 结构体里的 `match` 函数指针。内核在 `driver_match_device()` 里调用它，返回 1 进入 probe，返回 0 静默跳过。
