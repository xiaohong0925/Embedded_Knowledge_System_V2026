...
[图片占位：platform_driver注册与匹配流程图——展示platform_driver_register() → __platform_driver_register() → driver_register() → bus_add_driver() → driver_attach() → platform_match()的完整链路。同时展示三种匹配条件：OF匹配 > id_table匹配 > name匹配。风格：流程图+优先级表]
