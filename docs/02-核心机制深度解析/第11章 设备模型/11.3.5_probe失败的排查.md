...
probe 失败的五类现场按主线顺序排除：设备未注册 → 匹配失败 → 资源获取失败 → 时钟电源未使能 → 硬件初始化错误。这个顺序就是排查顺序，跳着查是新手浪费时间的主要方式。

两个静默点要特别防备。匹配失败内核不打任何日志——匹配不上不构成错误，`drivers/base/dd.c` 的 attach 路径对此完全沉默，只能靠逐字符对比 compatible。deferred 卡死则是另一种沉默：`driver_deferred_probe_add()` 把驱动挂上待重试链表，provider 一天不就绪它就一天不重试，而 `devices_deferred` 遍历的正是这条链表，`dev_err_probe()` 记录的原因也在这里——两个沉默各有各的取证入口。

定位断点的组合拳：sysfs 三步走（devices 目录看设备在不在、drivers 目录看驱动在不在、driver 链接看绑没绑上），打印分级验证（probe 入口无打印 = 问题在匹配阶段；驱动注册处无打印 = 模块根本没加载）。先确认断在哪一环，再深入那一环的细节，这是本节全部方法论。
