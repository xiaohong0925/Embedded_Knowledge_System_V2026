...
第二、三步在 x86/PCIe 驱动里见不到，容易让有服务器或成熟板卡背景的读者困惑。根子是**主控平台对时钟和电源的所有权划分不同**：

| | x86/PCIe 平台 | ARM/RISC-V SoC 平台 |
|---|---|---|
| 参考时钟 | 板载晶振硬件走线直供，频率由规范定义 | 时钟树是可编程资源：PLL→分频器→门控，每个外设挂在可单独开关的叶子上 |
| 上电时序 | CPLD/PMIC 硬件状态机自动完成 | 外设供电常走 I2C 控制的 PMIC 或 GPIO 控制的 LDO/负载开关，开关控制端在 SoC 引脚上 |
| 驱动 probe 时的设备状态 | 已经带电带钟（link training 都做完了） | 可能没钟没电，驱动自己负责使能 |

SoC 这样设计的动机有两个：一是功耗——嵌入式设备散热和电池受限，不用的外设必须关钟断电（clock gating），内核默认"一切都可能是关着的，谁用谁开"；二是灵活性——同一颗 SoC 卖给几百家板厂，晶振频率和外设搭配各不相同，时钟配置没法烧死在硬件里，只能交给设备树描述 + 驱动使能。

> 💡 机器人、无人机这类产品是这两步的重灾区：激光雷达、相机、电机驱动器各自一路供电轨都要软件使能，多传感器时间同步依赖精确可配的时钟，电池供电又要求 runtime PM 反复断钟断电——enable/disable 配对写错就漏电或唤醒失败。

一个统一的认知模型：probe 前四步（取资源→取时钟→取电源→IO 映射）本质是**把"设备活着"这个前提从硬件保证变成软件保证**。硬件团队用 CPLD 托管一切的成熟板卡里这个前提已被保证，驱动可以直接从注册子系统开始；SoC 产品里这个前提要驱动自己建立。
...
probe 跑没跑的证据有两个：`dmesg` 里出现设备日志、`/dev` 节点出现，且日志里的地址要与设备树 reg 对得上。probe 内部的六步——取资源、取时钟、取电源、映射、注册子系统、初始化硬件——顺序不是规范要求，是依赖关系推出来的：后五步全都依赖前面的成果。骨架允许变形：框架可以接管某步（如 serial 框架的 `UPF_IOREMAP`），依赖不存在可以省步；判断标准是"当前顺序符合依赖吗"，而不是把六步当形式主义 checklist。

失败路径上有一条托管分界线：第五步之前申请的是 devm 托管资源，失败直接返回错误码即可，内核自动回收；之后的非托管资源，失败路径必须手动配对回收。`-EPROBE_DEFER` 要原样返回、绝不能当普通错误吞掉，`dev_err_probe()` 把打印和返回合成一步，defer 时还会降级记录到 `devices_deferred`——`cat /sys/kernel/debug/devices_deferred` 是查 defer 卡死的第一站。

最容易漏的账是 enable 不托管：clk/regulator 的 enable 是状态切换不是资源申请，错误路径和 remove 里都要配对 disable。写完 probe 回头数一遍清理链，enable 配对漏一个，复现的就是"第一次加载正常、卸载后再加载硬件不醒"。
