...
overlay 文件的结构四要素：`/plugin/` 声明身份、fragment 划分改动单元、target 指定落点、`__overlay__` 装实际内容。符号机制是它能运行时叠加的关键：基板编译加 `-@` 生成 `__symbols__`，overlay 编译加 `-@` 生成 `__fixups__`/`__local_fixups__`，符号解析推迟到运行时——所以**基板和 overlay 两边的 `-@` 一个都不能少**，缺一边就是加载时报符号错误。编译命令固定：`dtc -@ -O dtb -o xxx.dtbo xxx.dts`。

configfs 加载流程三步：`mkdir` 建目录、`cat dtbo > .../dtbo` 写入、`status` 读到 `applied` 才算成功；卸载就是 `rmdir` 对应目录，会触发相关驱动的 `remove()`，设备节点随之消失。内核侧的两个依赖先确认：`CONFIG_OF_OVERLAY=y` 和 `CONFIG_CONFIGFS_FS=y`，configfs 记得挂载。建议完整跑通一次"编译 → 加载 → 验证 → 卸载"闭环，这套流程的手感只能在板子上练出来。
