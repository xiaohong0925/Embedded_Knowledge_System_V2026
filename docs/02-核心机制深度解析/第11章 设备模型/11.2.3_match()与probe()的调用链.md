...
从注册到 probe 的调用链值得对着 `drivers/base/dd.c` 走读一遍：`driver_register` → `bus_add_driver` → `driver_attach` → `bus_for_each_dev` → `__driver_attach` → `match` → `really_probe` → `call_driver_probe` → 你的 `probe`。链上每一环都可能成为"probe 没跑"的断点，所以这条链就是排查地图本身。

match() 的语义要刻进直觉：返回 1 进 probe，返回 0 静默跳过——不打任何日志，匹配失败时内核是沉默的，只能靠逐字符对比 compatible 自己找。probe 未执行归因为四类：match 失败、设备未注册、已被别的驱动绑定、probe 内部失败，按这个最小顺序排除；系统化的排查流程在 11.3.5。

`-EPROBE_DEFER` 是这条链上最容易被误读的错误码：它不是失败，是"依赖未就绪，推迟重试"——驱动返回它之后会被挂上待重试链表，依赖就绪后内核自动再来一次。`dmesg` 里的 deferred probe 打印不是报错，是这套机制在正常工作。

---

## <span class="blue"> 下一步

调用链的每一层都发生在 kobject 组织的对象树之上：`bus_add_driver()` 创建的 sysfs 目录、`really_probe()` 发出的 uevent，都来自这套基础设施。下一节（11.2.4）展开这套基础设施的两条线：kobject/kset 如何自动投影出 `/sys` 目录树，以及 uevent 事件在内核侧的产生与广播（用户空间段留到 11.5.2）。
