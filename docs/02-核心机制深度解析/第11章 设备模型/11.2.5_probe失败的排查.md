...
> 所属章节：第11章 设备模型 > 11.2 platform设备驱动实战
> 难度：[I→M] | 预计阅读时间：12分钟
...
[图片占位：probe失败排查流程图——中心是probe失败，分五个分支：1.设备未注册（检查设备树status和compatible）→ 2.匹配失败（检查of_match_table和compatible）→ 3.资源获取失败（检查reg和interrupts）→ 4.时钟/电源未使能（检查clock-names和power-domain）→ 5.硬件初始化错误（检查返回值和dmesg）。每个分支标注排查命令。风格：故障排查树状图，从中心向外辐射]
...
你已经学会了probe失败的排查套路，下一节（11.3）我们将进入设备树与OF匹配的深水区——compatible字符串怎么配对、of_match_table怎么组织、设备树绑定规范有哪些坑。设备树写不对，驱动写得再好也白搭。
