...
模式⑤处理的是 PMIC、音频 Codec 这类一颗芯片多个功能的场景，写法上是三方协作：父驱动占住总线、提供 regmap，通过 `mfd_cells` 声明有哪些子功能；子驱动以 platform_driver 形式注册，各管一摊。配对方式与前面所有模式都不同——不靠设备树匹配表，靠子驱动 `.name` 与 `mfd_cells` 里的 `name` 字符串相等。

资源共享是 MFD 的精髓：所有子功能对寄存器的访问全部经过父设备的 regmap，缓存和锁都由它统一处理，子驱动之间永远不用担心互相踩寄存器。记住一条判断标准：当你发现多个驱动在抢同一片寄存器地址空间时，这颗芯片就该按 MFD 组织，而不是各写各的。
