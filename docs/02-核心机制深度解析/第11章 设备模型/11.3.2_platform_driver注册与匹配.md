...
`module_platform_driver` 宏背后是一条完整的注册封装链：宏展开 → `__platform_driver_register()` → `driver_register()`，源码在 `drivers/base/platform.c`。链上最关键的只有一行赋值——`driver.bus = &platform_bus_type`，它决定了这个驱动挂进 platform 总线的驱动链表，之后 11.2.2 讲的双向触发机制接管一切：设备和驱动任一方注册都会触发匹配，加载顺序不影响绑定。

`platform_match()` 的四个分支按 OF → ACPI → id_table → name 的优先级依次尝试，前一种命中即返回；`driver_override` 则旁路全部规则强制指定。匹配成功后的路径是 `really_probe()` → `call_driver_probe()` → `platform_probe()` → 你的 `.probe()`——在 probe 入口加一行打印，是确认链路走通的最快验证。

`MODULE_DEVICE_TABLE` 的职责边界要划清：它供 depmod 生成模块别名、实现自动加载（`modinfo` 可见），不参与运行时匹配——别指望靠它救配对。配对的最终结果在 sysfs 双侧互为镜像：设备目录的 driver 链接与驱动目录的设备链接，两边 `ls -l` 对照一看便知。
