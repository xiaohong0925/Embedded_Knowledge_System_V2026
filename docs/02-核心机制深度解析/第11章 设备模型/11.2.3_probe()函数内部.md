...
> 所属章节：第11章 设备模型 > 11.2 platform设备驱动
> 难度：[E→M] | 预计阅读时间：20分钟
...
[图片占位：probe函数六步典型流程图——probe被调用 → ①platform_get_resource → ②devm_clk_get → ③devm_regulator_get → ④devm_ioremap_resource → ⑤注册字符设备/MISC子系统 → ⑥初始化硬件（readl/writel/request_irq）。每个步骤用矩形框，标注典型API和注意事项。风格：步骤流程图，六个步骤横向排列]
...
probe()里六步走完了，硬件初始化好了，用户空间也能看见设备节点了。但驱动还没完——probe函数里申请的那么多资源（内存、时钟、中断）在驱动卸载时怎么自动释放？下一节（11.2.4）我们讲`devm_*`资源管理机制，看看内核是怎么帮你
