...
overlay 的本质是只描述"改动"的设备树片段：运行时叠加进基板设备树，无需重启——和"改 dts → 重新编译 → 烧录 → 重启"的传统流程相比，它把配置变更的代价从一次重启压缩到一次加载。三个典型场景都对得上这个价值：FPGA 动态重配置、扩展板热插拔、一份基板多种配置化出货。判断要不要用它，就问手头项目有没有"为小改动重启整机"的痛点。

能力边界同样要刻在脑子里：overlay 适合添加和覆盖，删除节点/属性不可靠，启动早期参数（memory、chosen 这类）它根本管不到。还有一个基板前提经常翻车：基板 dtb 必须带 `__symbols__` 表，编译时加 `-@`——`dtc -I dtb -O dts base.dtb | grep __symbols__` 一条命令验明正身，没有它，后面所有加载流程都是白费。
