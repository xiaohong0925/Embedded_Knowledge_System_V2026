...
这个设备的硬件地址从 0xfe660000 开始，占了 0x100 个字节（也就是256字节）。

最终映射到内核的 resource 结构里就是：

- start: 0xfe660000
- end: 0xfe6600ff（因为 0x100 个字节，最后一个地址是 start + 0x100 - 1）
- 长度: 256 字节
...
设备树节点变身 platform_device 的完整路径是：DTB 展开成 `device_node`（注意，它还不是设备模型的成员）→ `of_platform_default_populate_init` 在 `arch_initcall_sync` 阶段调用 `of_platform_populate()`（源码 `drivers/of/platform.c`），为"status 可用且带 compatible"的节点创建 platform_device。两个判定细节经常成为坑点：`status` 缺省、ok、okay 三种写法都算可用，其余跳过；递归展开只发生在命中 `of_default_bus_match_table`（simple-bus 等）的节点上——`/soc` 节点能展开就靠它。

resource 数组的翻译规则很直接：`reg` 变 MEM、`interrupts` 变 IRQ，顺序与设备树书写一致，驱动里 `platform_get_resource()` 按类型和索引取。自查从 `/proc/device-tree/` 开始，节点的 status 和 compatible 都能直接 cat 出来。

最后一个容易误解的点：sysfs 里显示的名字是驱动注册名，不是 compatible——rk3568 的串口挂的是 `snps,dw-apb-uart` 通用驱动，看到名字对不上瑞芯微别慌，`ls -l /sys/bus/platform/devices/*/driver` 里的链接才是真相。
