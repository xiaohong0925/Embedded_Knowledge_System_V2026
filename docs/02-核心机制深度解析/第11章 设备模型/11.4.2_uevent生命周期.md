...
[图片占位：uevent生命周期四步时序图——参与者：设备驱动、内核kobject层、netlink套接字、udevd守护进程。时间轴：设备注册 → kobject_uevent(KOBJ_ADD) → netlink广播 → udevd接收 → 匹配规则 → 执行规则 → 创建/dev节点。同时展示KOBJ_REMOVE的卸载路径。风格：时序图，四个参与者横向排列]
...
## 下一步
你已经搞懂了uevent从内核到用户空间的全流程，也知道了udev规则怎么匹配、怎么干活。下一节（11.4.3）我们将动手写udev规则——从语法到实战，看看怎么让系统自动给你的设备节点起别名、改权限、执行脚本。
