...
驱动调用 `device_register()`（底层是 `device_add()`），内核把 `struct device` 挂到总线上、在 sysfs 建好目录。`device_add()` 收尾时调用 `kobject_uevent(&dev->kobj, KOBJ_ADD)`，事件就此发出（内核侧的源码链在 11.2.4 逐行拆解）。注销路径对称：`device_del()` 发 `KOBJ_REMOVE`。
...
uevent 的完整链路四段：设备注册触发 `kobject_uevent` 组包，经 netlink 组播发出，udevd 收到后匹配规则执行。消息格式是 `键=值` 环境变量——ACTION、DEVPATH、SUBSYSTEM、MAJOR、MINOR、SEQNUM，插拔一个 USB 设备、开着 `udevadm monitor --kernel --udev` 看一遍，整条链路就活了。ACTION 的 add/remove/change/move 覆盖设备全生命周期。

历史上的 uevent helper（每事件 fork 一个进程）已被 netlink 全面取代，新系统应禁用——`cat /proc/sys/kernel/hotplug` 为空才是正常。`/dev` 节点的创建是两家分工：devtmpfs 建基础节点，udev 补命名、权限和符号链接；裁剪系统里如果发现节点有了但名字不对、权限不对，问题在后半程，先 `grep DEVTMPFS /boot/config-*` 确认前半程是通的。
