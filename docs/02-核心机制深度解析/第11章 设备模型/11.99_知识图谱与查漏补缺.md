...
> 所属章节：第11章 设备模型 > 11.99 知识图谱与查漏补缺
> 
> 难度：[I→M] | 预计阅读时间：20分钟

## <span class="blue"> 本节导读

本章从总线-设备-驱动三角关系到设备树匹配、再到sysfs和uevent，知识点繁多且互相关联。本节用两张知识图谱帮你建立整体视图，同时附上一份完整的查漏补缺清单，你可以逐条自检，确保没有遗漏关键概念。

---

## <span class="blue"> 一、知识图谱

[图片占位：设备模型知识图谱——总线层（bus_type）、设备层（device/platform_device）、驱动层（driver/platform_driver）、Kobject基础设施（kobject/kset/kobj_type/sysfs）、Uevent生命周期（KOBJ_ADD/REMOVE/netlink/udevd）、DT Overlay机制（DTB/Overlay/合并/of_reconfig）六大模块的关联关系图。用不同颜色区分六大层，用箭头标注调用关系和继承关系。风格：技术架构图，分层清晰]

### 调用链时序图

[图片占位：设备模型调用链时序图——从用户空间insmod/modprobe开始，依次展示：platform_driver_register() → driver_register() → bus_add_driver() → driver_attach() → bus_match() → platform_match() → of_driver_match_device() → 匹配成功 → really_probe() → drv->probe() → 设备初始化资源 → sysfs_create_group() → kobject_uevent(KOBJ_ADD) → netlink广播 → udevd接收 → 匹配规则 → 创建/dev节点。同时展示Overlay加载路径：of_overlay_apply() → of_reconfig → 新增platform_device → device_register() → 触发probe()。风格：时序图，纵向为时间轴，横向为参与者]
