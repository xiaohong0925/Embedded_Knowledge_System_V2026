...
<!-- 待补图：11.99-设备模型知识图谱.png。内容：六大模块——总线层（bus_type）、设备层（device/platform_device）、驱动层（driver/platform_driver）、kobject 基础设施（kobject/kset/kobj_type/sysfs）、uevent 生命周期（KOBJ_ADD/REMOVE/netlink/udevd）、DT overlay 机制（dtb/dtbo/合并/of_reconfig）的关联关系，不同颜色分层，箭头标注调用与继承关系。风格：技术架构图，分层清晰，不用漫画风 -->

### 调用链时序图

![设备模型调用链时序：从insmod到platform_driver_register、bus_match、really_probe、sysfs创建、uevent广播、udev创建/dev节点，并列展示overlay加载路径](images/11.99-设备模型调用链时序.png)

<!-- 待补图：11.99-设备模型调用链时序.png。内容：主线从用户空间 insmod/modprobe 开始，依次为 platform_driver_register()→driver_register()→bus_add_driver()→driver_attach()→bus_match()→platform_match()→of_driver_match_device()→really_probe()→drv->probe()→sysfs_create_group()→kobject_uevent(KOBJ_ADD)→netlink 广播→udevd 匹配规则→创建 /dev 节点；并线展示 overlay 路径：of_overlay_apply()→of_reconfig 通知→新增 platform_device→device_register()→触发 probe。风格：时序图，纵向时间轴，横向参与者，不用漫画风 -->
