...
本节覆盖：缓存行（cache line）概念、MESI 协议四状态与跨核同步代价、伪共享（false sharing）的成因与"缓存行乒乓"现象、`____cacheline_aligned` 对齐解法，以及用 `perf c2c` 定位伪共享的实操方法。
...
**MESI协议**就是解决这个问题的经典方案。说白了，MESI给每个缓存行（cache line）贴了一个"状态标签"，告诉CPU这行数据能不能读、能不能写、需不需要跟别的核心同步。

### 缓存行：数据搬运的最小单位

在深入MESI之前，你得先理解**缓存行（cache line）**这个概念。CPU缓存不是以单个字节为单位操作的，而是以一个固定大小的数据块——缓存行——为最小单位。典型的缓存行大小是**64字节**（x86_64 与主流 ARM64 均是）。这意味着，即使你只读取一个`int`（4字节），整个64字节的缓存行都会被加载到CPU缓存中。
...
<!--
【待补图·优先级★】文件路径：images/13.3.4-伪共享缓存行布局.png

生图说明（整段复制给生图 AI，中文标注）：
画面主题：伪共享（false sharing）的空间布局——两个逻辑无关的变量挤在同一条 64 字节缓存行里，被两个 CPU 反复"踢皮球"。
构图：俯视空间布局图，分三层。上层方框"CPU0（频繁写 counter0）"，下层方框"CPU1（频繁写 counter1）"，各自框内画一条缓存行副本。画面正中央画一条放大的水平长条代表"主内存中的 64 字节缓存行"：左端 8 字节用橙色高亮（标注"counter0，8 字节"），紧邻的 8 字节用蓝色高亮（标注"counter1，8 字节"），剩余 48 字节灰色（标注"无关数据"）。从中央长条向上下两个 CPU 各画一条双向红色锯齿箭头，箭头旁标注"一整行失效 ↔ 整行重新加载（cache line ping-pong）"。CPU0 框内缓存行的 counter0 段同样橙色高亮，CPU1 框内的 counter1 段蓝色高亮，直观呈现"两个核争的是同一条物理缓存行"。
风格：扁平矢量技术插图，白底，橙/蓝/灰三色为主，红色仅用于冲突箭头，细线框，16:9 横版。
禁用：照片质感、3D 效果、卡通元素、英文标注。
注意：生图 AI 对中文文字标注容易出错，生成后需人工核对所有文字，必要时后期补字。

图片生成完成后删除本注释块，替换为：
![伪共享缓存行布局：counter0与counter1落在同一64字节缓存行，CPU0与CPU1各自缓存整行，写任一变量都导致对方整行失效，缓存行在两核间来回弹跳](images/13.3.4-伪共享缓存行布局.png)
-->
...
两个CPU明明在写不同的变量，缓存行却在两个核心之间来回弹跳。这就是所谓的**缓存行乒乓（cache line ping-pong）**。性能可能暴跌到单核水平的零头。
...
> 💡 `____cacheline_aligned`（内核态）和`__attribute__((aligned(64)))`（GCC通用）会把变量对齐到缓存行边界。对于频繁被多核并发访问的统计计数器、生产者-消费者队列的索引变量，这招很管用，直接用独立缓存行把它们隔开。

> 🔴 对齐填充会浪费内存。一个`uint64_t`本来只占8字节，对齐后独占64字节，是原来的8倍。只在**真正被多核高并发访问**的变量上使用，别滥用。
...
> ⚠️ `perf c2c`需要特定的硬件支持（Intel Skylake微架构及以上）。旧款CPU（如Haswell、Broadwell）不支持HITM事件的精确采样，无法使用此工具。AMD平台也不支持`perf c2c`，需要用其他工具（如AMD uProf）代替。
...
缓存一致性协议是理解 RCU 的硬件背景——RCU 正是利用了现代 CPU 的缓存和内存序特性，实现了读端几乎零开销的并发机制。
