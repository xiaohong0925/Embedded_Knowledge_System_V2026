...
mutex 的路径分三段：快速路径一条 `atomic_long_try_cmpxchg_acquire` 尝试获取；失败先乐观自旋——等持有者释放，持有者被抢占才放弃，这是 v6.6 慢速路径的第一站；真正撑不住才进慢速路径：入 FIFO `wait_list`、设状态、循环 trylock 加 `schedule()` 睡眠、被唤醒后重试。释放时的 handoff 机制把所有权直接移交队首，避免一群等待者群抢。

优先级继承（PI）的归属必须说准：标准 mutex **没有** PI，PI 由 rt_mutex 提供（红黑树等待队列，深入内容在 8.4.2）；PREEMPT_RT 转换的是 spinlock_t/rwlock_t/local_lock/rw_semaphore，并不替换 mutex（见 10.4.3）。递归获取在 Linux 内核锁（含 rt_mutex）里一律不支持，重入问题靠重构调用路径解决，没有捷径。

最大禁忌压轴：mutex 不能在中断、软中断或 spinlock 保护区中使用——它会睡眠，而那些上下文不允许睡眠。这条和第 10 章的知识在这里正式合流。
