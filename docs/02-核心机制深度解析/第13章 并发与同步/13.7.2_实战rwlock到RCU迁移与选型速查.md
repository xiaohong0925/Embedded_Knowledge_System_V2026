...
rwlock 到 RCU 的迁移走五步：iperf3 打基准 → 改造为 copy-modify-publish 模式 → lockdep 加 KASAN clean → 功能验证 → 性能对比（读多写少场景预期 3 倍以上提升）。迁移的三条关键约束不能破：写者之间用 mutex 串行（RCU 不管写写并发）；发布必须用 `rcu_assign_pointer()`；旧版本数据必须等 grace period 结束才释放——这三条对应 13.4 的全部理论，这里是它们的实战考场。

选型速查浓缩为四档：读多写少用 RCU；高频短临界区用 atomic/percpu；中断上下文用 `spin_lock_irqsave()`；长临界区用 mutex。

核心原则收尾全章的实战部分：先测量再优化，先验证再提交，lockdep 开发阶段必开。同步原语没有银弹，但这三条纪律能保证你手里的每一发子弹都打在靶上。
