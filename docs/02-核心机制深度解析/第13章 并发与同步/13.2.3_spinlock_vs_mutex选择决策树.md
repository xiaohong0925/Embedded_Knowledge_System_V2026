...
本节覆盖：spinlock 与 mutex 选择的四问决策树（上下文能否睡眠、临界区长度、是否需要优先级继承、是否访问可能睡眠的资源）、7 种常见场景的速查表，以及三个典型错误选择的案例与修复方案。
...
`spinlock` 的本质是忙等待——CPU 反复执行锁状态检查，直到锁释放。临界区只有几条指令？没问题，空转时间忽略不计。但临界区里要遍历一个链表、拷贝一块缓冲区、或者调用复杂的计算逻辑？这时候用 `spinlock` 就是在浪费 CPU 周期，其他 CPU 上的线程干瞪眼等着。<br>

**经验法则**：临界区超过"几十个 CPU 周期"或者"可能触发 cache miss"的操作，优先考虑 `mutex`。`mutex` 会让等待者睡眠，把 CPU 让给其他任务，系统整体的吞吐率更好。

### 第三问：需要优先级继承吗？

如果你的代码运行在对延迟敏感的实时场景，需要正面应对优先级反转问题，这问就很关键。

先澄清归属：**标准 `mutex` 本身不带 PI**（13.2.2 已说明），提供优先级继承的是独立的 **`rt_mutex`**。`spinlock` 更不做这件事——在 RT 内核中普通的 `spinlock_t` 会被转换为基于 rt_mutex 的睡眠锁以获得 PI 语义（转换对照见 10.4.3），但那是内核内部的替换，不是你选锁的依据。<br>

需要 PI？→ `rt_mutex`（或依赖 RT 内核对 `spinlock_t` 的自动转换）。不需要？→ 继续看。
...
> 💡 "能睡用 mutex，不能睡用 spinlock，不确定用 irqsave"。这句话背下来，关键时刻能救命。最后一句话的意思是：如果你不确定当前上下文是否可能被中断打断，直接用 `spin_lock_irqsave()`——它自动关中断并保存中断状态，是最安全的保守策略。
...
> ⚠️ 在进程上下文中用 `spinlock` 保护长临界区，等于白白浪费 CPU。你可能会说"但我测试的时候没问题啊"——那是因为你的测试环境 CPU 负载低、核数多。一到生产环境高并发场景，这个问题就会暴露出来，而且定位起来极其痛苦。

> 🔴 永远不要在中断上下文里调用任何可能睡眠的函数。这条红线包括但不限于：`mutex_lock()`、`kmalloc(GFP_KERNEL)`、`copy_from_user()`、`schedule()` 以及所有的 I/O 操作。不确定一个函数是否睡眠？查它的源码，看有没有 `might_sleep()` 宏。

## <span class="blue"> 本节总结


选型四问按顺序过：第一问决定生死——不能睡眠的上下文（中断/atomic）必须用 spinlock，没有商量；第二问看时长——临界区长用 mutex，短则两者皆可；第三问看实时性——需要优先级继承上 rt_mutex（标准 mutex 无 PI，13.2.2 已划定归属）；第四问看内容——临界区里要访问 I2C/SPI 或做内存分配，必须用 mutex，因为这些操作可能睡眠。

保守策略只有一条：不确定中断状态时，`spin_lock_irqsave()` 兜底。口诀收口全节："能睡用 mutex，不能睡用 spinlock，不确定用 irqsave。"

决策树解决的是"选哪把锁"，但选对了不代表用对了——多把锁同时登场时，加锁顺序本身就是新的 bug 来源。下一节处理的就是这个问题。

## <span class="blue"> 下一步

锁选对了只是第一步。现实中多个锁嵌套使用的情况比比皆是，嵌套顺序出错就是经典死锁：A 等 B、B 等 A，双双卡住。
