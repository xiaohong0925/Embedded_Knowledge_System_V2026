...
写端五步把"复制替换"落成代码：copy 旧数据 → 修改副本 → `rcu_assign_pointer()` 发布 → 等 GP → 释放旧数据。发布这一步的内存序已由内核兜底：v6.6 的 `rcu_assign_pointer()` 用 `smp_store_release()`，release 语义确保新数据对其他 CPU 可见之后指针才被发布；读端 `rcu_dereference()` 是 READ_ONCE 加数据依赖序（无屏障），两端合起来构成完整的发布/订阅对。

等 GP 有两条路，按上下文选：`synchronize_rcu()` 同步阻塞，适合低频写、可睡眠上下文；`call_rcu()` 异步非阻塞，回调由 RCU 线程在 GP 后执行，适合高频写、不可睡眠上下文。禁区划死：`call_rcu` 的回调里禁止调 `synchronize_rcu()`——回调在等 GP，你让它等另一个 GP，死锁教科书。GP 等待通常几毫秒到几十毫秒，取决于负载，急用有加速版 `synchronize_rcu_expedited()`。

最后一条常被忽略：RCU 不保护写者之间的并发。多个写者同时改？自己加锁——RCU 管的是读写之间，不是写写之间。
