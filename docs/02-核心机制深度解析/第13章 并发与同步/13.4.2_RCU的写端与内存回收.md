...
上节搞懂了RCU的读端——读者不拿锁、不关中断，仅关抢占就能安全遍历数据。但RCU真正的复杂度和艺术感，全在写端。复制数据、修改副本、原子发布、等待 grace period、回收旧内存……每一步都暗藏内存序的玄机。<br>
本节带你走通RCU写端的完整五步流程，搞懂`rcu_assign_pointer()`背后的发布语义，以及`synchronize_rcu()`和`call_rcu()`的本质区别。学完后，你将能写出内核级别的RCU写端代码，也能理解为什么RCU写者"又慢又麻烦"却物有所值。
...
`rcu_assign_pointer()`看起来只是赋值一个指针，但它内部包含了一条`smp_wmb()`（写内存屏障），提供的是**发布语义（release semantics）**。说白了就是：**确保新指针写入之前，新数据的所有修改都已经对其他CPU可见**。

```c
/* rcu_assign_pointer 的伪代码逻辑 */
#define rcu_assign_pointer(p, v)                \
do {                                             \
    uintptr_t _r_a_p__v = (uintptr_t)(v);       \
    smp_wmb();          /* ① 写内存屏障 */       \
    WRITE_ONCE(p, _r_a_p__v);  /* ② 原子写指针 */ \
} while (0)
```
...
rcu_assign_pointer(global_ptr, new);   // 屏障+原子发布
```

没有`smp_wmb()`的话，编译器和CPU可能把"写`global_ptr`"重排序到"写`new->value`"之前——也就是说，别的CPU可能先看到了新指针，但新数据还没初始化完。`rcu_assign_pointer()`里的屏障杜绝了这种乱序：**所有对新数据的写入，都必须在指针发布之前完成**。

> 💡 **提示**：读者那边配合的是`rcu_dereference()`，它内部包含`smp_rmb()`（读内存屏障），与写端的`smp_wmb()`成对使用。一发布、一订阅，天衣无缝。

---

### 第④步：等待 Grace Period —— 两种选择

指针已经发布，新读者立刻就能看到新数据。但老读者呢？可能还有CPU正在`rcu_read_lock()`里拿着旧指针遍历呢。这时候释放旧数据，就是在玩火。
...
> 💡 **提示**：`synchronize_rcu()`通常在几毫秒到几十毫秒内返回，具体取决于系统负载和RCU配置。对于非频繁写操作来说，这点延迟完全可以接受。
...
> ⚠️ **陷阱**：`call_rcu()`注册的回调函数中**绝对不能调用`synchronize_rcu()`**。为什么？回调在RCU内核线程的上下文执行，而`synchronize_rcu()`要等 grace period 结束——如果回调里调了它，就形成了"等自己完成"的死循环。内核会检测到这种情况并报`BUG_ON`，但那时候你的系统已经 hang 了。
...
> 🔴 **危险**：写端代码通常需要用锁（如mutex或spinlock）保护，防止多个写者同时修改。上方的`rcu_dereference_protected()`表示"我在持有锁的情况下解引用，lockdep请不要报警"。记住：RCU保护的是读者和写者之间的并发，**写者之间的并发需要自己加锁**。
...
| `rcu_assign_pointer()` | 含`smp_wmb()`写屏障，提供发布语义，确保新数据可见后才发布指针 |
...
| GP等待时间 | 通常几毫秒到几十毫秒，取决于系统负载 |

---

## <span class="blue"> 下一步

13.4.3 RCU变体与适用场景——我们将介绍Sleepable RCU（SRCU）、 Tasks RCU等面向特殊场景的RCU变体，以及一张"什么场景选什么同步机制"的决策地图。RCU不是银弹，有些场景它比锁更差——下节告诉你怎么避开这些坑。
...
| 代码清单 | `rcu_assign_pointer()`的屏障语义展开 | 上方伪代码 |
