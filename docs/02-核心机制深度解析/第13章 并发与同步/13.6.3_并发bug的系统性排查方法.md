...
> 所属章节：第13章 并发与同步 > 13.6 调试与排错
>
> 难度：[I][M] | 预计阅读时间：25分钟

## <span class="blue"> 本节导读

并发Bug就像幽灵——它不在固定的地方出现，却总在最关键的时刻给你致命一击。你加了printk，问题消失了；你恢复代码，问题又回来了。这种"海森堡效应"让无数开发者抓狂。<br>

本节给你一套经过实战验证的**四步排查方法论**，并总结了Linux内核中最常见的并发Bug模式。学完后，你将拥有一张清晰的"作战地图"——面对随机崩溃、数据不一致、死锁等问题，知道该用什么工具、按什么顺序排查，而不是盲目地到处加锁碰运气。

> 💡 **提示**：先确认bug类型再选工具，不要一上来就启用所有调试选项。很多人遇到并发问题就同时开启KASAN + KCSAN + lockdep + ftrace，结果系统慢得像蜗牛，日志淹没了真正有用的信息。精准定位，逐个击破。

---

## <span class="blue"> 四步排查方法论 [I][M]

面对一个疑似并发Bug，你的第一反应是什么？加锁？加volatile？还是开始随机注释代码？<br>
...
不同的并发Bug有不同的"面孔"。你的任务是在第一步就搞清楚面对的是哪类怪物。
...
Lockdep是Linux内核的锁依赖验证器，它能在运行时检测锁的死锁循环、不一致的加锁顺序，以及IRQ安全问题。
...
modprobe/1503 is trying to acquire lock:
 (&sb->s_type->i_mutex_key#7){+.+.+.}, at: [<c01b6bb5>] inode_lock+0x19/0x1b
...
> 💡 **提示**：Lockdep的报告虽然看起来吓人，但格式其实很规范。重点关注"trying to acquire lock"和"already holding lock"这两行——它们直接指出了冲突的锁对。
...
|------|---------|-------------|---------|---------|
| KASAN | Use-after-free, 越界访问 | `CONFIG_KASAN=y` | 约2-3x slowdown | `KASAN: use-after-free in foo_read` |
| KCSAN | Data Race | `CONFIG_KCSAN=y` | 约10-20x slowdown | `KCSAN: data-race at ip_schedule_irq+0x123` |
...
void foo_read(char *out)
{
    ...
    memcpy(out, global_buf, 100);  // ⚠️ 可能已经释放了！
}
```
...
> ⚠️ **陷阱**：不要盲目加锁！看到数据竞争就随手加一个spinlock，这是新手最常见的错误。加锁顺序如果和已有代码冲突，可能引入新的死锁。每加一把锁，都要检查：这把锁和已有锁是否存在顺序冲突？是否可能在已有锁持有的情况下被获取？
...
> 💡 **提示**：注意流程图最后有一个"失败→回到Step 1"的回路。这很关键——有时候你以为找到了根因，修完之后压力测试却报出新问题。这说明你看到的只是冰山一角，真正的并发问题可能更深层。不要气馁，这是并发调试的常态。
...
| 模式 | 典型表现 | 根因 | 修复策略 |
|------|---------|------|---------|
| **模式1：忘记关抢占** | 低优先级任务持有锁时被抢占，高优先级任务死等 | `preempt_disable()`缺失，或spinlock配置错误 | 检查`CONFIG_PREEMPT`下的锁实现，确保抢占被正确禁用 |
...
| **模式4：RCU读者中睡眠** | `BUG: scheduling while atomic` 出现在rcu_read_lock保护区域 | 在`rcu_read_lock()`/`rcu_read_unlock()`之间调用了可能睡眠的函数 | 将睡眠操作移出RCU读临界区，或使用`synchronize_rcu()`等待 |
...
```c
// 错误示例：在PREEMPT_RT内核中，raw_spinlock不会关抢占！
void buggy_func(void)
{
    raw_spinlock_t *lock = &my_lock;
    
    raw_spin_lock(lock);
    // 如果CONFIG_PREEMPT_RT，这里可能被抢占！
    // 高优先级任务如果也尝试获取my_lock，就会死等
    do_something_long();
    raw_spin_unlock(lock);
}

// 正确做法：明确需要关抢占时使用spin_lock
void fixed_func(void)
{
    spin_lock(&my_lock);  // 在RT下会正确关抢占
    do_something_long();
    spin_unlock(&my_lock);
}
```
...
> 🔴 **危险**：`spin_lock_irqsave`是中断上下文中唯一安全的锁选择。如果你在中断里只用`spin_lock`，而对应进程上下文用了`spin_lock_irqsave`，看起来都加了锁，但不同CPU上的中断和进程可能并发执行，还是会竞争！必须两边都用`_irqsave`才能确保中断状态被正确保存和恢复。
...
> 💡 **提示**：**ftrace是不需要重新编译内核的！** 这是它最大的优势。在生产环境中，如果问题偶发且难以复现，优先考虑用ftrace收集现场信息，而不是试图重编一个带KASAN的内核。很多关键问题只出现在特定的负载条件下，换了内核可能就复现不了了。
...
| **四大Bug模式** | 忘关抢占、中断无保护访问、释放锁后用数据、RCU读者睡眠 |
| **工具选择原则** | 先ftrace（不编译），再KASAN/KCSAN（需编译），不要同时全开 |

> ⚠️ **陷阱**：并发Bug最危险的地方在于**它们往往成对出现**。你修了一个use-after-free，可能暴露出一个更深层的锁顺序问题。每次修复后都要跑完整的Lockdep + KASAN回归测试，确保没有引入新问题。

---

## <span class="blue"> 下一步

光说不练假把式。下一节 **13.7.1 综合实战：一次并发崩溃的完整排查过程**，我们将跟随一个真实的案例——从偶发的`Unable to handle kernel NULL pointer dereference`出发，走过确认Bug类型、启用KASAN、解读报告、代码审查、修复验证的完整流程。你会看到，一个看似简单的空指针解引用，背后隐藏着怎样的并发陷阱。
...
| 资源类型 | 内容 | 获取方式 |
|---------|------|---------|
| **内核文档** | Lockdep详细说明 | `Documentation/lockdep-design.rst` |
...
| **代码清单** | 本节四个Bug模式的完整修复代码 | `samples/concurrent-bug-patterns/` |
| **诊断脚本** | 根据症状自动推荐工具的脚本 | 配套脚本：`tools/concurrency-diagnose.sh` |
