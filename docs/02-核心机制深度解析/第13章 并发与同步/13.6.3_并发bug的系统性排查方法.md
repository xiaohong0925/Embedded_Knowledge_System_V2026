...
系统性排查四步走：确认 bug 类型 → lockdep 检测锁问题 → KASAN/KCSAN/ftrace 动态分析 → 代码审查验证修复。第一步从症状反推类型：数据不一致是缺同步，卡死是死锁，随机崩溃是内存破坏，atomic 睡眠是上下文违规——类型判错，后面全白跑。代码审查阶段的三问收口：共享数据有 Owner 吗？锁顺序一致吗？中断访问有保护吗？

四大高频 bug 模式直接背：per-cpu 忘关抢占、中断路径无保护访问、释放锁后还用数据、RCU 读者睡眠。工具使用讲顺序：先 ftrace（不用重新编译），再 KASAN/KCSAN（要重编内核），而且不要同时全开——插桩叠加会改变时序，把 Heisenbug 藏得更深。

最后一条用教训写成：并发 bug 往往成对出现——修掉一个 use-after-free，可能暴露更深层的锁顺序问题。每次修复后跑完整的 lockdep + KASAN 回归，确认没有引入新问题，这一步省了，下一个崩溃已经在路上。
