...
两个实战各走一条完整链路。"scheduling while atomic"：oops 分析 → decodecode 定位 → 确认原子上下文里的睡眠函数 → 缩短临界区或把工作移到 workqueue → 24 小时压力验证。AB-BA 死锁：softlockup 症状 → 开 `CONFIG_LOCKDEP=y` → 解读 circular dependency 链（13.6.1 的方法）→ 统一锁顺序或 trylock 回退 → lockdep clean 验证。

修复时的取舍有明确先后：缩短临界区优先于更换原语——多数"睡眠"问题是临界区里塞了不该塞的活；统一锁顺序优先于 trylock 回退——顺序一致是结构性解法，trylock 是防御性补丁。

验证标准两条都是硬指标：24 小时以上压力测试，加 lockdep clean。并发修复"看起来好了"和"真的好了"之间，隔着的就是这两个数字——少一个都别合入。
