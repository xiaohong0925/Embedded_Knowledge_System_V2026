...
本节覆盖：per-cpu 变量的核心思想（每个 CPU 一份独立副本，从根上消除共享）、静态声明与动态分配两套 API、`get_cpu_var()`/`put_cpu_var()` 关抢占配对的原因、跨 CPU 汇总遍历的方法，以及适用与不适用的边界。
...
每个CPU有独立的副本，访问时**不需要任何锁**，也不需要原子操作。数据天然就是隔离的，没有竞争可言。

per-cpu的另一个好处是**避免缓存伪共享**（false sharing，机制见 13.3.4）。多个CPU各自操作自己的变量，这些变量存放在不同的cache line里，不会互相踩踏。相比全局变量被多个CPU抢来抢去，性能提升非常明显。
...
> ⚠️ `get_cpu_var()`和`put_cpu_var()`**必须严格配对**。如果在`preempt_enable`之前忘了调用`put_cpu_var()`，当前CPU的抢占就永远关着，调度器无法抢占这个任务，轻则延迟抖动，重则系统僵死。这比你想象的要容易犯——特别是代码里有多个return路径的时候。
...
> 🔴 `free_percpu()`必须确保所有CPU都不再访问这个变量。如果某个CPU还在`get_cpu_ptr`和`put_cpu_ptr`之间操作，释放后就是UAF（Use-After-Free）崩溃。

### 遍历所有CPU：汇总全局视图

既然每个CPU有自己的副本，那想看总量怎么办？遍历所有CPU求和：
...
> 💡 `for_each_possible_cpu()`遍历的是系统所有"可能存在"的CPU（包括离线的），而不是当前在线的。做全局汇总时，只要各CPU的数据互不干扰，遍历全部再求和就能得到精确的全局视图。如果只需要在线CPU，改用`for_each_online_cpu()`。
...
per-cpu 的核心思想釜底抽薪：每个 CPU 一份独立副本，消除共享，从而消除竞争——无锁、无原子操作、无缓存伪共享，性能天花板。声明分两路：静态用 `DEFINE_PER_CPU(type, name)` 编译期分配，动态用 `alloc_percpu()`/`free_percpu()` 运行期管理。访问的关键在配对：`get_cpu_var()`/`put_cpu_var()` 关抢占、开抢占必须成对；`this_cpu_ptr()` 只取指针不关抢占，安全性自己担保。全局汇总用 `for_each_possible_cpu()` 加 `per_cpu(var, cpu)` 遍历各副本求和。

核心限制与优势同根：各 CPU 副本之间没有自动同步，所以全局一致性要靠自己汇总——这也意味着它天生只适合"各自记账、定期汇总"的场景：CPU-local 计数器、统计、临时缓冲区。

两个陷阱都是配对问题：get/put 不配对造成抢占泄漏（调度器行为从此诡异）；free 时还有 CPU 在访问，就是教科书级的 use-after-free。

---

## <span class="blue"> 下一步

per-cpu 解决的是"各算各的"场景，但有些情况你需要等待某个异步事件完成——比如发起 I/O 请求后阻塞等结果。
