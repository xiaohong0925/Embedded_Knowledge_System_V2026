...
> 所属章节：第13章 并发与同步 > 13.6 调试与性能工具
>
> 难度：Expert | 预计阅读时间：20分钟

## <span class="blue"> 本节导读

Lockdep擅长检测**锁的错误使用**，但它对另一类bug无能为力：内存访问错误和data race。试想，一个指针被kfree后又去访问——这就是use-after-free；两个CPU同时无锁修改同一个变量——这就是data race。这些bug不会触发lockdep的任何警告，却能让系统在几小时甚至几天后才莫名其妙地崩溃。<br>
本节介绍Linux内核的两把"精密手术刀"：**KASAN**（检测内存访问错误）和**KCSAN**（检测data race）。学完后，你将能独立启用这两个工具、读懂它们的报告、并在实战中定位并发相关的内存bug。

---

## <span class="blue"> KASAN —— 内核的"体检仪" [E][M]

KASAN（Kernel Address Sanitizer）从4.0版本开始引入内核，设计思路简单粗暴：**在每次内存访问前插入检查代码**，一旦发现访问了不该访问的内存，立即报panic。
...
> 💡 **提示**：红区（redzone）是KASAN在合法内存周围插入的"警戒区"。访问红区=越界。
...
# 内核配置（5.8+）
CONFIG_KCSAN=y
CONFIG_KCSAN_DELAY=80            # 采样延迟（纳秒）
CONFIG_KCSAN_SKIP_WATCH=4000     # 采样率控制

# 启动参数（可选）
kcsan.skip_watch_random=100      # 提高采样率（调试时）
```
...
| **使用阶段** | 开发/调试阶段 | 开发/调试阶段 | 始终可用（推荐常驻） |
| **配置选项** | `CONFIG_KASAN=y` | `CONFIG_KCSAN=y` | `CONFIG_LOCKDEP=y` |
| **典型输出** | UAF完整报告 | Data race报告 | 死锁链/递归警告 |
| **误报率** | 极低 | 较低（可能有 benign race） | 较低 |

> ⚠️ **陷阱**：KASAN的内存开销很大（影子内存占1/8），**绝对不能在生产环境启用**。只用于开发调试阶段。有些嵌入式板子内存本就紧张，开了KASAN可能直接OOM。

> 💡 **提示**：KCSAN在5.8+内核中可用，是data race检测的**首选工具**。如果你遇到"变量值莫名其妙不对"的bug，而KASAN又没报任何错误，试试KCSAN——八成是data race。
...
| **使用场景** | 两者都用于开发调试，不可用于生产 |

---

## <span class="blue"> 下一步

学会了检测工具的原理和使用，但现实中的并发bug往往更隐蔽——可能涉及多种问题交织。下一节 **13.6.3 并发bug的系统性排查方法**，将带你建立一套完整的排查流程：从现象到根因，从猜测到验证，面对最棘手的并发问题也能有条不紊。
...
| 示例代码 | 本节的buggy模块 | `code/buggy_uaf.c`、`code/buggy_race.c` |
| 内核配置 | 调试相关配置汇总 | `code/debug_config.fragment` |
