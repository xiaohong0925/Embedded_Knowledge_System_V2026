...
KASAN 和 KCSAN 的分工一句话：KASAN 管"内存对不对"，KCSAN 管"访问乱不乱"。KASAN 靠编译插桩加影子内存（1:8 映射），每次内存访问前检查，抓 use-after-free、堆/栈/全局越界、双重释放；开 `CONFIG_KASAN=y` 的代价是内存多 12.5%、CPU 减速 2~4 倍。KCSAN 走另一条路：延迟采样、随机检查内存访问冲突，专抓 data race——两个 CPU 无锁访问同一变量且至少一个在写；`CONFIG_KCSAN=y`（5.8+）只减速约 10%。

使用边界要清醒：两者都是开发调试工具，都不能上生产。生产环境想要类似的兜底，用 KFENCE——低开销采样版的 KASAN，这才是为常驻运行设计的形态。

它们和 lockdep 合起来正好覆盖并发 bug 的三个面：锁顺序（lockdep）、内存正确性（KASAN）、数据竞争（KCSAN）。开发机上三件齐开，并发 bug 能藏身的角落就不多了。
