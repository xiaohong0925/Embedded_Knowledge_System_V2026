...
spinlock 的本质是忙等：获取失败就循环检查，绝不让出 CPU。v6.6 的实现是 qspinlock（MCS 队列）——ARM64 早已不是 ticket lock，别再看老资料；快路径只有一条 `atomic_try_cmpxchg_acquire`，慢路径上等待者挂 per-CPU qnode、各自在本地缓存行自旋、FIFO 唤醒。内存序已由实现兜底：加锁自带 acquire、解锁是 `smp_store_release`，临界区内部不需要额外屏障。

四种变种的选用看上下文：`spin_lock()` 是基本款；`_irq()` 关本地中断；`_irqsave()` 保存中断状态后再关，是最安全的默认选项；`_bh()` 关底半部。持锁不能睡眠的原因值得说透：持锁者一旦睡眠，同 CPU 的竞争者会自旋占满 CPU，持锁者永远得不到调度机会——死锁由此铸成。机制上的看守是 preempt_count（持锁时大于零挡住抢占点）和 `might_sleep()`（主动睡眠当场抓出）。

使用四法则收口：临界区越短越好（微秒级）；临界区内绝不睡眠；不确定中断状态时一律 `spin_lock_irqsave()`；中断上下文分配内存只能用 `GFP_ATOMIC`。
