...
> 所属章节：第13章 并发与同步 > 13.5 内核同步机制进阶
>
> 难度：[E] Expert | 预计阅读时间：25分钟

## <span class="blue">本节导读

内核里有大量场景需要"等某个事件发生了再继续"——驱动probe要等硬件就绪，read()要等数据来，进程A要等进程B干完某件事。自旋锁和互斥锁都搞不定这种"等待-通知"模型，怎么办？<br>
本节带你深入Linux内核的两种核心等待机制：**completion**（完成通知）和**waitqueue**（等待队列）。学完它们，你就能优雅地处理"事件通知"和"条件等待"这两大类同步问题，而不是傻乎乎地自旋烧CPU。<br>
学完后，你将掌握completion和waitqueue的API用法、适用场景，以及它们与互斥锁的本质区别。
...
想想DMA传输：你发起一个DMA请求，硬件传完数据后触发中断，中断处理函数里调用`complete()`，用户态的`wait_for_completion()`被唤醒，继续处理数据。这个流程里，`complete()`就是那一声"好了！"
...
> 💡 **提示**：completion的设计哲学是"做一次就完事"——`complete()`发出信号后不会自动清零，如果你需要重复使用同一个completion，必须在下次等待前调用`reinit_completion()`重新初始化。
...
> ⚠️ **陷阱**：`complete()`只唤醒**一个**等待者。如果你的代码里可能有多个进程同时`wait_for_completion()`在同一个completion上，却调用了`complete()`而不是`complete_all()`，那剩下的进程将永远睡下去——经典的"丢唤醒"bug。
...
waitqueue在内核里无处不在：`read()`系统调用等数据、`poll()`等事件、`select()`等fd就绪，底层都是waitqueue在撑腰。
...
// 或加入可中断等待队列：
add_wait_queue_interruptible(&my_wq, &wait);
...
另一处代码（比如中断处理函数、生产者）在条件满足时唤醒等待者：

```c
wake_up(&my_wq);                    // 唤醒一个等待者（可中断的也会被唤醒）
wake_up_all(&my_wq);                // 唤醒所有等待者
wake_up_interruptible(&my_wq);      // 只唤醒可中断等待者
wake_up_interruptible_all(&my_wq);  // 唤醒所有可中断等待者
```

### 推荐写法：wait_event宏

手写的"add_wait_queue + set_current_state + schedule + remove_wait_queue"太啰嗦，内核提供了一组宏来简化：
...
> ⚠️ **陷阱**：`wait_event()`系列宏内部已经帮你处理了**条件检查循环**——被唤醒后会自动重新判断condition，不满足就继续睡。但如果你手写waitqueue代码（不用宏），一定要记得在被唤醒后重新检查条件，因为`wake_up`可能是个假唤醒。
                       ...
                       size_t count, loff_t *off)
{
    ...
    /* 等待缓冲区有数据 */
    // wait_event_interruptible：可中断等待，条件dev->data_len > 0
...
| 特性 | completion | waitqueue | semaphore/mutex |
|------|-----------|-----------|-----------------|
| **核心语义** | 事件通知（一次性） | 条件等待（可循环） | 互斥访问（可循环） |
| **等待方式** | 进程睡眠 | 进程睡眠 | 进程睡眠（mutex）或忙等（semaphore） |
| **条件判断** | 无，直接等信号 | 有，等condition为真 | 无，等锁可用 |
| **计数器** | 内部done计数，但不对外暴露 | 无计数器 | 有明确的计数值 |
| **一次性/可重用** | 一次性，需`reinit_completion`重置 | 可循环使用 | 可循环使用 |
| **唤醒行为** | `complete`唤醒1个，`complete_all`唤醒全部 | `wake_up`唤醒1个，`wake_up_all`唤醒全部 | 按等待顺序逐个唤醒 |
| **适用场景** | DMA完成、probe完成、异步任务完成 | 等缓冲区非空、等fd就绪、等数据到来 | 保护临界区、资源计数 |
| **典型代码** | `wait_for_completion()` + `complete()` | `wait_event_interruptible()` + `wake_up()` | `mutex_lock()` + `mutex_unlock()` |

> 💡 **提示**：选错机制会引入隐蔽bug。规则很简单：只需要"做完了通知一声"用completion；需要"等某个条件成立"用waitqueue；需要"保护共享数据不被同时访问"用mutex。别混着用。

## <span class="blue">wake_up函数族一览

| 函数名 | 唤醒数量 | 可被信号中断 | 适用场景 |
|--------|---------|-------------|---------|
| `wake_up()` | 1个 | 唤醒所有状态 | 通用唤醒，唤醒任一个等待者 |
| `wake_up_all()` | 全部 | 唤醒所有状态 | 广播场景，所有等待者都需要处理 |
...
| `wake_up_nr()` | 指定N个 | 唤醒所有状态 | 精确控制唤醒数量 |
| `wake_up_interruptible_nr()` | 指定N个 | 只唤醒`TASK_INTERRUPTIBLE` | 精确控制唤醒可中断等待者 |

> 🔴 **危险**：`wake_up()`和`wake_up_all()`会唤醒**所有状态**的等待进程（包括`TASK_UNINTERRUPTIBLE`）。如果你不想唤醒不可中断的进程，一定要用`wake_up_interruptible`系列。唤醒不该唤醒的进程可能导致不可预期的行为。

## <span class="blue">新知识点258：waitqueue机制深入
...
| `add_wait_queue(&wq, &wait)` | 将等待项加入队列（非独占） | 广播等待场景，所有等待者都需要被唤醒 |
| `add_wait_queue_exclusive(&wq, &wait)` | 将等待项加入队列（独占） | 资源竞争场景，每次只唤醒一个等待者 |
| `remove_wait_queue(&wq, &wait)` | 从队列中移除等待项 | 等待结束后清理，或中途取消等待 |
| `prepare_to_wait(&wq, &wait, state)` | 加入队列 + 设置进程状态 | 手写等待循环时使用，比单独调用add+set更原子 |
| `finish_wait(&wq, &wait)` | 设置进程为RUNNING + 从队列移除 | 等待循环结束时调用，无论条件是否满足 |
| `wake_up(&wq)` | 唤醒队列中的一个等待者 | 通用唤醒，唤醒任意一个（优先非独占） |
| `wake_up_interruptible(&wq)` | 只唤醒`TASK_INTERRUPTIBLE`状态的等待者 | 大多数场景首选，避免唤醒不可中断进程 |
| `wake_up_locked(&wq)` | 调用者已持有队列锁的版本 | 在中断上下文或已加锁的场景中避免死锁 |

### 等待状态：TASK_INTERRUPTIBLE vs TASK_UNINTERRUPTIBLE

| 状态 | 可被信号唤醒 | 可被`wake_up`唤醒 | 区别 | 使用场景 |
|------|-------------|-------------------|------|----------|
| `TASK_INTERRUPTIBLE` | **是** | **是** | 睡眠期间收到信号会提前返回 | 绝大多数驱动等待场景，read/poll/select |
| `TASK_UNINTERRUPTIBLE` | **否** | **是** | 忽略所有信号，只能被显式唤醒 | 等待极短且必须完成的操作，如磁盘I/O、页换入 |

`TASK_UNINTERRUPTIBLE`的一个危险变体是`TASK_KILLABLE`（内核2.6.25引入）：只响应致命信号（SIGKILL），其他信号忽略。长时间等待推荐使用`TASK_INTERRUPTIBLE`，否则不可中断的睡眠进程用`kill -9`都杀不掉，会让运维非常头疼。
...
> ⚠️ **陷阱**：如果你手写等待代码时用了`if (!condition)`而不是`while (!condition)`，虚假唤醒（spurious wakeup）会直接跳出判断，导致在条件并未真正满足时继续执行。内核中`wake_up`不保证条件一定成立——可能是信号导致的伪唤醒，也可能是多个等待者竞争时别的进程抢先消费了资源。`while`循环确保每次唤醒后重新检查条件，这是防御性编程的硬性规则。
...
> 💡 **提示**：waitqueue是可复用的——完成一次等待后，同一个`wait_queue_head_t`可以继续用于下一次等待事件。队列的生命周期独立于单次等待，你可以在一个驱动模块的整个生命周期内反复使用同一个等待队列头，只需确保每次等待和唤醒的逻辑配对正确。
...
学会了这些睡眠等待机制，你也许会想：用了这么多锁和等待，万一不小心搞出死锁怎么办？别担心，内核自带一个神器——**lockdep**死锁检测器。它能自动分析锁的依赖关系，在开发阶段就帮你发现潜在的死锁风险。<br>
下一节 **13.6.1 lockdep死锁检测**，带你认识这个内核调试的"保命神器"。
...
| 📊 表格 | completion vs waitqueue vs semaphore 三大机制对比表 |
| 📊 表格 | `wake_up`函数族全览（唤醒数量/可被中断/适用场景） |
| 📁 参考文档 | `Documentation/scheduler/completion.rst` |
| 📁 参考源码 | `kernel/sched/completion.c`、`include/linux/wait.h` |

---

> 💡 **思考题**：你在一个中断处理函数里调用了`complete()`，在中断返回后，`wait_for_completion()`所在的进程才会真正开始执行。这段时间里发生了什么事情？hint：涉及到软中断和进程调度。