...
completion 和 waitqueue 都是"等一件事发生"，区别在表达力：completion 是一次性事件通知，`wait_for_completion()` 睡眠等待，`complete()`/`complete_all()` 唤醒，没有条件判断；waitqueue 是条件等待机制，支持条件判断循环，`wake_up` 系列唤醒，可反复使用。

选型跟着语义走：DMA 完成、probe 完成、异步任务完成这类"做完喊一声"的场景用 completion；等缓冲区非空、等 fd 就绪、等数据到来这类"等到某个条件成立"的场景用 waitqueue——需要条件判断的那一刻，completion 就出局了。

避坑三点：多等待者场景必须用 `complete_all()` 而不是 `complete()`；手写 waitqueue 唤醒后务必重新检查 condition（虚假唤醒是常态不是异常）；可被信号中断的等待要处理 `-ERESTARTSYS`——忘了它，Ctrl-C 行为就会变成玄学。
