...
AB-BA 死锁的原理一句话：两条路径以相反顺序加同两把锁，死锁是必然事件而非概率事件——只要压测跑得够久。解法是给锁分级编号，全局只允许从低向高获取，从机制上杜绝循环等待；动态锁场景用 trylock 回退策略，要么全拿到要么全不拿，绝不持有一把等另一把。

重入问题别再找递归锁：Linux 内核锁一律不支持递归获取，解法只有一个——重构调用路径，让同一把锁不会被同一条路径拿两次。优先级反转在实时场景必须正面应对：带 PI 的 rt_mutex（归属问题 13.2.2 已讲清）。

终极建议放在最后也最重要：锁越少越好，能用一把就不用两把。每多一把锁，死锁分析的状态空间就上一个量级——锁的减法设计，比任何死锁检测工具都更早地解决问题。
