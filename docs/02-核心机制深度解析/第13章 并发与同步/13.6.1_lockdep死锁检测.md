...
> 所属章节：第13章 并发与同步 > 13.6 调试与排障工具
>
> 难度：[E][M] | 预计阅读时间：25分钟

## <span class="blue"> 本节导读

死锁问题最烦人的是什么？不是修复，而是复现——它可能一周才出现一次，出现时又没有任何线索。Linux内核提供了一个神器 `lockdep`，它能在死锁真正发生之前就预警出来。<br>
本节带你深入 lockdep 的工作原理——它如何追踪每一次加锁顺序、构建依赖图、检测循环依赖和 IRQ 安全问题。同时还会介绍 `lock_stat`，帮你找到系统中竞争最激烈的"热点锁"。<br>
学完后，你将能够启用 lockdep 分析死锁风险，读懂 AB-BA 报告，并用 lock_stat 优化锁竞争。
...
> ⚠️ **陷阱**：Lockdep 运行时开销极大（10%以上性能损失），**绝对不能用于生产环境**。它只适用于开发和测试阶段。有些嵌入式项目为了节省资源甚至不会编译进 lockdep——你需要在 debug 版本内核中单独启用。

> 💡 **提示**：给锁起**有意义的名字**（如 `uart_tx_lock` 而不是 `lock1`），lockdep 报告中的依赖图会清晰很多，排查问题时会感谢你自己的。

### 读懂 lockdep 报告：逐行分析

当 lockdep 检测到问题时，会在 dmesg 中输出类似下面的报告：

```
[   12.345678] ========================================================
[   12.345689] WARNING: possible circular locking dependency detected
[   12.345691] 5.15.0-lockdep #1 Tainted: G        W         
[   12.345693] --------------------------------------------------------
...
看明白了吗？核心矛盾是：**历史上曾经出现过 `tx_lock → rx_lock` 的加锁顺序，但现在有代码试图在持有 `rx_lock` 时获取 `tx_lock`**。如果两个 CPU 同时走这两条路径，就会死锁。
...
> 💡 **提示**：`waittime [max]` 特别大的时候，说明某次临界区执行时间异常长。配合 `ftrace function_graph` 追踪谁持有了这么长时间，往往能发现意外的阻塞操作（比如在持有 spinlock 时调用了 `msleep()`）。
...
> ⚠️ **陷阱**：上面的优化有一个隐藏风险——如果在循环中频繁加解锁，开销可能反而更大。更好的做法是把 SPI 传输缓冲区分成 per-CPU buffer，避免共享锁。没有银弹，只有权衡。
...
掌握了锁的死锁检测和竞争分析之后，我们继续探索另一类棘手的并发问题——**内存访问的正确性**。下一节 `13.6.2 KASAN 与 KCSAN` 将介绍两个更强大的调试器：KASAN 检测内存越界和 use-after-free，KCSAN 检测数据竞争。它们和 lockdep 配合使用，几乎能覆盖所有并发相关的 bug 类型。
...
| 资源 | 内容 | 位置 |
|------|------|------|
| 代码清单 | lock_stat 启用和读取脚本 | `tools/lockstat_show.sh` |
| 图示 | lockdep 锁依赖图（mermaid） | 本文件内 |
| 示例代码 | 故意触发 lockdep 的 demo 模块 | `code/deadlock_demo.c` |
| 参考文档 | Kernel Lockdep 官方文档 | `Documentation/locking/lockdep-design.rst` |
