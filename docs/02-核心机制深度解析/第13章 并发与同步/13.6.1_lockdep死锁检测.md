...
lockdep 的机制一句话：记录每次锁获取的顺序，构建有向依赖图，检测循环和违规。它内建四个检测器——循环依赖、IRQ inversion、递归死锁、释放正确性，编译开 `CONFIG_LOCKDEP=y` + `CONFIG_PROVE_LOCKING=y`，只用于调试构建。报告解读的关键是找到"已持有锁依赖新锁"的关系链，顺藤摸出反向加锁的代码路径。

搭档 lock_stat 管另一件事：统计每把锁的争用率、等待时间、持有时间，`contentions/acquisitions` 比率超过 20% 的锁就是热点，需要关注。修复思路也按工具分家：lockdep 报的问题统一加锁顺序，lock_stat 暴露的问题缩短临界区或做锁拆分。

lockdep 和 lock_stat 是并发调试的左膀右臂：一个防死锁，一个治竞争。开发阶段全程开着它们，锁相关的问题基本活不到产品阶段——这笔性能开销，是调试期最值得付的保费。
