...
rwlock 和 rwsem 解决的是同一个问题——读者并发——分歧只在睡觉还是自旋：rwlock 是自旋锁，不可睡眠，中断上下文可用；rwsem 是睡眠锁，适合长临界区，仅限进程上下文。rwsem 的慢性病是写者饥饿：持续的读者流让写者长期拿不到锁，v6.6 有缓解但无根除；`downgrade_write()` 让写者用完就降级为读者，是减少读者等待的实用手法。

选型原则按读写比例分档：读极多写极少直接上 RCU（13.4 的全部理由）；读多写少用 rwsem；读写平衡退回 mutex——别把读写锁当默认选项。核心记住一点：rwlock 自旋、rwsem 睡眠、rwsem 有写者饥饿，而读多写少的极致场景，读写锁两兄弟都打不过 RCU。
