...
并发来源四分：多线程、中断、底半部、SMP——来源不同，防护策略各异。多线程并发最直观，调度器抢占所致；中断并发最突然，硬中断随时打断进程，上下文完全独立；底半部并发是 tasklet/softirq 的延后执行，同样不能睡眠；SMP 并发是真正的并行执行，内存可见性要靠屏障保证（13.3 展开）。判断自己站在哪种上下文里，四个宏：`in_interrupt()`、`in_irq()`、`in_softirq()`、`in_atomic()`。

两个术语的边界要划清：data race 是内存访问层面的非同步并发写，KCSAN 能检测；race condition 是逻辑层面的时序依赖，是 data race 的超集——工具能抓住前者，后者只能靠分析和评审。

最实用的一件武器是临界区识别矩阵：把"谁在什么上下文中访问什么数据"列成表，每个格子就是一个潜在竞态。写驱动前先花五分钟填这张表，可能省去五小时的调试——这不是修辞，是并发 bug 排查成本的真实量级。
