...
7.1.2 解决了"BootROM 按什么顺序探测介质、全部失败时怎么进下载模式"。

介质找到之后还剩最后一环：BootROM 怎么把 SPL 从介质里读出来。答案是一条厂商固化好的裸块协议——从哪个偏移读、先读多少字节、头部校验什么、主体读多大、放到哪段 SRAM，全部写死在 BootROM 代码里。SPL 镜像必须严格按这套协议烧写，偏移差一个扇区、头部差一个字节，板子就是一块砖。<BR>
...
> 分区与文件系统：SD 卡本身是一块按扇区读写的存储空间，没有"文件"的概念。分区是把这块空间切成若干区域，切法记录在第 0 扇区的分区表里；文件系统是分区内部组织目录和文件的规则。"把文件拖进卡里"依赖操作系统解析文件系统，BootROM 没有这层能力。
<!-- -->
> FAT 与 ext4：两者都是文件系统格式——FAT32/exFAT 是 Windows 和 SD 卡出厂常用，ext4 是 Linux 常用，"格成 FAT"就是把分区格式化为 FAT。卡的布局千差万别，BootROM 一种都不支持，这正是它只认固定偏移的原因。
...
裸块读取需要一套与文件系统无关的编址方式，这套方式就是扇区与 LBA。

> 扇区（sector）：存储介质被切成的固定大小的块。SD 卡和 eMMC 每扇区 512 字节——这是硬件层面的切分，与卡上有没有分区、格成什么文件系统完全无关。
<!-- -->
> LBA（Logical Block Address，逻辑块地址）：每个扇区从 0 开始的编号。不管这张卡在 Windows 里显示为什么盘符、装的是什么文件系统，"第 64 扇区"永远指向同一个物理位置。

其他介质的编址粒度不同，思想完全相同：<BR>
SPI NOR 按字节偏移直接编址；NAND 按页/块编址。BootROM 的加载协议就建立在这套固定编号上。

同一套裸块思想落到每种介质上，读取粒度、SPL 摆放位置、控制器初始化步骤各不相同。下表对照六种介质的协议参数，烧写时查自己平台对应的那一行：

| 介质类型 | 读取粒度 | SPL固定位置 | 控制器初始化要点 | 典型失败场景 |
|---------|---------|-----------|---------------|-----------|
| SD/MMC | 512B Block | 第64扇区（偏移0x8000） | 时钟 0→400kHz→25MHz<br>CMD0/8/55/41/2/3/7 | 卡检测引脚极性反；CMD线无上拉 |
| eMMC | 512B Block | Boot Partition或User Area第64扇区 | 切换 BOOT_PARTITION_ENABLE<br>读 BOOT_ACK 确认 | BOOT_ACK超时；PARTITION_CONFIG未烧写 |
| SPI-NOR | 按地址直接读取 | 偏移0x0或0x10000（厂商相关） | 设置 SPI 时钟模式（Mode 0/3）<br>读取 JEDEC ID 验证芯片 | 时钟过快（>芯片fR）；CS极性错误 |
| NAND | Page (2K/4K) | 第0/1/2 Block（跳过坏块） | 读取 ONFI 参数页<br>设置 ECC 模式 | 坏块表损坏；ECC模式不匹配 |
...
逐字段看 i.MX 的 IVT 和 Rockchip 的 IDBlock 这两家代表结构，会发现字段总是落在"认出镜像、说清读多少、放到哪里"这三件事上：IVT 的关键字段是 `entry`（入口地址）、`self`（IVT 自身地址）、`dcd`（指向 DRAM 初始化表，SPL 代码运行前先按它配好 DRAM 控制器）；IDBlock 经 RC4 混淆存储，内含 SPL 加载地址与长度（具体布局可查 U-Boot `tools/rkcommon.c` 的生成逻辑）。
...
BootROM 用裸块读取加载 SPL，原因是纯粹的尺寸账：片内 ROM 只有 32~128KB，放不下任何文件系统驱动，所以协议只能建立在扇区/LBA 这种与文件系统无关的统一编址上。

镜像头的设计各家不同（TI GP Header / NXP IVT / Rockchip IDBlock / 全志 eGON.BT0）但结构同构——magic 认镜像、长度与目的地决定再读多少放到哪、校验和做自检；校验体系则是三层递进：结构校验（magic+checksum）→ 完整性（CRC/SHA）→ 真实性（RSA/ECDSA 签名），安全启动逐级加码。

工程上三件事必须记牢：烧写偏移是换平台时第一件要抄下来的参数（Rockchip 扇区 64，`dd bs=512 seek=64 conv=notrunc`，烧错位置的症状是静默跳过，没有任何报错）；SPL 的大小上限由片内 SRAM 容量决定，超限在编译期就该被拦截，装不下就拆两级加载；排错分界一句话——**串口有输出说明 BootROM 已经成功了**，完全静默才去查烧录偏移和头部格式。

BootROM 协议是启动链上唯一不可协商的环节：SPL 往后每级都能改代码换配置，唯有"哪个偏移、什么头部、放哪段 SRAM"由芯片厂写死在 ROM 里。
