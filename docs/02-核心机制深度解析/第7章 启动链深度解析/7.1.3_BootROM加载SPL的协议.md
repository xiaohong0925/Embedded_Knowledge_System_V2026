...
BootROM 用裸块读取加载 SPL，原因是纯粹的尺寸账：片内 ROM 只有 32~128KB，放不下任何文件系统驱动，所以协议只能建立在扇区/LBA 这种与文件系统无关的统一编址上。镜像头的设计各家不同（TI GP Header / NXP IVT / Rockchip IDBlock / 全志 eGON.BT0）但结构同构——magic 认镜像、长度与目的地决定再读多少放到哪、校验和做自检；校验体系则是三层递进：结构校验（magic+checksum）→ 完整性（CRC/SHA）→ 真实性（RSA/ECDSA 签名），安全启动逐级加码。

工程上三件事必须记牢：烧写偏移是换平台时第一件要抄下来的参数（Rockchip 扇区 64，`dd bs=512 seek=64 conv=notrunc`，烧错位置的症状是静默跳过，没有任何报错）；SPL 的大小上限由片内 SRAM 容量决定，超限在编译期就该被拦截，装不下就拆两级加载；排错分界一句话——**串口有输出说明 BootROM 已经成功了**，完全静默才去查烧录偏移和头部格式。BootROM 协议是启动链上唯一不可协商的环节：SPL 往后每级都能改代码换配置，唯有"哪个偏移、什么头部、放哪段 SRAM"由芯片厂写死在 ROM 里。
