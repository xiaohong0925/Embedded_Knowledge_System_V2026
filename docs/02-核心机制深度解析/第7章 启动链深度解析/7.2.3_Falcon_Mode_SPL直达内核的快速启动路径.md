...
Falcon Mode 的本质是从架构上消掉一个阶段：SPL 直接加载 kernel + dtb 并跳转，U-Boot proper 整个不运行。它的加速来源值得体会——不是把代码写快，而是**不做不需要的事**（网口、USB、CLI、ENV 初始化全部跳过）。代价与收益同样清晰：失去 CLI、网络启动、环境变量、bootdelay 和动态 dtb 修改，所以量产铁律是必须留回退通道（GPIO 检测、Boot Counter 超限回退、组合键 Recovery、看门狗兜底），没有后门的 Falcon 是给产线挖坑。使能上注意 `spl_start_uboot()` 的返回值语义是反直觉的——返回非 0 才回退 U-Boot。
