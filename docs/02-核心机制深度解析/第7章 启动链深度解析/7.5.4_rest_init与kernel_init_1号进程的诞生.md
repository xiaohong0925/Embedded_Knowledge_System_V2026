...
这里需要先建立一个概念区分：**内核线程与用户进程**。两者在调度器眼里都是 `task_struct`，走同一套调度逻辑，区别在于地址空间——用户进程拥有独立的 `mm_struct` 和用户态页表，内核线程没有，它只在内核地址空间里运行，永远不需要切到用户态。这个区分马上就会用到：`rest_init()` 要创建的两个新进程都是内核线程，而其中一个最终还要"蜕变"成用户进程。
...
noinline void __ref rest_init(void)
{
    ...
    /*
     * ① 先创建 PID=1：kernel_init 内核线程。
     * 必须先拿到 1 号，但 kernel_init 很快就要创建 kthread，
     * 所以它会被 kthreadd_done 阻塞，直到 2 号就位。
     */
...
flowchart TB
    subgraph rest_init["rest_init() 执行流"]
        ...
        B --> C["kernel_thread(kernel_init)<br>PID = 1"]
...
| **一分为三** | 复用当前上下文为 PID 0（idle），`kernel_thread()` 依次创建 PID 1（kernel_init）与 PID 2（kthreadd） |
