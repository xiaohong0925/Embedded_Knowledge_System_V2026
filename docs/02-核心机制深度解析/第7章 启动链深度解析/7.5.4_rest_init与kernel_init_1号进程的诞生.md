...
rest_init 是整条启动链上"代码"与"系统"的分水岭：此前是一段顺序执行的初始化程序，此后是一个按调度器规则运转的多进程操作系统。start_kernel 无调用者可返回，只能以不返回的 rest_init 收尾；它把世界一分为三——当前上下文退化为 PID 0（idle，每 CPU 一个的调度保底，WFI 省电、cpuidle 的挂载点），`user_mode_thread()` 创建 PID 1（kernel_init，自带 exec 规格），`kernel_thread()` 创建 PID 2（kthreadd）。先 1 后 2 的顺序有讲究：kernel_init 必须先占住 PID 1 的名分，但阻塞在 `kthreadd_done` 上等 kthreadd 就位——因为 `kthread_create()` 只能在进程上下文执行，此后所有运行时内核线程的 PPID 都是 2。

`Freeing unused kernel memory` 是这条路径上的分界线日志：`kernel_init` 在 `async_synchronize_full()` 之后调 `free_initmem()` 回收 `.init` 段——闸门顺序不能反，异步任务可能还在执行 `.init` 段代码。理解这三个 PID 的分工，就拿到了阅读此后每一行启动日志的坐标系；kernel_init 如何蜕变成用户态的 1 号进程，是 7.6.1 的下一棒。
---
