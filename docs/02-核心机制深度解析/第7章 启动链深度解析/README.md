...
独特设计：逐行分析启动日志，对应到每个子系统的初始化。读者可以拿着自己板子的启动日志，跟着本章逐行理解。有经验工程师能移植U-Boot、能编译内核、能让系统跑起来——但面对启动日志中某一行的失败，常常束手无策。为什么？因为他们对启动链的每个阶段只有模糊的”大概在这时候初始化”的认知，缺乏精确的源码级对应关系。本章的价值在于建立“启动日志 ↔ 源码路径 ↔ 子系统初始化”的三重映射。
