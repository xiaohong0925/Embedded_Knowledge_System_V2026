...
`Waiting for root device` 是启动日志里最容易被误读的一行：它不是"挂载失败"，而是 `wait_for_root()` 还在等块设备的异步 probe 跑完。排错时先分清两件事——等待发生在挂载之前，挂载失败发生在等待之后。等待类问题看设备列表：列表里有设备但分区名不符，是 `root=` 写错了；列表里根本没有设备，是驱动没就绪或设备树没配对；信息对不上或时好时坏，去查设备树。`rootdelay` 和 `rootwait` 的区别也就在这条等待路径上：前者无条件睡死固定秒数，后者失败重试、就绪即走，eMMC/SD 场景没有选前者的理由。

initramfs 的价值用一句话说透：它编进内核 `.init.ramfs` 段，解压进 tmpfs 就能跑，零驱动依赖，正好破解"存储驱动在根文件系统里、根文件系统靠存储驱动读"的死锁。别把它和 initrd 混为一谈——initrd 是内存块设备镜像，本身还得要文件系统驱动，是纯粹的遗产。从 initramfs 切到真实根用 `pivot_root` 操作挂载树本身，`chroot` 只改进程视角、旧根释放不掉；用 `switch_root` 时三个陷阱都指向同一症状：旧根 `EBUSY` 或内存不释放——忘卸旧挂载点、fd 泄漏、残留进程的工作目录还挂在旧根上。

量产纪律只记一条：U-Boot 里的设备编号和内核枚举出来的编号没有任何对应关系，`root=/dev/mmcblk0p2` 这类写法是把启动正确性赌在枚举顺序上。用 `root=PARTUUID=` 把依赖从顺序换成分区自身的 UUID，这一步做完，换卡、换批次、加设备都不会再咬人。`VFS: Mounted root` 这行日志是整个启动排错里价值最高的坐标：它之前的问题在内核与设备树，之后的问题在 rootfs 内容，分界线一目了然。
