...
当你看到RK3568开发板上电后串口打印的第一行 `U-Boot SPL 2021.10` 时，是否想过：CPU刚刚从RESET引脚释放到执行这行代码之间，究竟发生了什么？本节从硬件复位信号释放那一刻开始，追踪PC寄存器的精确轨迹，解剖ARM64复位向量表的偏移布局，并深入解释为什么每颗SoC都必须在片内固化一段无法修改的BootROM代码——以及为什么它偏偏要占据地址空间最高端那64KB。

---

## 知识点1：复位向量表 — 上电后第一条指令在哪里？ [I] ~900字
...
## 知识点2：BootROM的存储位置 — 它到底存在哪里？ [I] ~700字

### 问题场景

你在设计一块新板子的启动策略时，FAE告诉你"芯片上电直接从SPI NOR启动"。但你查阅原理图发现，SPI NOR Flash是外部器件，出厂时还是空的——空的Flash怎么可能"直接启动"？真相是：**永远先执行片内BootROM**，BootROM再去初始化外部Flash并加载代码。
...
## 知识点3：BootROM代码特征 — 极简主义的极致 [I] ~800字

### 问题场景

你在阅读Rockchip TRM时发现BootROM只有32KB，却要支持从SPI NOR、eMMC、SD、NAND、USB等多种介质启动。32KB连一个完整的SPI驱动都放不下，这是怎么做到的？更进一步：如果BootROM有bug（比如某批次eMMC初始化失败），芯片就报废了吗？
...
### 表格清单
1. **表1**：ARM64异常向量标准偏移表（见知识点1）
2. **表2**：BootROM存储介质对比表（Mask ROM / OTP / eFuse）（见知识点2）
3. **BootROM精简技巧速查表**（见知识点3）
4. **复位后寄存器初始值表**（见知识点1）

### 图示清单
- **图1**：ARM64异常向量偏移布局mermaid图（知识点1）
- **图2**：芯片上电到BootROM到SPL的完整执行流程mermaid图（知识点2）

### 代码清单
- **代码1**：ARM64复位入口点 + 异常向量表汇编代码（ATF风格，知识点1）
- **代码2**：BootROM三阶段逻辑伪代码（C语言，知识点3）
- **代码3**：RK3568调试分析命令序列（GDB，实践案例）
