...
选对文件系统比优化代码更重要——选错了，再牛的程序员也救不了。怎么选？两个维度：你的存储介质是什么（eMMC/UFS/裸 NAND）？你的读写模式是什么（读多/写多/小文件）？<br>
本节覆盖：

- 介质 × 读写模式的两维度选型框架
- fio 基准测试方法与 iostat/blktrace 的 I/O 模式分析
- 可操作的选型决策树与场景速查表
- 三个真实产品的选型案例验证

---

## <span class="blue"> 选型决策框架 [E→M]
...
**任意介质（只读场景）** 若系统镜像烧录后不再修改，**SquashFS** 提供只读压缩，镜像体积小、启动快、无运行时写放大。**tmpfs** 用于纯内存场景，掉电即失，但速度极快。只读系统盘若需要运行时写能力，用 **overlayfs** 在 SquashFS 上叠一层可写文件系统（见 12.3.4）。
...
### I/O 模式分析：iostat 与 blktrace

fio 回答"这个文件系统能跑多快"，但选型前还有一个更根本的问题：**你的产品实际的 I/O 模式是什么**？两个工具负责回答它。

**iostat：负载画像**。`iostat -x 1` 周期输出块设备级指标，重点看四列：

| 列 | 含义 | 选型启示 |
|:---|:---|:---|
| `r/s`、`wkB/s` | 读 IOPS 与写带宽 | 读写占比决定"读多"还是"写多"分支 |
| `avgrq-sz` | 平均请求大小（扇区） | 接近 8（4KB）说明小随机写为主，f2fs 收益最大；远大于 8 说明顺序化良好 |
| `aqu-sz` | 平均队列深度 | 持续大于 1 说明存储是瓶颈；配合 %util 接近 100% 确认 |
| `await` | 平均 I/O 延迟（ms） | 随机写下延迟尖刺常与 GC/journal 提交相关 |

**写放大的量化**：对比应用视角的写入量与介质视角的写入量。应用侧取 `iostat` 的 `wkB/s` 累加，介质侧读 `/sys/class/block/<dev>/stat` 的第 7 个字段（已写扇区数）的增量，两者之比即为块层可见的放大系数。eMMC 内部 FTL 的进一步放大需用厂商工具（如 mmc-utils 的 `mmc extcsd read` 寿命寄存器）估算。

**blktrace：逐请求跟踪**。当 iostat 只给出统计均值、需要看请求级行为时用 blktrace：

```bash
# 采集 30 秒块层 I/O 事件（Q 下发 / D 驱动执行 / C 完成）
blktrace -d /dev/mmcblk0 -w 30 -o trace

# 解析输出：时间戳、进程、读写、起始扇区、长度
blkparse -i trace -f "%T %p %a %S %N\n"
```

从 blkparse 输出可直接判断：相邻请求的扇区是否连续（合并率）、写入是否集中在少数区域（热点）、fsync 触发的 flush 请求频率。开发板资源紧张装不了 blktrace 时，可用内核 tracepoint 替代：`echo 1 > /sys/kernel/debug/tracing/events/block/block_rq_issue/enable` 后读 `trace` 文件。

> 💡 典型用法：先 iostat 确认"小随机写 + 高 %util"，再 blktrace 确认写请求不连续，此时从 ext4 迁 f2fs 几乎必有收益——这正是 12.3.2 案例的决策路径。
...
| 维度 | 要点 |
|------|------|
| **两维框架** | 介质定候选集（eMMC→ext4/f2fs，裸 NAND→UBIFS，只读→SquashFS），负载定最终选型 |
| **量化方法** | fio 测性能上限；iostat 画负载画像（avgrq-sz 判随机/顺序）；blktrace 看请求级连续性 |
| **写放大测量** | 应用写入量（iostat wkB/s 累加）÷ 介质写入量（/sys/class/block/*/stat 扇区增量） |
| **三案例共性** | 介质与负载必须同时匹配；分区隔离不同负载是混合系统的通用解法 |
| **核心口诀** | eMMC 写多用 f2fs，裸 NAND 只用 UBIFS，只读必上 SquashFS，/tmp 丢给 tmpfs |

---

## <span class="blue"> 下一步

12.3 一直在"块设备之上的文件系统"世界里打转，UBIFS 提到了它底下的 UBI。下一节深入裸 Flash 的世界：MTD 子系统与块设备到底有什么不同、UBI 的磨损均衡和坏块管理怎么实现、量产烧录工具链怎么用。
...
| 资源类型 | 内容 |
|----------|------|
| **基准工具** | fio 三类负载命令（见正文）；`iostat -x 1` 负载画像；`blktrace`/`blkparse` 请求级跟踪；`mmc extcsd read` 读 eMMC 寿命寄存器 |
| **排错索引** | 写小文件慢/eMMC 磨损快 → iostat 看 avgrq-sz 与 %util → blktrace 确认写请求连续性 → 评估迁移 f2fs |
| **关联小节** | 12.3.1 ext4 三种日志模式的性能差异；12.3.2 f2fs GC 调优参数；12.3.4 overlayfs 与只读场景组合 |
