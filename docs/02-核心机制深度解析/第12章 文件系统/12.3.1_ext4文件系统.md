...
12.2 讲完的数据最后落到了具体文件系统手里。ext4 是 Linux 通用场景的事实标准，它的三个设计决定了数据在磁盘上的组织方式：inode 存元数据、extent 管块映射、journal 保崩溃一致性。<br>
本节覆盖：

- ext4_inode 的字段布局与 i_block 的两种解释方式
- extent 结构为什么能把大文件的元数据开销压缩到极限
- 三种 journal 模式各自保护什么、不保护什么
- 日志事务从 running 到 checkpoint 的完整生命周期

---

## <span class="blue"> ext4 核心数据结构：inode 与 extent [E→M]

### inode：文件的元数据中心

在 ext4 中，inode（index node）是每个文件或目录的核心元数据结构。每个 inode 默认占用 256 字节（早期兼容格式为 128 字节），存储文件的权限、大小、时间戳以及最关键的数据块位置信息。`struct ext4_inode` 定义了完整的元数据布局，其关键字段如下：
...
`i_block[EXT4_N_BLOCKS]` 数组是 inode 与数据块之间的桥梁。`EXT4_N_BLOCKS` 定义为 15，在传统块映射模式下，前 12 个为直接块指针（direct block），第 13 个为一级间接块指针，第 14 个为二级间接块指针，第 15 个为三级间接块指针。而在启用 extent 功能的 ext4 中，这 60 字节的 `i_block` 空间被重新解释为 extent 树的头节点，彻底改变了块寻址方式。

### extent：连续的块映射

传统 Unix 文件系统（如 ext2/ext3）采用直接/间接块指针机制管理数据块。当文件需要第 N 个数据块时，系统需遍历多级间接指针链表，时间复杂度为 O(N)，且每个块指针仅指向一个 4KB 块。对于 1GB 的文件，传统方案需要 262,144 个独立的块指针，元数据膨胀至数 MB。
...
extent 的引入不仅降低了元数据的空间占用，更使得文件系统在进行大块 IO 时能够一次性读取连续的物理块。对 SSD 而言，减少元数据量同样意味着更低的写入放大和映射表压力。

---

## <span class="blue"> ext4 的日志（journal）机制 [E]
...
| 写入放大（额外 IO） | 最低 | 中等 | 最高（数据写两遍） |
| 顺序写性能 | 最优 | 良好 | 较差（日志区带宽瓶颈） |
| 适用场景 | 临时数据、缓存、性能敏感 | 通用场景（默认推荐） | 关键数据、数据库类负载 |
| 恢复速度 | 快 | 快 | 最快（日志重放即可） |

默认的 `data=ordered` 模式是 ext4 为大多数工作负载选定的平衡点：它避免了 journal 模式的双写惩罚，又比 writeback 模式提供了更合理的顺序性保证。需要原子性数据更新的数据库类应用，通常在应用层实现 WAL（Write-Ahead Logging），而非依赖文件系统的 `data=journal` 模式。
...
当事务完成检查点后，其占用的日志区域可被后续事务循环复用。这种环形缓冲区（circular buffer）设计使日志区域的空间需求与文件系统总容量无关，通常 128MB 的日志即可满足绝大多数工作负载。

---

## <span class="blue"> 实践案例：2TB SSD 上的海量小文件

某工业嵌入式设备采用 2TB SSD 存储海量传感器日志，需频繁创建约 10 万个 4KB ~ 128KB 的小文件。在对比测试中发现，采用间接块映射的 ext2 文件系统随着文件数量增加，inode 表与间接块消耗大量 SSD 空间，元数据总量超过 3GB，且频繁的间接块分配导致 SSD 写入放大系数升至 2.8，文件创建速率从初始的 12,000 文件/秒骤降至 6,000 文件/秒。迁移至 ext4 后，extent 结构使小文件的元数据平均占用降至每个 inode 仅需 56 字节（extent header + 1 个 extent 条目），总元数据量压缩至约 5.5MB；SSD 写入放大系数降至 1.3，文件创建速率稳定维持在 11,500 文件/秒。同时启用 `data=ordered` 日志模式后，系统在 200 次模拟断电测试中均实现 100% 文件系统一致性恢复，验证了 extent 与 journal 协同设计在大规模存储场景下的可靠性优势。
...
| 维度 | 要点 |
|------|------|
| **inode** | 256 字节元数据中心；`i_block[15]` 在传统模式下是块指针数组，在 ext4 中被重新解释为 extent 树头节点 |
| **extent** | 三字段（逻辑块号、物理起始、长度）描述一段连续块；1 个 extent 最多覆盖 128MB；超出 4 个时构建 extent 树 |
| **journal 三模式** | writeback 只保元数据；ordered（默认）额外保证"数据先于元数据落盘"；journal 连数据一起进日志、写两遍 |
| **事务生命周期** | running → locked → flush → commit → checkpoint；日志区环形复用，大小与文件系统容量无关 |
| **核心口诀** | 元数据找 inode，块映射看 extent，崩溃恢复靠 journal，默认 ordered 别乱改 |

---

## <span class="blue"> 下一步

ext4 的就地更新策略对闪存并不友好。下一节看三星为 eMMC/UFS 量身设计的 f2fs：怎么把所有写操作变成日志尾部追加，把随机写放大压下来。
...
| 资源类型 | 内容 |
|----------|------|
| **内核源码** | `fs/ext4/extents.c`（extent 树操作）、`fs/ext4/inode.c`（inode 读写）、`fs/jbd2/`（journal 事务实现） |
| **排错工具** | `dumpe2fs -h` 查看 inode 大小与日志配置；`debugfs -R "stat <inode号>"` 查看文件的 extent 分布；`filefrag -v` 检查文件碎片 |
| **关联小节** | 12.2.3 fsync 与日志事务的关系；12.5.2 journal 模式与掉电保护的组合选型 |
