...
两点行业现状顺带建立坐标。一是**国产 IP 已经进场**：国内厂商已有 16/28 Gbps 档的 JESD204B/C PHY + Controller 完整方案出货，这条链路不再是 ADI/TI + 国外 FPGA 三家的专属组合——做国产化选型时它已是可以评估的选项。二是**版本还在往前走**：JESD204D 已被提上日程，方向延续"更高速率、更高编码效率"的老路。读本篇时不必背每个版本细节，抓住"物理层复用 SerDes 通用技术、协议层只解决成帧与对齐"这个结构，新版本出来时你只需重学参数表和对齐机制的增量。
...
<!-- 【待补图】images/b-f-16-4-framing-layers.png（优先级：★必要）
图名：JESD204 成帧分层示意——采样数据 → 帧 → 多帧/扩展多块 → lane
生图提示词：工程线框图，白底深色线条，配色克制，16:9，中文标注。画面从左到右四级漏斗：最左是"M 个转换器内核"各自吐出采样点流（标注 N/N' 位宽）；第二级"帧"把采样点按 S 打包（标注 S、F 作用于此层）；第三级 K 个帧捆成"多帧"（64b 模式对应"多块/扩展多块"，标注 K、E）；最右摊到 L 条 lane 平铺发出。每个参数字母（M/L/S/F/K/E）用引线标注在它作用的那一层上。底部一行小字：LMFS=2861 实例——8 内核、2 lane、每帧每 lane 6 字节。 -->
...
<!-- 【待补图】images/b-f-16-4-link-bringup-timing.png（优先级：★必要）
图名：JESD204B 建链时序图——SYNC~ 握手三阶段
生图提示词：横向时间轴时序图，白底深色线条，各阶段用浅色块区分，16:9，中文标注。两条波形线：上线是 SYNC~ 电平（开始为低，CGS 完成后跳高），下线是 lane 上的数据流。从左到右三个阶段：① CGS 阶段（SYNC~ 低，lane 上是密集的 /K28.5/ 逗号码流，标注"接收端 CDR 锁定 + 字符对齐"）；② ILAS 阶段（SYNC~ 刚释放，lane 上 4 个多帧，第 2 个多帧高亮标注"内含发送端完整参数表 LMFSK"）；③ 用户数据阶段（lane 上均匀数据流，标注"链路进入工作态"）。阶段交界处画竖直虚线。 -->
...
六步表是单点排查，一线调试还有三条更上层的节奏原则，能把"链路起不来"这类大问题的收敛时间缩短一个量级：

**先时钟、后数据。** 时钟芯片没 lock（lock detect 引脚或寄存器标志未置位）之前，不要碰链路的任何东西——Device Clock 不稳时建链失败是随机的，越调越乱。先把两颗芯片的时钟输入用示波器或频率计确认到 ppm 级，再谈协议。

**先低速、后高速。** lane 速率砍半（降采样率或降线速率档位）跑通了，说明参数表、lane 映射、极性全部正确，剩下的纯粹是信号完整性问题——直接带着这个结论回 16.1 的眼图与均衡方法论。低速也不通，则问题在配置与连接，与 SI 无关。一刀切开两个故障域。

**先单通道、后多通道。** 多片 ADC 同步系统先只接一片、只开一条 lane 跑通，再逐片逐 lane 加。多通道一次性上电，lane 映射错误、SYSREF 相位错误、参数不一致会混在一起报错，无法归因。

链路全绿但**确定性延迟不对**，是另一类问题，有它自己的排查顺序，按代价从小到大：

1. **SYSREF 抖动**：示波器同时打 SYSREF 与 Device Clock，看 SYSREF 边沿是否落在 Device Clock 的建立/保持窗口内——不满足就调时钟芯片的输出相位延迟寄存器（这类芯片每个输出都有可编程延迟步进）。这是最常见的原因。
2. **LMFC/LEMC 复位**：确认 SYSREF 真的复位了链路两端的本地对齐计数器——查两端 subclass 配置是否都是 1、SYSREF 模式是单次还是周期（周期 SYSREF 会持续重对齐，本身可能就是抖动源）。
3. **弹性缓冲区 RBD**：接收端的弹性缓冲用多帧边界做释放对齐，RBD（Rx Buffer Delay）参数决定释放点。SYSREF 和计数器都正确但延迟仍漂移一个采样点量级时，调 RBD 让释放点避开边界竞争区。

> 💡 这条顺序的逻辑是"时钟域问题先于数据域问题"：SYSREF 相位是物理层，计数器复位是协议层，RBD 是缓冲配置层——延迟问题从底层往上查，和建链问题"先时钟后数据"是同一个哲学。
...
JESD204 的全部复杂度来自一个朴素的矛盾：GSPS 级采样率乘上分辨率就是几十 Gbps 的数据洪流，而并行导出必然撞死。于是它把采样数据 SerDes 化，物理层完全复用 16.1 的通用技术，协议层只新增两件事——用 LMFS 参数表规定采样数据怎么切片摆进 lane，用 subclass 机制回答"链路两端的时间基准怎么对齐"。204B 用一根 SYNC~ 握手线走 CGS→ILAS→数据三阶段，204C 的 64b 模式把同步编进数据流本身、换来 3.125% 的编码开销和 32 Gbps 级速率，代价是只剩 subclass 1 一条路、SYSREF 时钟树再也省不掉。对软件工程师，这条链路的日常是三件事：按"时钟芯片→转换器→IP 核"的顺序用 SPI 把三套参数配成逐字母一致，通过内核 JESD204 框架与 IIO 读状态取数据，链路起不来时按"先时钟、后数据；先低速、后高速；先单通道、后多通道"的节奏收敛问题。

关键结论速查：

| 问题 | 一句话答案 |
|------|-----------|
| 为什么需要专用接口 | GSPS × 分辨率 = 几十 Gbps，并行 LVDS 撞"并行之死" |
| 204B 与 204C 的本质差异 | 204B 用 SYNC~ 引脚喊口号对齐；204C 把同步编进数据流，换 3.125% 开销 |
| 三套参数指什么 | ADC 寄存器、FPGA IP 核、时钟芯片各一套 LMFS，必须逐字母一致 |
| lane 速率怎么估算 | M × S × N' × (10/8) × 采样率 / L（8b/10b 模式） |
| 确定性延迟靠什么 | subclass 1 的 SYSREF 同时复位两端 LMFC/LEMC 计数器 |
| 链路起不来的排查节奏 | 先时钟后数据、先低速后高速、先单通道后多通道 |
| 延迟不确定的排查顺序 | SYSREF 相位 → LMFC/LEMC 复位 → 弹性缓冲 RBD |
| 数据全乱先查什么 | lane 极性翻转位，不是协议参数 |

### 本节自查

- 给出 ADC 采样率与 LMFS 参数，能用心算公式算出 lane 速率并判断落在哪个版本的能力范围内
- 能向同事讲清 SYNC~ 在 204B 建链三阶段中各自的电平与数据流状态
- 能解释为什么 64b 模式只支持 subclass 1，以及这对时钟树设计意味着什么
- 链路起不来时，能按三条节奏原则设计排查顺序，而不是随机试参数
- 多片采集相位错位时，能按 SYSREF→计数器→RBD 的顺序排查并说清每步在排除什么
- 方案评审时，能说清这条链路上 ADC、时钟芯片、FPGA 三个角色各自的成本与职责

---

## <span class="blue"> 下一步

JESD204 是把"模拟世界的数据"搬进数字系统的专用管道。下一篇 [B-F.16.5 USB4 与 Thunderbolt](B-F.16.5_USB4与Thunderbolt.md) 换到另一个极端：不追求采样点的确定性，而追求把 PCIe、DisplayPort、USB3 三种完全不同的流量隧道化进同一根线缆——SerDes 通用技术在消费级高速接口上的最激进形态。

> 💡 想继续深挖本篇：规范原文是 JEDEC JESD204C.01；版本差异可读 TI SBAA517《What to Know About the Differences Between JESD204B and JESD204C》，国产落地案例见 TI ZHCAA75；内核侧读 `drivers/iio/jesd204/` 框架源码与设备树绑定文档。JESD204 链路在整机里如何与 PCIe、CMIS 管理面组网，见 B-E.15.5 数通仪器仪表整机总线架构；高速 lane 上的眼图、均衡、信道预算方法论，回 B-F.16.1。
