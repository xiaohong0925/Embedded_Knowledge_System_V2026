...
下一节**B-C.7.3 USB Linux Host驱动**，我们将从协议层跃升到内核层：了解Linux USB子系统的核心数据结构（`usb_device`、`usb_driver`、`urb`）、USB Gadget Framework的内核实现，以及如何编写一个从设备树到用户空间完整打通的USB设备驱动。你还将亲手配置一个树莓派作为USB Gadget（模拟U盘或虚拟串口），体验从设备端的视角。
