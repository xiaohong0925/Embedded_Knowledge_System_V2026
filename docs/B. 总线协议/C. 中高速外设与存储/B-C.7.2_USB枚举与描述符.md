...
<!-- 【待补图】images/b-c-7-2-enumeration-sequence.png（优先级：★必要）
图名：USB 枚举八步完整时序图
生图提示词：协议交互时序图风格，白底，16:9 横版。两条竖直生命线：左侧"主机（Hub）"、右侧"设备"。从上到下按时间顺序画出八个交互阶段，每个阶段用箭头表示方向并配中文标注：① 连接检测（设备侧标注"D+ 上拉 1.5kΩ"，箭头从设备指向主机）；② 总线复位（主机→设备，标注"SE0 ≥10ms"）；③ SET_ADDRESS（主机→设备，标注"分配地址 1~127"）；④ GET_DESCRIPTOR Device（双向，标注"先取 8 字节拿 bMaxPacketSize0，再取全 18 字节"）；⑤ GET_DESCRIPTOR Configuration（双向，标注"先取 9 字节头拿 wTotalLength，再取整棵树"）；⑦ GET_DESCRIPTOR String（双向，可选，虚线箭头）；⑧ SET_CONFIGURATION（主机→设备，标注"点火：驱动匹配"）。每个阶段右侧用编号圆圈对应。配色：主机箭头深蓝、设备箭头棕色、可选步骤虚线灰色，关键字段名用红色小字，风格参考 USB 协议教科书时序图，中文标注工整。 -->
...
## <span class="blue"> 实战：解读一个真实 U 盘的描述符
...
枚举的本质是主机与新设备之间一套严格的身份核查：EP0 上跑控制传输，八步顺序强制，任何一步断掉设备就等于不存在。记住三个支点就能撑起整个流程——三阶段握手（Setup/Data/Status，每段都有 ACK）保证每次问答可靠；描述符四层树（设备→配置→接口→端点）是设备的自报家底，驱动匹配绑在接口级而不是设备级；Class/SubClass/Protocol 三字段是标准类驱动接管的钥匙，绝大多数日常设备根本不需要厂商驱动。排障时 dmesg 的错误码直接对应断点步骤：-71 查描述符响应、-84 查信号完整性、只有连接日志没有后续查设备固件——枚举这张地图读熟了，"USB 插上没反应"就从玄学变成收敛的排查。

速查表：

| 主题 | 要点 |
|------|------|
| 枚举八步 | 连接检测→复位→SET_ADDRESS→设备描述符→配置描述符→接口/端点→字符串→SET_CONFIGURATION |
| 地址 0 | 未分配保留地址，总线上同时只能有一个设备用它——Hub 逐口枚举的原因 |
| EP0 | 必备双向端点，承载全部枚举对话；bMaxPacketSize0 决定包大小 |
| 三阶段握手 | Setup→Data→Status，每阶段 ACK；任何一环无应答即重试 |
| 描述符四层 | 设备 18B → 配置 9B → 接口 9B → 端点 7B；驱动绑在接口上 |
| 驱动匹配 | 接口级 Class/SubClass/Protocol 三字段 → 标准类驱动（usb-storage/cdc_acm/uvcvideo…） |
| 错误码 | -71 协议错（包大小/固件）、-110 超时（无响应）、-84 CRC（信号完整性） |
| 工具链 | dmesg 看断点步骤 → usbmon 抓包看应答 → lsusb -v 读描述符 |

本节自查：

1. 按顺序说出枚举八步，并指出每步失败时枚举停在哪里、dmesg 会呈现什么。
2. 为什么主机取设备描述符时先只请求前 8 字节？这个设计在防什么？
3. 画出一次控制传输的 Setup/Data/Status 三阶段交互，说明每阶段的 ACK 由谁发出。
4. 描述符四层（设备→配置→接口→端点）各自回答什么问题？驱动为什么绑在接口上？
5. `bMaxPacketSize0`、VID/PID、`bInterfaceClass` 三个字段分别影响什么环节？
6. 拿到一段 `lsusb -v` 输出，怎么还原出设备的速度、供电、接口类别和端点布局？

---

## <span class="blue"> 下一步

枚举成功、驱动绑定之后，设备在内核里是什么样子：**B-C.7.3 USB Linux Host 驱动**——USB 子系统的四大核心数据结构、URB 传输请求块、xHCI/EHCI 控制器驱动、热插拔从引脚电平到设备节点的完整链条，把本篇的协议流程映射到内核对象。

> 💡 螺旋衔接："上电发现 → 读能力 → 分资源"这套枚举范式会在 D.10.2 PCIe 枚举里原样重现（配置空间 ≈ 描述符、BAR 分配 ≈ 地址分配）；四种传输类型里 Isochronous 的"不重传"取舍，与 B-B.5 UART 无校验流控、F.16.1 SerDes 的 FEC 取舍构成可靠性的三个档位；Gadget 侧（设备角色）的实现见 C.7.4/7.5。
