# B-C.8.4 SPI NAND与QSPI

> 所属章节：第五部 B. 总线协议 > B-C.8 SPI系列
>
...
SPI协议族到这里就告一段落了。你掌握了从基础四线SPI到QSPI/OSPI高速扩展，从EEPROM到NOR/NAND Flash的完整知识体系。下一节我们将进入一个全新的领域——**B-C.9.1 MIPI D-PHY物理层**，这是现代嵌入式显示和摄像头接口的物理基础。MIPI D-PHY的低压差分信号（LP/HS模式切换）与SPI完全不同，它为DSI（显示）和CSI（摄像头）提供了高带宽的物理层传输能力。
