...
## <span class="blue"> UFS 架构：一套为性能设计的协议栈
...
## <span class="blue"> UFS vs eMMC：代际差距在哪
...
## <span class="blue"> Linux ufshcd 驱动
...
## <span class="blue"> 实战验证：从初始化日志到性能基准
...
| 症状 | 根因 | 定位动作 |
|------|------|----------|
| dmesg 无 UFS 任何输出 | ref_clk 频率/摆幅不对；VCCQ 供电缺失 | 示波器量 REF_CLK（38.4MHz？幅度够？）；量三路供电 |
| link up 但 Gear 低于预期 | 设备树 lanes/Gear 配置保守；PCB 走线质量 | 核对 `lanes-per-direction`；降 Gear 复测区分配置与信号问题 |
| 只有 1 lane | `lanes-per-direction` 写错，或 lane1 焊接/走线问题 | 先查设备树，再查硬件 |
| 性能远低于标称 | fio 队列深度太小；没绕开页缓存 | `iodepth=32 --direct=1` 重测 |

---

## <span class="blue"> 本节总结

UFS 不是 eMMC 的提速版而是换了一套世界观：物理层换 M-PHY 差分串行（TX/RX 物理分离成全双工），协议栈搬来服务器领域的 SCSI（32 条命令乱序队列），分区模型从 eMMC 的物理固化换成 LU 逻辑单元。两条认知最值钱：随机 IOPS 的差距（10~20 倍）比带宽差距（5 倍）更决定系统顺不顺，因为 App 启动、数据库、日志全是随机小 I/O；全双工是读写混合场景的本质优势。落到工程上记住三处：`/dev/sdX` 背后是 ufshcd + SCSI 子系统；设备树的 `lanes-per-direction` 和 `ref_clk` 是两大配置雷区；fio 测随机必须 `iodepth=32` 才吃得到命令队列。选型判据一句话：有随机 I/O 密集负载上 UFS，纯顺序记录场景 eMMC 够用且省一半成本。

速查表：

| 主题 | 要点 |
|------|------|
| 四层栈 | SCSI 应用层 → UTP（UPIU 封装）→ UIC（UniPro 链路）→ M-PHY（差分串行） |
| Gear 档位 | Gear3 5.8G / Gear4 11.6G / Gear5 23.3G（每 lane 每方向） |
| 全双工 | TX/RX 差分对物理分离，读写同时跑满；REF_CLK 独立参考时钟 |
| 命令队列 | 32 条乱序并发——随机 IOPS 领先 eMMC 一个数量级的根源 |
| LU 分区 | 最多 8 个逻辑单元，产线可配容量；Boot LU A/B 支撑无缝更新 |
| 内核路径 | ufshcd → SCSI 子系统 → `/dev/sdX`（嵌入式板上见到 sda 即 UFS） |
| 设备树雷区 | `lanes-per-direction` 写 1 带宽减半；`ref_clk` 频率必须与晶振一致 |
| fio 测法 | 顺序 128K direct；随机 4K + iodepth=32 吃满队列 |

本节自查：

1. 画出 UFS 四层协议栈并说清每层职责，解释复用 SCSI 的收益。
2. TX/RX 物理分离为什么构成全双工？eMMC 的半双工在读写混合场景下的代价是什么？
3. UFS 随机 IOPS 领先 eMMC 一个数量级的机制是什么？fio 的 `iodepth=32` 对着谁设计的？
4. LU 与 eMMC 物理分区的差异是什么？Boot LU A/B 的升级流程怎么走？
5. 在嵌入式板子上看到 `/dev/sda`，怎么确认背后是 UFS？初始化日志里哪一行是协商结果的关键证据？
6. 给定一个数据记录仪和一个车载信息娱乐系统，分别选 UFS 还是 eMMC？说出判据。

---

## <span class="blue"> 下一步

大容量闪存看完了，回到小容量但无处不在的另一族：**B-C.8.4 SPI NAND 与 QSPI**——启动代码的保险箱（SPI NOR）、便宜大碗但有坏块的 SPI NAND、数据线加倍的 QSPI/OSPI，以及裸 Flash 的统一抽象 MTD。

> 💡 螺旋衔接：UFS 的命令队列思想与 D.10 PCIe/NVMe 的队列模型同源（NVMe 把队列深度推到 64K）；全双工差分与 F.16.1 SerDes 通识直接衔接；Boot LU A/B 与 eMMC 双 Boot 分区、第 21 章 OTA 的 A/B 方案是同一设计模式的三次出现；SCSI 子系统的错误恢复路径对照 B-B.6.4 I3C 的四级恢复可见"错误可归因、状态可复位"的跨总线共性。
