# B-C.7.4 USB Gadget模式与ConfigFS

> 所属章节：第五部 B. 总线协议 > B-C.7 USB协议族
>
...
USB Gadget模式让嵌入式设备可以灵活地扮演各种USB角色，但我们关注的不仅是协议本身，更是数据如何可靠地存储。下一节 **B-C.8.1 eMMC协议深度解析** 将带你深入了解嵌入式设备最主要的内部存储介质——eMMC。我们会详细讲解eMMC的物理接口、命令协议、EXT_CSD寄存器，以及如何在Linux中通过MMC子系统驱动eMMC设备，理解它与SD卡的区别和联系。
