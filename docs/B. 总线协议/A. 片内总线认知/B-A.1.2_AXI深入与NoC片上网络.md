...
B-A.1.1 给了片内总线的地图：设备挂在哪条总线、地址多少、设备树怎么配。本篇往下走一层，回答"软件什么时候会被总线层的问题咬到"——DMA 传输为什么要求地址对齐、`ioremap` 与 `ioremap_wc` 映射出来的内存有何不同、系统 CPU 负载不高却周期性丢帧该怀疑谁。这些问题的答案都在 AXI 的事务属性与互连结构里。
