...
| 症状 | 根因 | 定位动作 |
|------|------|----------|
| Sats 恒 0~2、Fix 恒 0 | 室内 GPS 信号穿不过钢筋混凝土，不是代码问题 | 模块放窗边/引出天线；等 30~60 s 看 PPS 灯 |
| 间歇性校验和失败、字段错位 | 未开 raw，ldisc 转换 0x0D、吞 0x11/0x13 | `stty -a` 看到 `icrnl`/`ixon` 即实锤；补 raw 配置 |
| 字段错乱、解析结果乱跳 | 把 read 返回值当行边界，半行就解析 | 按 `$`/`\n` 自行组帧，read 不保证行边界 |
| 落点偏差几十公里 | ddmm.mmmm 被当十进制度直接解析 | 拆分度与分：度 + 分/60 |
| `cat` 出来全是乱码 | 硬编码 115200，NEO-6M 默认 9600 | UART 设备第一动作永远是查手册默认波特率 |

---

## <span class="blue"> 本节总结

这个实战展示的是 UART 设备的标准形态：没有内核驱动路径，没有设备树绑定，一台外设只管往串口倒数据，全部工作在用户态。流程骨架与 EEPROM/Flash 实战依然同构——读手册、接线、设备树、工具先行验证、再写代码——但"工具先行"在 UART 场景的分量更重：`stty raw` + `cat` 看到 `$GP` 滚动流，物理层、串口配置、模块状态三件事一次确认，之后全是纯软件问题。

解析层的三个纪律都是血汗教训：组帧必须自己做（read 不保证行边界）、raw 模式不可省（ldisc 会静默改写数据）、校验和先行（坏帧不污染位置数据）。加上 NMEA 自己的两个坑——ddmm.mmmm 不是十进制度、Fix=0 时经纬度字段无效——这个模块的全部陷阱就齐了。GPS 特有的耐心指标是 TTFF：冷启动 30~60 秒是物理过程不是故障，室内永远定不了位是射频常识不是 bug。

**速查表**

| 项 | 要点 |
|----|------|
| 硬件 | TX/RX 交叉、必共地、确认 IO 电平（5V 模块要电平转换） |
| 设备树 | aliases 定 ttyS4 编号 + status okay；base_baud 与 GPS 波特率无关 |
| 工具先行 | `stty 9600 raw -echo` + `cat` 看到 `$GP` 流再写代码 |
| NMEA | `$` 起 `\r\n` 止、逗号分字段、`*` 后 XOR 校验；GPGGA 是主来源 |
| 坐标 | ddmm.mmmm = 度 + 分/60；Fix=0 时字段无效必须丢弃 |
| termios | 9600 8N1 + raw（关 ICANON/ECHO/ISIG/IXON/ICRNL/OPOST）；VMIN=0 VTIME=5 |
| 解析 | 逐字节组帧 → XOR 校验先行 → strtok 切字段 |
| 冷启动 | TTFF 30~60 s 正常；室内定不了位是射频常识 |
| 无硬件 | socat PTY 对 + 灌 NMEA 样例文本，软件链路全同 |

**本节自查**

1. 为什么 GPS 实战没有"内核驱动路径"可走？这与 EEPROM/Flash 实战的本质差异是什么？
2. `read()` 一次返回半行 NMEA 语句，直接解析会怎样？正确的组帧锚点是哪两个字符？
3. 不开 raw 模式，哪两个字节会被 ldisc 处理掉？分别导致什么症状？
4. `4807.038,N` 换算成十进制度是多少（写出算式）？
5. 程序里为什么必须先判 Fix 字段再使用经纬度？
6. 板子放桌上调试两小时没定位，按什么顺序排除"代码问题"的嫌疑？

---

## <span class="blue"> 下一步

UART 板块至此完整收口：物理层（5.1）→ 驱动与调试（5.2）→ RS-485/Modbus（5.3）→ 本实战。接下来 **B-B.6 I3C** 看低速传感器总线的现代化演进——同样两根线，12.5 MHz、带内中断、动态地址分配，I2C 的正式接班人。
