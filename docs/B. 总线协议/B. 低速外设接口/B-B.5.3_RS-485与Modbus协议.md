...
<!-- 【待补图】images/b-b-5-3-common-mode.png（优先级：△有更好）
图名：RS-485 差分共模抑制原理
生图提示词：波形对比示意图，上下两部分。上部：A、B 两条信号线波形（蓝色与橙色镜像差分），中途叠加一段剧烈的共模噪声（A、B 同时向上隆起 +5V，灰色底纹标注"动力电缆感应的共模噪声"）；下部：A−B 差值波形（绿色），噪声段几乎平直不受影响，标注"差值判决：噪声被抵消"。右侧竖排标注判决门限 ±200mV 两条虚线。示波器风格，白底，蓝橙绿三色，中文标注，比例 16:9。 -->
...
> 💡 无 485 硬件时的完整演练：PC 上用 socat 建 PTY 对，一端跑 pymodbus 从机模拟器（或 diagslave），另一端用 libmodbus/pymodbus 主站读写寄存器——RTU 帧收发、CRC 验证、异常响应全部走通，协议层与硬件无关。
...
| 症状 | 根因 | 定位动作 |
|------|------|----------|
| 末字节被截断或收到自己发的数据（echo） | DE 切换早于 TEMT，方向没等发完就切 | 等发送保持寄存器空再切；用内核 RS485 模式托管 |
| 总线电阻实测约 40 Ω，信号质量差 | 中间设备误闭合 120 Ω 跳线 | 终端电阻只在物理两端；逐个检查跳线状态 |
| 通信全无甚至烧收发器 | GND 未共地，两地电位差超 −7~+12 V 共模范围 | 长线带信号地线，或换隔离收发器（ADM2483） |
| 某台设备永远"通信不通" | 从机地址误设成 0（广播地址不应答） | 从机地址从 1 开始分配；核对拨码/配置 |
| 高负载下 CRC 错误频发 | 用户态主站调度延迟撑破 1.5T 字节间隔 | 提进程优先级（SCHED_FIFO）或移内核态；先查调度再查硬件 |

---

## <span class="blue"> 本节总结

RS-485 和 Modbus 是工业串行通信的两层答案，分开记才不会混：差分电气层解决"一对线挂几十个节点、扛千米干扰"（共模抑制是物理基础，终端/偏置电阻是工程配套），Modbus 协议层解决"各家设备说同一种语言"（地址点名 + 功能码动词 + CRC 保真）。排查时的第一层二分就是这一刀：量 A−B 空闲电压和总线电阻是电气层问题，看帧边界和 CRC 是协议层问题——本篇的五步排查流程（电压→电阻→参数→抓包→CRC）正是按这两层组织的。

两个机制细节值得超出本篇范围地记住。DE/RE 方向切换的 TEMT 等待是"软件必须尊重硬件完成时刻"的又一例（和 EEPROM 写周期、Flash WIP 同族），内核 RS485 模式把它收进驱动正是为了摆脱用户态调度延迟；Modbus 用 3.5T/1.5T 时间静默定帧边界，是无标志位协议的典型设计，代价是把实时性要求转嫁给了主站——高负载 CRC 错误先查调度延迟，这是反直觉但高产的排查方向。

**速查表**

| 项 | 要点 |
|----|------|
| 差分原理 | 看 A−B 差值（±200mV 门限）；共模 −7~+12 V；GND 必须共地 |
| 总线工程 | 两端各 120 Ω（实测约 60 Ω）；偏置保空闲态；stub <0.3 m |
| 节点数 | 32/128/256 取决于单位负载；超标表现为间歇失败 |
| 方向切换 | DE 拉高早于首 bit、拉低晚于 TEMT；echo 成因在此 |
| 内核 RS485 | `rs485-rts-active-high` + TIOCSRS485；优于用户态翻 GPIO |
| Modbus 帧 | 地址+功能码+数据+CRC16；3.5T/1.5T 静默定界 |
| 功能码 | 0x03 读保持寄存器最常用；异常响应 = 功能码+0x80 |
| CRC16 | 0xA001 反射、初值 0xFFFF、低字节在前 |
| 排查五步 | 空闲电压 → 总线电阻 → 参数三方一致 → socat 抓包 → 逐字节 CRC |

**本节自查**

1. 共模噪声 +5 V 叠加在 A/B 上，为什么差分接收端不受影响？共模范围的极限是多少？
2. 断电测总线 A-B 电阻约 120 Ω，说明什么？约 40 Ω 又说明什么？
3. DE 拉低为什么要等 TEMT 而不是等 FIFO 空？不等会怎样？
4. 内核 RS485 模式比用户态 GPIO 翻转可靠在哪两个层面？
5. Modbus RTU 靠什么定帧边界？这条规则给主站实现加了什么约束？
6. 从机回复 `01 83 02 CRC`，这笔响应在说什么？

---

## <span class="blue"> 下一步

UART 板块到此收尾：**B-B.5.4 实战篇** 用 NEO-6M GPS 模块把 5.1~5.3 串成完整项目——设备树使能、termios 编程、NMEA 解析与冷启动陷阱。之后进入 **B-B.6 I3C**：MIPI 联盟如何用两根线做到 12.5 MHz、带内中断与动态地址分配，低速传感器总线的现代化演进。
