...
I2C 板块到此闭环：物理层（2.1）→ 协议层（2.2）→ 驱动框架（2.3）→ 调试（2.4）→ 实战（2.5）。下一条总线是 **B-B.4 SPI**：同样是板级低速总线的主力，但走全双工、四线、片选的路子——物理层四种模式（3.1）、协议与时序（3.2）、驱动框架（3.3）、调试与选型（3.4），实战篇用 W25Qxx SPI NOR Flash 收尾（3.5）。
