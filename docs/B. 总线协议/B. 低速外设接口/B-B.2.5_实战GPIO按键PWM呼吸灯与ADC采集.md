...
| 症状 | 根因 | 定位动作 |
|------|------|----------|
| 程序报打开失败 / 无反应 | 设备树没生效，三件套注册缺失 | 先跑三查（iio/pwm/input），缺哪个回设备树补哪个，不在应用层排查 |
| LED 不亮但程序无报错 | PWM 引脚被复用冲突或当 GPIO 占用 | 查 pinctrl 是否配到 PWM 功能；`gpioinfo` 看引脚占用状态（呼应 B-B.2.2 四查） |
| 一次按键连跳 2~3 个模式 | 防抖未生效，机械抖动 5~20 ms 被当成多次边沿 | 确认 `debounce_period_us` 已设置且 ≥20 ms；`cat` 中断计数对比按键次数 |
| 亮度上限明显偏低（最高只有约 1/4） | 把 10-bit ADC 当 12-bit 换算（1023 误写 4095） | 换平台先查 ADC 位宽；核对 `ADC_MAX` 与 `in_voltageX_raw` 满量程读数 |
| 旋钮调节无响应 / 响应方向不对 | IIO 通道号与原理图网络名张冠李戴 | 万用表量抽头电压，对照各通道 raw 值换算结果，一次确认映射 |
...
工程上有四个细节值得带走，它们都不在 API 文档的显眼位置：防抖在内核配置而非应用层延时，poll 超时可以兼作节拍源省掉定时器线程，sin² 曲线是对人眼对数感知的修正而非炫技，PWM 的纳秒单位与 `duty ≤ period` 钳位是 sysfs 接口最容易写错的地方。最后记住调试顺序的铁律：三查注册先行，应用代码殿后——注册层面的问题在应用层永远查不出来。

**速查表**

| 项 | 要点 |
|----|------|
| 系统骨架 | 采集（ADC）→ 决策（模式机）→ 输出（PWM），三路资源一个 poll 循环 |
| 设备树 | saradc（vref-supply）/ pwm0（pinctrl）/ gpio-keys 三个使能点 |
| 按键 | libgpiod v2：下降沿 + `debounce_period_us(20000)` 内核防抖 |
| ADC | `in_voltageN_raw` 单次读；RK3568 为 10-bit 满量程 1023 |
| PWM | 纳秒单位；duty = period × ratio；写 duty 前钳位 ≤ period |
| 呼吸曲线 | sin² 修正对数感知；相位步进 × 旋钮上限 |
| 验证顺序 | 三查注册 → 联调清单 7 步 → 快速连按验证防抖 |
| 无硬件 | gpio-sim / pwm-sim / IIO dummy 三件套，程序原样运行 |

**本节自查**

1. 三态模式机的状态切换由什么事件驱动？电位器在状态机里扮演什么角色？
2. 为什么用 `poll` 的 20 ms 超时兼作呼吸节拍，而不是另开定时器线程？
3. 内核防抖与"应用层延时再确认"相比，赢在哪两点？
4. `pwm_set_ratio` 里的钳位防护的是哪条 sysfs 写入约束？
5. 程序跑起来 LED 不亮，按什么顺序排查（写出前三步）？
6. 把输入源换成 I2C 温度传感器、输出保持 PWM 呼吸灯，本程序的哪几个函数需要改、哪几层骨架不动？
