...
- **SCL**（Serial Clock）：时钟线，主设备驱动；从设备可拉低实现时钟延展（见 B-A.2.2）
...
I2C 通信异常时，第一排查手段分两步：**断电状态用万用表量 SDA/SCL 对地电平**——上电空闲时应为上拉电压（≈VDD），若接近 0 V 说明有设备死拉总线或上拉缺失；**空闲电平正常后跑 `i2cdetect -y -r <bus>` 扫描**（工具细节见 B-A.2.4），扫不到地址再回头查物理层。
...
- **工具**：i2c-tools（`i2cdetect` 等，详见 B-A.2.4）、示波器/逻辑分析仪

---

## <span class="blue"> 下一步

物理层就绪后进入协议层：**B-A.2.2 I2C 协议层与帧格式**——地址帧与读写位、完整读写事务的帧序列、重复 START 的用途、10 位地址与时钟延展。随后 **B-A.2.3** 讲 Linux 驱动框架，**B-A.2.5 实战篇**用 AT24C02 EEPROM 把物理层到代码全链路走一遍。

> 💡 螺旋衔接：本篇的开漏/线与正是 B-A.1.1「开漏必配上拉」的总线级应用；设备树里 I2C 控制器挂在 APB 上的位置，回看 B-D.13.1 的地址映射。
