...
③ i2cget/i2ctransfer 读写寄存器
   ...
   └─ 读写正常 ↓
④ 数据值不对 → 协议理解问题：字节序、寄存器映射、复合消息（B-A.2.2）
⑤ 间歇性失败 → 上仪器看波形：边沿圆钝/毛刺 → 物理层（B-A.2.1）
```
...
i2c-tools 是 Linux I2C 调试的事实标准，全部走 `/dev/i2c-N` 用户态接口（B-A.2.3）。
...
三种格子含义：数字 = 设备应答；`--` = 无应答；**`UU` = 该地址已被内核驱动绑定**（此时用户态访问会被拒绝，见 B-A.2.3 陷阱）。
...
"写寄存器地址再读数据"这类操作必须用 i2ctransfer——拆成 i2cset + i2cget 会在中间插入 STOP，部分器件因此复位内部地址指针，读回错误数据（B-A.2.2 的原子性问题）。
...
解码结果逐条对应 B-A.2.2 的帧格式：START / 地址+R/W / ACK / 数据 / NACK / STOP。抓包时重点核对三处：
...
| 间歇性失败 | 边沿超标/线缆过长/速率过高 | 示波器量上升沿对照 t_R 表 | 减小上拉/缩短走线/降速（B-A.2.1） |
| 总线挂死 | 时钟延展死锁/异常中断未发 STOP | 量空闲电平 + 看 SCL | clock out 恢复（B-A.2.2） |
...
工具链就绪后进入实战：**B-A.2.5 实战：AT24C02 EEPROM 端到端**——从手册时序读起，经设备树配置、内核 at24 驱动、sysfs 与 `/dev/i2c` 双路径读写，到写周期与页边界两个经典坑的实测验证，把 B-A.2.1~2.4 的知识全部落地。

> 💡 螺旋衔接：决策树 ① 的电气参数表在 B-A.2.1；抓包解读的帧格式在 B-A.2.2；UU 与 `I2C_SLAVE_FORCE` 的机制在 B-A.2.3 用户态一节。本篇是 I2C 板块四篇知识的收敛点。
