# B-A.1.5 实战：GPIO 按键 + PWM 呼吸灯 + ADC 电位器采集
...
B-A.1.1 ~ B-A.1.4 分别讲了 GPIO、PWM、ADC、DAC 四件套。本篇把它们合成一个可运行的最小产品：**电位器旋钮调节 LED 呼吸灯的亮度上限，按键切换工作模式（常灭 / 呼吸 / 常亮）**。三个子系统在同一份设备树、同一个程序里协同工作，覆盖嵌入式产品最常见的"采集 → 决策 → 输出"闭环。
...
2. **防抖在内核完成**：`gpiod_line_settings_set_debounce_period_us(20000)`，应用层不再写延时确认循环（B-A.1.1 的 v2 差异点在此落地）
3. **sin² 呼吸曲线**：人眼对亮度是对数感知（B-A.1.2 的对数表同理），线性三角波呼吸在高亮段显得"顿"，sin² 平滑且计算便宜
...
> ⚠️ PWM 引脚被复用冲突。程序运行无报错但 LED 不亮：查 pinctrl 是否真正把引脚配到 PWM 功能、`gpioinfo` 看引脚是否被当 GPIO 占用（与 B-A.1.2 的四查呼应）。
...
基础外设板块（B-A.1）到此完整闭环。下一篇进入 I2C 系列——**B-A.2.1 I2C 物理层与电气特性**：两线制、开漏与上拉、线与仲裁，理解 B-A.1.1 埋下的"开漏必配上拉"在真实总线上如何变成完整的通信协议。

> 💡 螺旋衔接：本篇的"采集→决策→输出"闭环是嵌入式产品的最小骨架——B-A.2/B-A.3 的 I2C/SPI 实战篇（AT24C02、W25Qxx）会把输入源换成总线器件，骨架不变；按键中断的完整内核态写法（request_threaded_irq + input 上报）归 D 扩展中断篇，与第 10 章中断机制对接。
