...
## <span class="blue"> PROFINET：三档实时等级
...
## <span class="blue"> GSDML：设备描述驱动的组态流程
...
## <span class="blue"> PROFINET 与 EtherCAT 的取舍
...
## <span class="blue"> OPC UA：解决语义互操作
...
## <span class="blue"> Client/Server 与 Pub/Sub
...
## <span class="blue"> open62541 最小用例
...
## <span class="blue"> 排障：PROFINET 与 OPC UA 常见故障
...
`B-D.12.5 实战：IgH EtherCAT 主站搭建与 CSP 伺服控制` 是工业以太网组的收官实战：从编译 IgH 主站、组态伺服链，到用 ecrt 写一个 CSP 周期位置控制程序——12.2/12.3 的机制（Datagram、DC、SYNC0、ecrt 四段式）全部在那里落地为可运行的工程。

> 💡
> 深入阅读：IEC 61158 Type 10（PROFINET）、PI 系统描述、IEC 62541（OPC UA，Part 14 是 Pub/Sub）、open62541 官方文档（open62541.org，含服务器/客户端/PubSub 教程）、OPC 基金会配套规范库（ISA-95、PackML、OPC 40001）、OPC UA FX 进展见 FLC 工作组。本篇引用的本书章节：12.1（版图与三分类）、12.2/12.3（EtherCAT 机制与 DC）、E.15.1（工厂自动化整机架构）。
