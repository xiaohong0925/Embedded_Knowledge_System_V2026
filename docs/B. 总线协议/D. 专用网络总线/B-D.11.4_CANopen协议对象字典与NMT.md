...
## <span class="blue"> 先认帧：COB-ID 的分配规则
...
## <span class="blue"> PDO 映射：OD 条目到 CAN 帧字节
...
## <span class="blue"> 节点在线监控：心跳与节点守护
...
## <span class="blue"> 完整启动序列：伺服驱动器实例
...
## <span class="blue"> 排障：NMT 与 OD 相关故障
...
下一篇 **B-D.11.5 CANopen PDO/SDO 与 Linux 驱动**：本篇的对象字典和 NMT 是"数据模型与生命周期"，下一篇把 PDO/SDO 的传输机制细节（SDO 命令符全集、分段传输、块传输）和 Linux 主站的代码实现（CANopenNode/LELY 选型与集成）讲完， candump 里那些 `40/43/2B/2F` 命令符在那里逐一拆解。

> 💡 本篇是应用层协议，往下每帧的传输都靠 B-D.11.3 的 SocketCAN；帧 ID 的优先级依据（ID 越小越优先）在 B-D.11.1 的仲裁机制；CAN FD 承载的 CANopen FD（CiA 1301）与 64 字节 PDO 的演进背景在 B-D.11.2。开源协议栈参考 CANopenNode（github.com/CANopenNode/CANopenNode），伺服状态机的权威定义在 CiA 402。机器人关节伺服组网的综合实战见 B-E.15.3。
