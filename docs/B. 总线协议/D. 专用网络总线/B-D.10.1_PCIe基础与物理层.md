...
>
> 难度：[B] Beginner ~ [I] Intermediate | 预计阅读时间：40 分钟
...
PCIe 取代 PCI 的整套逻辑可以压成一句话：把"大家排队用一条共享马路"换成"每对设备之间拉专线，再用交换结构把专线织成网"。仲裁消失、带宽独享、扩展靠堆 Lane 而不是硬推频率——后面学到的一切（树形拓扑、Switch、xN 宽度、LTSSM 协商）都是这个基本决策的推论。反过来说，点对点的代价是广播介质消失，一切通信变成数据包交换，这正是三层架构存在的原因。

分层是排障的地图。事务层管"软件要什么"（读写内存、配置、中断都是 TLP），数据链路层管"包不能丢"（ACK/NAK + 重传缓冲 + 流控信用），物理层管"比特怎么上线"（编码 + LTSSM 训练）。记住两个对称的认知：Non-Posted 事务必有回程包，所以"读寄存器卡死"问题在对端不在 CPU；链路层重传会掩盖物理层误码，所以误码恶化的表现不是数据出错而是降速和抖动。

工程上最常用的三个把手：带宽口诀（每 Lane 每代约 250/500/1000/2000/4000 MB/s，乘 Lane 数）、`lspci -tv` 读树（缩进即父子）、`lspci -vv` 的 `LnkCap`/`LnkSta` 对比（能力 vs 协商结果，降速实锤）。SSD 跑不满先查协商速率，设备认不到先想 LTSSM 卡在哪个状态——这两个反射动作能覆盖嵌入式 PCIe 问题的八成。

### 速查表

| 项 | 要点 |
|----|------|
| 拓扑 | RC 一棵根，Switch 扩端口，EP 是叶子；`lspci -tv` 直接可见 |
| Lane | 两对差分 = 全双工；宽度取双方最大公约数，物理槽宽 ≠ 实际接线 |
| TLP 分类 | Posted（发完就走，如 DMA 写）vs Non-Posted（必等 Completion，如寄存器读） |
| MSI 本质 | 向特定内存地址发 Memory Write TLP，没有物理中断线 |
| 流控 | 信用制主动配额，链路永不因对端来不及收而丢包 |
| LTSSM | Detect→Polling→Configuration→L0；卡 Detect 查供电/复位，卡 Polling 查时钟/信号 |
| 带宽口诀 | 每 Lane 单向 ~250/500/1000/2000/4000 MB/s（Gen1~5） |
| 降速判读 | `LnkSta` 低于 `LnkCap` = 训练失败的实锤 |

### 本节自查

1. 共享并行总线的三个局限分别是什么，点对点串行如何一次性解决掉它们？
2. x16 显卡插在只接了 x4 走线的 x16 插槽上会发生什么？用哪两个字段确认？
3. 驱动里一次 `readl()` 卡死，为什么可以断定问题不在 CPU 侧代码？
4. 链路从 Gen4 降到 Gen2 运行，中间大概率发生过什么？
5. 一张标称 7 GB/s 的 Gen4 NVMe SSD 插在 Gen3 x4 的 M.2 口上，理论上限是多少？

## <span class="blue"> 下一步

下一篇 **B-D.10.2 PCIe 枚举与配置空间**：本篇讲了链路和拓扑这些"新东西"，下一篇回到 PCI 时代传下来的老模型——BDF 地址怎么分配、256 字节配置空间里每个字段什么含义、BAR 怎么把设备寄存器映射进内存地址空间，`lspci -vv` 的完整输出将在那里逐段解读。

> 💡 本篇埋了三条往后的线：MSI"中断就是一笔 Memory Write TLP"的完整机制接第 10 章中断子系统（10.2 节中断入口）；设备 DMA 读写内存的驱动侧实现（`dma_alloc_coherent`、流式映射）在 B-D.10.3 展开；物理层之下的 SerDes、均衡、眼图在 B-F.16.1 有完整论证，10.4 的降速定位方法论会把两边接上。工具方面，`lspci`/`setpci` 的逐字段用法从下一篇开始密集出现。
