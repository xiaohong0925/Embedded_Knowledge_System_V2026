...
<!-- 【待补图】images/b-d-11-6-wiring.png（优先级：△有更好）
图名：双板 CAN 链路接线实物示意
生图提示词：技术接线示意图，白底工程风格，中文标注，横版 16:9。左右两块开发板（标注"板 A Linux can0"与"板 B Linux can0"），中间一条双绞线标注"CAN_H / CAN_L 双绞"，线的两端各画一个 120 Ω 电阻跨接在 CAN_H 与 CAN_L 之间并标注"终端电阻 120 Ω ×2，测总电阻 ≈60 Ω"。下半部分画一台伺服驱动器（标注"CiA 402 伺服 Node-ID=1"）挂接在同一总线上。扁平矢量、细线条、无装饰。 -->

## <span class="blue"> 第一段：vcan 虚拟链路（无硬件路径）
...
## <span class="blue"> 第三段：CANopen 伺服接入
...
本篇把 CAN 组五篇的机制全部变成了手感。链路层的心法是一句话：**先证明物理链路健康，再谈协议**。双板对测三步（单向通断 → FD 专项 → 负载加压）加上拔终端电阻的错误注入演练，把 11.2 那张错误状态机表从纸面变成了亲眼所见的 TEC 爬升与状态降级——见过故障长什么样，排障时才不会慌。vcan 则是零成本的备胎：所有软件路径先在虚拟接口上跑通，再原样切到 can0，环境问题的嫌疑就提前排除了。

协议层的心法是另一句话：**手工帧用来理解，协议栈用来量产**。整个伺服控制程序没有任何黑盒——NMT Start、SDO 命令字节 `0x2F/0x23`、控制字三级跳 `0x06→0x07→0x0F`、bit4 上升沿触发，每一帧都能对照 11.4/11.5 的机制逐字节解释。状态字位掩码解码（`sw & 0x6F`）是 CiA 402 调试的反射动作，看到"电机不动"的第一反应永远是读状态字对状态机。而切换判据也很明确：控制周期进 1 ms 量级、轴数超过两个、需要分段传输时，手工帧的维护成本立刻超过 CANopenNode 的学习成本——理解之后再换工具，换得理直气壮。

速查：

| 要点 | 结论 |
|:---|:---|
| 无硬件路径 | vcan 虚拟接口跑通全部软件流程，但不支持波特率参数 |
| 链路健康三证据 | 终端电阻 ≈60 Ω、canbusload 负载率、错误计数不爬升 |
| FD 帧识别 | candump 中 `##` 分隔符 + flags（0x1=BRS / 0x4=FDF） |
| 伺服上线确认 | 发 `000#8100` Reset，看到 `701#00` Boot-up 再往下走 |
| 最小控制路径 | SDO 配参数（0x6060/0x607A）+ RPDO1 跑控制字 + TPDO1 收状态字 |
| 状态字掩码 | `sw & 0x4F`/`sw & 0x6F` 判状态，bit10=到位，Fault 先读 0x603F |
| 使能序列 | 0x06 → 0x07 → 0x0F 逐级发，触发用 bit4 上升沿（0x1F→0x0F） |
| 换协议栈判据 | 周期 ≤1 ms / 轴数 >2 / 需要分段 SDO，任一成立即换 CANopenNode |
...
`B-D.12.1 工业以太网总览：EtherCAT/PROFINET/EtherNet-IP 版图` 开启工业以太网组。CAN 搞定了机器人的关节级通信，但当轴数上到几十、周期压到 250 µs 以下、还要和 PLC 工厂网络互通时，CAN 的 1 Mbps 与 8 字节 payload 就不够用了——下一组看工业以太网如何用以太网的带宽做到比 CAN 更狠的确定性。

> 💡
> 本篇工具链的出处：can-utils（candump/cansend/cangen/canbusload/canfdtest）在 github.com/linux-can/can-utils，vcan 配置见内核文档 `Documentation/networking/can.rst`；CiA 301/402 定义了 SDO 命令字节与控制字/状态字位；伺服默认 PDO 映射以所用型号的 EDS 文件与手册为准。多轴机器人场景的完整总线选型在 `B-E.15.3 机器人整机总线架构` 展开。
