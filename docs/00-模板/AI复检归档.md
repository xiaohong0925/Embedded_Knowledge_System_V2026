...
- **10.3 节组总结模式补全 + threaded IRQ 悬空修复（2026-09-02）**：10.3.6 导读"前面几节把五种底半部机制都讲完了"系假话（threaded IRQ 在 10.4，10.3.x 未讲）——导读纠正为"四种已讲完 + threaded IRQ 在 10.4"，并补 threaded IRQ 概念卡片（IRQ_WAKE_THREAD 唤醒专属线程、可睡眠，机制指针 10.4）。同日 10.3.1/10.3.5/10.3.6/10.3.7 四节总结按新模式重排（10.3.7 原有散文收口句上移至表格前），10.3 节组 7 节总结全部统一为「散文收口 + 速查表」。各节构建均 0 WARNING。
- **10.4.2 反悬空重构（2026-09-02，用户阅读反馈）**：「irq_thread 的创建与生命周期」小节改为四问驱动（何时创建/在哪等/被什么唤醒/怎么退出），补唤醒侧真实代码链 `__irq_wake_thread()`（handle.c:61：IRQTF_RUNTHREAD 已置位直接返回 + wake_up_process）与退出侧 `irq_finalize_oneshot()`（manage.c:1083）；删除纯复述代码的 mermaid 流程图（用户判定"程序员不看莫名其妙的流程图，看代码+注释"）。「优先级与调度策略」小节补动机段（唤醒≠执行，调度延迟是线程化的新代价，内核必须回答给什么优先级），sched_set_fifo 不再凭空出现；新增 SCHED_FIFO 概念卡片。本节总结按「散文收口+速查表」重排。新增缓存 `kernel_irq_handle.c`（此前两次拉取失败，本次 jsdelivr 成功）。构建 0 WARNING。
- **10.4.2 二次重构（2026-09-02，用户手改后整理）**：用户手动扩写生命周期小节后结构与事实双重混乱，整理要点——①层级：「阶段1/2」### 标题与加粗路标两套结构合并为统一 #### 五路标（何时创建/首次运行/在哪等/被什么唤醒/怎么退出），均归在 ### 创建与生命周期 之下；②去重：阶段1 两个调用树块合并为一个（删除重复出现的 API 原型）；③事实修正：`__irq_wake_thread` 用户手抄版参数顺序颠倒且丢了 PF_EXITING 检查，换 handle.c:61 真实节选（含 threads_oneshot 记账 + threads_active 计数）；④事实修正：`free_irq` 代码块系虚构（`wait_for_completion(&action->thread_done)` 不存在），换 `__free_irq()` 真实尾部（kthread_stop 自带阻塞等待 + put_task_struct），并点出与 10.3.4 destroy_workqueue 同语义；⑤irq_thread 主循环修正：直接调 `irq_finalize_oneshot(action->irq, ...)` 签名错误，改为真实包装 `irq_thread_fn(desc, action)`（manage.c:1207，内含 thread_fn + finalize 两步）；⑥删除示例中位置不当的"唯一一行驱动可见"注释（其内容已由生命周期小节首句承载）。构建 0 WARNING。
- **10.4.3 用户改造验收（2026-09-03）**：用户重写了 threadirqs 机制与 PREEMPT_RT 全景两大块，方向正确，核验后修五处——①`setup_forced_irqthreads` 函数体更正：v6.6 是 `static_branch_enable(&force_irqthreads_key)`（static key），不是 `force_irqthreads = true`（manage.c:30）；②GRUB 步骤补 `update-grub` + reboot（原流程缺重新生成配置，参数不会生效）；③删误导性"开启方法二"清单（CONFIG_IRQ_FORCED_THREADING=y 只是编译前提不是开启方式），改写为两个前提（config 编译进 + RT 内核默认强制）；④`irq_setup_forced_threading` 代码节选补回两个关键细节：`new->flags |= IRQF_ONESHOT`（强制线程化顺手补 ONESHOT——10.4.2"ONESHOT 标配"的代码层来源，已点破呼应）与 already-threaded 跳过分支；清除悬空编号 2.1/2.2/2.3 与错乱缩进；合并两句重复表述；⑤本节总结按「散文收口+速查表」重排。PREEMPT_RT 时间线（5.15 实时锁 / 6.12 收尾）、ktimersd、cyclictest 用法核验无误。构建 0 WARNING。
- **10.5 节组二轮复检（2026-09-07，节组级设计调研→逐节闭环）**：设计调研结论——大纲 1 节扩为 6 节（精度问题→分层架构→timer wheel→hrtimer→idle 交互→alarm/posix）扩编充分必要、叙事线顺畅、与 10.4/10.6 边界干净，不增删节。逐节修复——①10.5.1：补 `update_process_times()` 首次出现解释（反悬空），事实零错误；②10.5.2 两处事实错误：`struct timekeeper` 路径 kernel/time/timekeeping.h → **include/linux/timekeeper_internal.h**（前者不存在）；注册流程虚构的 `__clocksource_register()` → 真实 `clocksource_register_hz()` → `__clocksource_register_scale()`（clocksource.c:1186）；rating 表四值全核（arch_timer 400@:254、TSC 300、HPET 250、ACPI PM 200、jiffies 1）；③10.5.3 **改名去「级联」**：文件 `10.5.3_timer_wheel级联实现.md` → `..._时间轮实现.md`（内容早已是 4.8 后扁平实现，标题误导），同步 mkdocs.yml、第10章 README、10.5.2 下一步、10.99 清单、D.5 分工注记共 5 处；④10.5.4 两处事实错误：timerqueue 最左缓存 `timerqueue_head->next`（旧实现）→ v6.6 实为 `rb_root_cached` + `rb_first_cached()`（timerqueue.h:14/34）；「nanosleep 走 soft 类」错误——nanosleep 用 HRTIMER_MODE_REL 是 **hard 类**（hrtimer.c:2135），改为默认方向说明：不带 SOFT 标记皆 hard 类，PREEMPT_RT 反转（hrtimer.c:1557）；⑤10.5.5：`available_governor` → `available_governors`（sysfs.c:101）；音频案例"每 20.8μs 触发 DMA 填充"把采样周期当填充节奏，改为 period 64 帧 ≈1.33ms 双缓冲的真实节奏；测试模块补 `#include <linux/math.h>`（int_sqrt64）；⑥10.5.6：alarm API 签名更正——`alarm_start` 返回 **void**（非 int）、回调类型是 `enum alarmtimer_restart (*)(struct alarm *, ktime_t)`（alarm_callback_t 不存在，alarmtimer.h:51-53）；补 ALARM_*_FREEZER 变体一句；补 timerfd 概念卡片（fd 进 epoll，fs/timerfd.c）。六节总结全部重排为「散文收口+速查表」。新增缓存：clocksource.c/clockevents.c/timekeeping.c/clockchips.h/timekeeper_internal.h/jiffies.c/timer.h/timerqueue 两件/alarmtimer 两件/posix-timers.c/cpuidle sysfs.c/int_sqrt.c/tsc.c/hpet.c/acpi_pm.c/io_uring_timeout.c。每节构建均 0 WARNING。遗留：10.7.2 与 10.5.5 爆音案例收敛状态待 10.7 复检时反向核对。
- **10.6 节组二轮复检（2026-09-07，同流程）**：设计调研结论——大纲要素（periodic tick 问题/NO_HZ_IDLE/NO_HZ_FULL/cpuidle 协同/统计失真/timer_list 验证）4 节全覆盖，不增删节；10.6.3 对 10.5.5 的指针收敛良好。逐节修复——①10.6.1：事实零错误（10.1.1 跨节引用 tick_handle_periodic 属实），仅总结格式；②10.6.2 **P1**：「`mode: 2` 为 periodic」错误——tick-sched.h:7-10 枚举只有 0=TICKDEV_MODE_PERIODIC / 1=ONESHOT，与 10.6.4 口径统一；两段 tick_nohz_idle_enter/exit 代码节选复查与 v6.6 逐行一致；③10.6.3 **P1**：策略三误用 `low_power_idle_system_residency_us` 作延迟约束接口——该属性实为 Intel S0ix 平台**只读驻留统计**（LPIT 文档），整段重写为真正的约束接口 PM QoS `/dev/cpu_dma_latency`（写入生效/关 fd 解除/C 示例 + 内核侧 cpu_latency_qos_add_request）；爆音案例口径同步 10.5.5（period 64 帧≈1.33ms）；虚构符号 `nohz_full_kick` → `tick_nohz_full_kick_cpu()`（tick-sched.c:351）；配套资源行同步；④10.6.4：「msleep(1) 拖到几十毫秒」夸大——实为 jiffies 粒度取整 4~8ms@HZ=250 起步，改写并补 usleep_range 建议；tick_stop 事件格式/dependency 名/timer_list P() 字段复查无漂移。四节总结全部重排为「散文收口+速查表」。每节构建均 0 WARNING。新增缓存：drivers_cpuidle_cpuidle.c、abi_sysfs_cpu.txt。
- **10.7 节组二轮复检 + 第10章闭环（2026-09-07）**：设计调研——大纲排错指南 3 案例核对：案例1（PLC GPIO）→10.7.1 ✅、案例2（音频爆音）→10.7.2 ✅、案例3（中断风暴/NAPI）无独立节，落点为 10.7.2 症状对照表补一行（指针 10.3.5 + 第14章），不新增节；10.7.2↔10.5.5 收敛反向核对良好（无重复模块代码，均指针引用）。修复——①10.7.1：trace 示例头 5.10.65-rt53 → 6.6.0（对齐全书基准）；两个 C 代码块 `//`+emoji 注释统一为 `/* */` 行尾风格；②10.7.2 **P1**：示例输出 `samples: 2857142` 与 10.5.5 现行模块 MAX_SAMPLES=600000 矛盾（旧模块遗物），改为 600000；症状表补中断风暴行。两节总结重排为「散文收口+速查表」。index.md 第10章行更新为二轮复检完成/2026-09-07。每节构建均 0 WARNING。**第10章 31 节 AI 复检至此全部闭环**（一轮 8-10 全量事实修复 + 二轮 9-07 节组级设计调研驱动复查）。
...
## 第 17 章写作存档（2026-09-07 闭环）

- **结构变革**：从 v1 设计的 22 小节文件改为 v2「节=文件」6 篇厚章节（17.1~17.6 单文件 + README）。原因：小文件脚手架（导读/总结/速查表/下一步约 35-45 行）把正文稀释到每篇仅 45-60 行；原 22 小节降级为单文件内 `##` 段落，论证链按设计稿 §9 蓝图执行。旧 15 个 17.x.y 小文件已全部删除。
- **产出**：17.1 数据生命周期（263 行，失效时间线 mermaid timeline）/ 17.2 介质决策（325 行，判决流程 mermaid）/ 17.3 文件系统决策（323 行）/ 17.4 分区架构（384 行，OTA 决策树 mermaid，用户验收的样板）/ 17.5 断电安全（230 行，需求分级决策树 mermaid）/ 17.6 综合实战（约 490 行，走查流程 mermaid）。mkdocs.yml 第 17 章 nav 已更新为 6 篇结构，终验构建 0 WARNING，各篇 mermaid 产物渲染验证通过。
- **关键决策**：无字数上限（用户明示"只要把问题讲清楚"），验收以质量维度为准、行数为筛查信号；案例数字（智能音箱 3%→0.1%、HMI 5000 次断电、T-Box 写放大降 40%）按行业典型实践口径叙述不冒充实测；与 12/16/21 章重叠内容用螺旋分层话术显式呼应，章 README 放分工边界表；读者分层承载（术语就地解释服务零基础，论证链服务 3~5 年经验）。
- **17.6 走查冲突调和**（本章方法论的高光点）：17.2 判定的 pSLC（rootfs+recovery 2 GiB，TLC 3:1 计价耗 6 GiB）与 17.4 的 8GB 预算算不平——走查中暴露后按"保持力风险=静止时间×温度"重审，pSLC 收敛至 recovery 单区，rootfs 用 OTA 轮换+巡检重写兜底，寿命按 96% 占用率重算（WA 上调后约 68 年对 5 年），16GB 方案作为触发条件写死的备案。
- **待补图占位**（HTML 注释含完整生图提示词）：17.6 完整分区方案图（★必要）。
- 章 README 重写（失效时间线意义→方法论预告→12/16/21 分工边界表→双学习方案→6 篇阅读路线）；index.md 第 17 章行已更新为「完成 / 否（待人工复检）/ 2026年9月7日」。

## 第 18 章写作存档（2026-09-08 闭环）

- **从零写作**：本章原只有大纲拼贴骨架 README，按设计稿 `docs/00-模板/第18章_构建系统设计_设计稿_v1.md`（§8 开放问题全部闭环）从零写为 6 篇厚章节（节=文件结构，沿用 17 章 v2 模板）。
- **产出**：18.1 决策矩阵（279 行，2 张 mermaid 决策树）/ 18.2 TCO 模型（~230 行，xychart-beta TCO 曲线）/ 18.3 Layer/Recipe 组织（~300 行，任务链+分层依赖 mermaid）/ 18.4 CI/CD 流水线（~210 行，流水线 mermaid）/ 18.5 合规与 SBOM（~290 行，CVE 闭环 mermaid）/ 18.6 综合实战（~630 行）。mkdocs.yml nav 已更新为 6 篇结构，每篇写完即构建 0 WARNING，mermaid 产物渲染逐一验证。
- **关键决策**：贯穿场景定稿机器人整机厂商（AMR/机械臂/人形 3 产品线 5 SKU；合规锚点 = CRA + IEC 62443 审计；功能安全划给 19/30 章）；数通仪器仪表方案因行业偏窄被用户否决；大纲医疗场景保留为 18.6 案例一（FDA=合规驱动选型极端参照）。案例数字按行业典型实践口径叙述不冒充实测；TCO 按公式+量级示例口径。
- **时代性内容已联网核实**：Yocto LTS 6.0 Wrynose（2026-04，支持到 2030-04）/ 非 LTS ~7 个月；Buildroot 2025.02 起 LTS 延至 3 年、2026.02 改善 CycloneDX SBOM；OpenWrt 24.10 于 2026-09 EOL、25.12 起 apk 替换 opkg；CRA（EU 2024/2847）2026-09-11 漏洞报告义务生效（24h 预警/72h 通报，Art.69(3) 覆盖存量）、2027-12-11 全面适用；Yocto create-spdx/cve-check/CVE_STATUS 六态、2024 NVD 停摆 → CVEv5 + VEX（CSAF/OpenVEX）+ yocto-vex-check。
- **18.6 三悬念裁决**（本章方法论收口）：①人形线 Buildroot 诉求 → 按镜像角色选型（主系统 Yocto 统一 + recovery/开发镜像 Buildroot，defconfig 包数红线 40）；②CI 4h → 三层预算（PR 33min / Nightly 2h / Release 不计时），sstate 命中率 61%→89% 纯结构手段；③5 SKU → 四层 Layer 树（公司层/产品线层×3/BSP 层），bbappend 方向+补丁 Upstream-Status 固化为 CI 静态检查。20 项评审清单（四组、三里程碑 S/R/Y、逐项要证据）。
- **待补图占位**：第 18 章无生图占位（mermaid 覆盖充分）。
- 章 README 重写（债务滞后 vs 17 章失效滞后的意义对位→组织性决策主线图→2/5/17/19/21 分工边界表→双学习方案→6 篇阅读路线）；index.md 第 18 章行已更新为「完成 / 否（待人工复检）/ 2026年9月8日」。

## 第 22 章写作存档（2026-09-08 闭环）

- **从零写作**：原仅骨架 README，按设计稿 `docs/00-模板/第22章_驱动架构设计_设计稿_v1.md`（§8 五项裁决全部闭环）写为 6 篇厚章节（节=文件，沿用 17/18 章 v2 模板）。
- **产出**：22.1 驱动放置量化决策（~330 行，决策树 mermaid）/ 22.2 三条路径（~430 行，L1~L4 状态机+电源依赖图 mermaid）/ 22.3 HAL 位置（~340 行，决策树 mermaid）/ 22.4 总线选型与故障域（~350 行，纯文本+表格）/ 22.5 多媒体 pipeline（~400 行，全局 pipeline mermaid）/ 22.6 综合实战（~470 行，电源依赖图 mermaid）。mkdocs.yml nav 6 篇齐全，终验构建 0 WARNING，全部 mermaid 产物渲染验证通过。
- **用户裁决记录**（与 17/18 章"零重复"口径的差异，后续章节沿用）：①目的为讲清楚，不可避免的代码重复可接受，不必纯指针；②22.4 总线可适量扩展代码，节末指向 B 扩展；③22.5 必须有简单代码示例（"没有代码干讲不符合软件工程师直觉"）；④贯穿场景沿用大纲 IoT 平台；⑤22.6 综合实战模式同 17.6/18.6。
- **时代性事实已联网核实**：libgpiod 2.x（2025 v2.2）API 破坏性重建（v1 教程照抄编译失败）；UIO 的 DMA 被内核社区判"fundamentally broken and unsafe"（2025 LWN），正解 VFIO/IOMMUFD（6.2 合入），VFIO 前提是 SMMU 覆盖该设备；DPDK/SPDK 已迁 vfio-pci。
- **22.6 核心裁决**：五类外设四类进标准栈零私有接口；4G 零自研驱动（ModemManager）；供电域按唤醒/休眠组合聚类；触摸屏与传感器按故障域分总线（交互命脉独占 I2C 控制器）；摄像头带宽预算 300MB/s<10% 故**不上**零拷贝（否决型裁决与肯定型同等举证）；时间轴演练验收标准——换传感器收敛在驱动舱、换板卡在 DT 舱、换 SoC 在 BSP 舱。
- **待补图占位**：第 22 章无生图占位（mermaid+表格覆盖充分）。
- 章 README 重写（耦合滞后 vs 17 失效滞后、18 债务滞后的三部曲对位 → 五决策域主线图 → 10/11/15/D/B/30 分工边界表 → 双学习方案 → 6 篇阅读路线）；index.md 第 22 章行已更新为「完成 / 否（待人工复检）/ 2026年9月8日」。

## 17/18 章代码补丁（2026-09-08，§6.4 裁决驱动）

按 agent.md §6.4（决策级章节也要配最小代码示例）复查 17/18 章，评估结论：17.1~17.5、18.3~18.5 代码密度达标；补 4 篇 5 块，全部经构建 0 WARNING 验证：

- **18.1**：补"三系统最小构建入口对照"代码块（Buildroot defconfig/make、Yocto oe-init-build-env+bitbake、OpenWrt feeds），插在 13 维 Trade-off 表之前——决策矩阵从纯表格升级为有手感。
- **18.2**：补"膨胀版 local.conf 标本"（24 行节选，注释标注四类问题行及归属）与"recipe 拷贝 vs .bbappend 对照"两块——技术债务四形态从散文变实物。
- **18.6**：补 CI 静态检查脚本 `layer-hygiene.sh` 全文（bbappend 方向 + 补丁 Upstream-Status 双检查）——"固化为检查项"从一句话变可抄资产。
- **17.6**：补定稿分区表的 DTS `partitions` 机器可读形态（评审签表、产线烧 DTS 的对读要求）+ 断电测试工装循环脚本骨架（随机断电时机是本质，3000 次≈一周机时的排产含义）。

## 第 16 章重构存档（2026-09-08 闭环）

- **性质**：v1 小文件制式（21 篇 16.x.y）→ v2 厚章制式（7 篇，节=文件），依据设计稿 `docs/00-模板/第16章_重构设计稿_v1.md`（§7 三个开放问题均按"我倾向"项拍板）。与 17/18/22 从零写作不同，本章是**旧章重排+缺口补写+时效刷新**。
- **产出**：16.1 内核版本生态与 LTS 格局（合并 16.1.1~1.5，LTS 矩阵按 2026-09 联网核实刷新）/ 16.2 版本选型决策（合并 16.2.1~2.3，决策树 mermaid 化、五维评分卡落成）/ 16.3 BSP 与厂商策略（合并 16.3.1~3.4，上游优先成本曲线）/ 16.4 设备树架构【新写 236 行，大纲承诺而 v1 缺位的最大缺口：三组织模式+DTS 质量指标+dtc CI】/ 16.5 启动链设计（合并旧 16.4.1~4.5，介质选型保留 16.4.5 编号锚点护 17 章引用）/ 16.6 启动实战配置（合并旧 16.5.1~5.5，Falcon 三项代价按大纲补强；原文件 NUL 字节已清）/ 16.7 综合实战【新写：BSP 负责人三决策走查（6.12 CIP SLTS 3.80 分胜出、2.7 秒启动预算、dtsi 继承+运行时 overlay）+ 三案例复盘 + 14 项评审清单】。
- **合并技术要点**：小节降级为厚章内 `## 16.x.y` 段落并保留原编号标题（全书文字引用不失效；已核查无跨章 .md 直链）；21 个旧文件全部删除；每篇合并后构建 0 WARNING，mermaid 产物逐一验证。
- **待补图占位**：图片需求清单 34 张 🔴 项已全部以 `<!-- 【待补图】...（★必要，建议生图） -->` 注释迁入 6 篇厚章对应小节首部（16.4/16.7 为新增篇无图需）；清单文件保留，节标题已同步 v2 新位置（原 16.4.x→16.5 厚章 16.5.x、原 16.5.x→16.6 厚章 16.6.x）。
- 章 README 按 17/18/22 模板重写（**窗口滞后**意义对位：EOL 窗口/BSP 支持窗口/启动预算窗口 vs 17 失效滞后、18 债务滞后、22 耦合滞后 → 决策咬合链主线图 → 3/7/11/19/21/24 分工边界表 → 双学习方案 → 7 节阅读路线）；index.md 第 16 章行已更新为「完成 / 否（待人工复检）/ 2026年9月8日」。
