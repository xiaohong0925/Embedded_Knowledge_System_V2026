...
## 本节导读
本节教你如何把交叉编译器"请"进你的电脑里，让它在 x86 电脑上替你的 ARM 开发板编译程序。学完本节，你将掌握工具链的获取渠道选择、安装部署、以及环境变量的正确配置。
...
- 官网地址：https://developer.arm.com/downloads/-/gnu-toolchain
...
### 表格清单
- 表1：工具链获取渠道对比表（四条路线的获取难度、版本匹配度、维护活跃度、推荐场景）

### 图示清单
- 图1：工具链获取决策树 [mermaid图] —— 帮助你根据当前条件选择最合适的工具链来源
- 图2：工具链安装路径示意图 [配图说明] —— `/opt/toolchain/` 目录结构示意，展示解压后的文件组织方式

### 代码清单
- 代码1：解压工具链的 `tar` 命令
- 代码2：临时配置 PATH 的 `export` 命令
- 代码3：永久配置 PATH 到 `~/.bashrc` 的命令
- 代码4：验证安装的 `gcc -v` 命令
- 代码5：编译测试程序并 `file` 检查的完整流程
