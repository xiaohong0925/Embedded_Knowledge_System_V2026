...
## 本节导读
本节汇总交叉编译新手最常踩的10个坑，涵盖编译失败、链接报错、运行时错误和工具链管理四大类。读完这10个问答，你将能独立排查80%的入门级编译问题。

---

## 知识点 2.6.2.a：10个高频问题速查 [B] ~850字
...
| 问题类型 | 典型症状 | 快速排查命令 |
|----------|----------|--------------|
| 命令找不到 | `command not found` | `which` / `echo $PATH` |
| 架构不匹配 | `Exec format error` | `file` / `uname -m` |
| 头文件缺失 | `No such file or directory` | `gcc -xc -E -v` |
| 链接失败 | `cannot find -lc` | `gcc -print-search-dirs` |
| 体积过大 | 静态链接几百 KB | `strip --strip-unneeded` |
| 版本差异 | 升级后行为异常 | `gcc -Q --help=target` |

本节列出的 10 个问题覆盖了 90% 新手在交叉编译环境中的踩坑场景。遇到报错时，建议先对照上面的速查表定位问题类型，再按对应排查命令验证，最后结合 `file`、`readelf`、`objdump` 等工具深入分析。
...
在下一章《2.7 实践案例：构建第一个交叉编译程序》中，我们将把这些知识串联起来，从零开始完成一个真实项目的交叉编译——包括 Makefile 编写、交叉编译参数配置、以及把程序部署到真实开发板上运行。
...
### 表格清单
- 表 1：厂商工具链 vs 主线工具链选择指南
- 表 2：FAQ 速查表（问题类型/症状/排查命令）

### 图示清单
- 图 1：多工具链切换最佳实践流程图 [mermaid 图]
- 图 2：`file` 命令输出解读示意图 [配图说明：展示 `file hello` 输出中各字段含义，如 ELF 格式、架构类型、是否 stripped 等，建议用高亮截图加标注方式呈现]

### 代码清单
- 代码 1：环境变量 PATH 检查与导出命令
- 代码 2：`file` 和 `strip` 命令示例
- 代码 3：头文件搜索路径排查命令
- 代码 4：链接器搜索路径与 sysroot 查询命令
- 代码 5：工具链版本与 C 标准查询命令
- 代码 6：多工具链别名切换脚本
- 代码 7：`gcc -Q --help=target` 差异对比方法
