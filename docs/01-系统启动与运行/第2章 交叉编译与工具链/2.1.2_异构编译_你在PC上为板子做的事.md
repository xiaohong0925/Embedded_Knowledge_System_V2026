...
## 知识点1：为什么PC的程序不能直接放到板子上跑？——三个根本原因 [B] ~800字
...
这并不是你的程序写错了，而是**PC和嵌入式板子是两套完全不同的世界**。就像你把柴油加到汽油车里，引擎根本不认识这种"燃料"。根本原因可以归结为三个字：**异构**（Heterogeneous）。
...
## 知识点2：Host vs Target —— 建立双机模型认知 [B] ~500字
