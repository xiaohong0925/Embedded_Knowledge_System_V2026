...
本节用"厨房工具"比喻讲解 GNU binutils 二进制工具集——gcc这位"厨师"做完切配后，还需要一套趁手的工具来完成后续的"烧制、装盘、质检"工序。你将认识 binutils 八大核心工具，并亲手用 objdump 和 readelf 解剖一个 ELF 可执行文件。

---

## 知识点1：核心工具一览——binutils八大金刚 [B] ~800字
...
[图1：binutils工具与厨房工具对应关系示意图——展示八大工具如何在"编译流水线"中各司其职]
...
## 知识点2：as和ld的实际角色——gcc的"幕后搭档" [B] ~300字
...
## 知识点3：objdump与readelf实战——解剖ELF文件 [B] ~600字
...
### 表格清单
- 表1：binutils 八大核心工具速查表（工具 vs 厨房比喻 vs 嵌入式场景）
- 表2：gcc 调用 as/ld 的阶段对照表

### 图示清单
- 图1：binutils工具与厨房工具对应关系示意图
- 图2：gcc编译流水线中的 binutils 工具链调用关系（mermaid流程图）
- 图3：ELF文件内部结构层次图（mermaid架构图）

### 代码清单
- 代码1：`gcc -v hello.c 2>&1 | grep -E "(as|ld|collect2)"` — 观察 gcc 自动调用 as/ld
- 代码2：`objdump -d -S test > test.dis_with_src` — 源码级反汇编
- 代码3：`readelf -d /path/to/target_app | grep NEEDED` — 查看动态库依赖
- 代码4：`readelf -h test` / `readelf -S test` / `readelf -s test` — ELF 全面体检三件套
