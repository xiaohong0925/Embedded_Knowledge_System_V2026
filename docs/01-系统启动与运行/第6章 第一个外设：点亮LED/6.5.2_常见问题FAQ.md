...
> 所属章节：第6章 第一个外设：点亮LED > 6.5 总结与展望
> 难度：[B] | 预计阅读时间：10分钟
...
> 🔴 直接使用网上教程里的编号，而不核对**自己板子**的原理图，是烧毁GPIO的头号原因。
...
> 💡 有厂商提供了 `gpio` 命令行工具（如 `gpio readall`），可以直接打印编号对照表。
...
> ⚠️ 不要试图 `unexport` 别人的设备。你需要检查设备树（`.dts`），确认该GPIO是否已被声明为 `leds` 或 `gpio_keys` 的节点。如果是，应直接使用对应驱动的sysfs接口（如 `/sys/class/leds/`），而不是手动操作GPIO。
...
> 🔴 直接将LED接到3.3V或5V而不串联电阻，会烧毁LED甚至损坏GPIO引脚。
...
> ⚠️ 修改后需要**重新登录**或执行 `newgrp gpio` 使组权限生效。
...
> 💡 先用 `dtc -I dts -O dtb -o test.dtb your.dts` 做语法检查，确认无错再部署到 `/boot`。
...
> ⚠️ LED处于某些trigger模式（如 `mmc0`）时，内核会接管控制权，你无法再写入 `brightness`。必须先切回 `none`：
...
> 💡 通过 `cat /proc/config.gz | gunzip | grep HZ` 查看内核节拍频率（通常100Hz或250Hz），可估算最小可控精度。
