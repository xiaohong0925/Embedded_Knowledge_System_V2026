...
> 📷 此处建议插图：本章核心概念的数据流，从设备树描述到LED发光的完整链路
...
> ⚠️ 混淆"GPIO全局编号"与"芯片手册上的bank.pin编号"。前者用于sysfs（如35），后者是物理分组（如GPIO1_IO03），两者需要通过`gpiochip base + pin`换算。

> ⚠️ 手动sysfs方式下，`echo 1 > value`灯却灭了，这通常是因为LED电路是ACTIVE_LOW设计，而sysfs的`active_low`文件默认为0，导致逻辑值与物理电平相反。LED子系统方式下，设备树中的`GPIO_ACTIVE_LOW`会自动帮你处理好这层翻转。

> 💡 设备树节点里的`label`属性决定了LED在`/sys/class/leds/`下的目录名，命名建议采用`"board:color:function"`格式，方便后续管理。
