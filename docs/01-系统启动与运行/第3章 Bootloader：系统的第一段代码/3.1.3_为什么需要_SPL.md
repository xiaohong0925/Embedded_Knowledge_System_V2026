...
本节解答一个嵌入式Linux初学者的经典困惑：既然芯片里有BootROM，为什么还要多一个SPL？学完本节，你将理解SRAM与DRAM的物理差异、SPL的具体职责，以及它如何像一座"桥梁"把系统从芯片内部的狭小空间送进内存的广阔天地。

---

## 知识点1：SRAM与DRAM的大小鸿沟 [B] ~700字
...
[图1：SRAM与DDR容量对比示意图——左侧是小块SRAM（128KB），右侧是大块DDR（1GB），中间用箭头标注"SPL负责打通这条通路"]

---

## 知识点2：SPL的职责清单 [I] ~1,000字
...
- 表1：SRAM与DRAM特性对比表
- 表2：SPL四项核心职责清单表
- 表3：Falcon Mode vs 两阶段启动对比表

### 图示清单
- 图1：SRAM与DDR容量对比示意图 [配图说明]
- 图2：SPL执行流程图 [mermaid图]

### 代码清单
- 代码1：U-Boot命令行查看内存布局（`bdinfo`）
- 代码2：SPL时钟初始化伪代码
- 代码3：板级设备树DDR参数定义
- 代码4：存储介质烧录命令（`dd`）
- 代码5：SPL跳转U-Boot伪代码
- 代码6：Falcon Mode参数导出与烧录命令
