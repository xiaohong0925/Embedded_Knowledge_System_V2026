...
本节教你使用U-Boot的Kconfig配置系统：从交互式菜单选择板卡型号、串口和启动介质，理解`defconfig`默认配置的工作机制，并弄清`.config`文件如何变成源码里可用的`CONFIG_`宏。学完本节，你能够独立完成一块新开发板的U-Boot功能裁剪与配置。

---

## 知识点1：Kconfig菜单配置 [I] ~1,200字
...
[图1：menuconfig主界面截图，显示Architecture、ARM architecture等顶层菜单]
...
对嵌入式工程师而言，U-Boot菜单里需要重点关注的配置项集中在**架构/板卡选择**、**串口控制台**、**启动介质**和**外设驱动**四大类。下表列出最常改动的条目：
...
⚠️ **陷阱**：**Board型号选错是最常见的配置错误**。U-Boot里板级配置不仅决定了DDR初始化参数，还决定了GPIO复用、电源域设置等硬件细节。如果选了错误的board，U-Boot可能完全无法启动，串口无任何输出。当你看到"完全没打印"时，首先检查`CONFIG_TARGET_xxx`是否正确。
...
🔴 **危险**：`make menuconfig`依赖终端的`ncurses`库。如果在Windows WSL或某些精简SSH客户端里出现界面乱码（边框变成字母`qqqq`或`xxxx`），请设置`export TERM=xterm-256color`后再试。如果仍然不行，改用`make nconfig`（基于ncurses的新界面）或`make xconfig`（需要Qt图形界面）。
...
⚠️ **陷阱**：直接手动编辑`.config`文件是**极其不推荐**的。`.config`有严格的依赖关系，如果某个父选项未开启，子选项即使写进`.config`也会被Kconfig重新生成时删掉。正确的做法始终是通过`menuconfig`操作，让Kconfig自动处理依赖。

🔴 **危险**：不要混淆`make xxx_defconfig`和`make xxx_config`。后者是老版本U-Boot的遗留命名，在新版源码中已经不存在，执行会报错`No rule to make target`。如果你看到旧教程写`make smdk2410_config`，请改用`make smdk2410_defconfig`。

---

## 知识点3：.config与autoconf.h [I] ~600字
...
在编译U-Boot时，构建系统会先调用`scripts/kconfig/conf`工具，把`.config`转换为C头文件`include/autoconf.h`。这是自动完成的，你不需要手动干预：
...
⚠️ **陷阱**：修改`.config`后必须**重新执行make**。`autoconf.h`是在make的早期阶段生成的，如果你在make中途改`.config`，已编译的`.o`文件不会感知到这个变化。保险的做法是改完配置后执行`make clean && make`，或者至少`touch include/autoconf.h`触发重建。
...
### 表格清单
- 表1：menuconfig按键速查表（操作键/功能/使用场景）
- 表2：Kconfig关键配置项速查表（菜单路径/CONFIG宏/作用/典型选择）
- 表3：本节核心概念总结表（概念/要点/操作）

### 图示清单
- 图1：menuconfig主界面截图（显示ARM architecture等顶层菜单结构）
- 图2：Kconfig配置生成流程图 [mermaid图] — 展示从Kconfig→.config→autoconf.h→u-boot.bin的完整数据流

### 代码清单
- 代码1：`make evb-rk3568_defconfig && make menuconfig` — 启动配置流程
- 代码2：`head -20 .config` — 验证配置已保存
- 代码3：`configs/evb-rk3568_defconfig`节选 — defconfig文件示例
- 代码4：`make savedefconfig && cp defconfig configs/myboard-rk3568_defconfig` — 保存自定义配置
- 代码5：`include/generated/autoconf.h`片段 — 自动生成的C宏定义
- 代码6：源码中的`#ifdef CONFIG_USB`和Makefile中的`obj-$(CONFIG_MMC_DW)` — 条件编译示例
- 代码7：`grep CONFIG_USB include/generated/autoconf.h` — 快速验证宏是否生效
