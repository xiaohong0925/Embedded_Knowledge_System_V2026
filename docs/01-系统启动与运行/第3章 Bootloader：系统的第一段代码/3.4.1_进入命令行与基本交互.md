...
本节将带领读者进入U-Boot的命令行世界。你将学会如何打断自动启动、使用基本命令、获取帮助、操作环境变量, 这些是调试和配置嵌入式设备的基础技能。<br>学完本节，你可以像使用Linux终端一样在U-Boot提示符下游刃有余。

## <span class="blue"> 进入U-Boot Shell [B] 
...
你已经学会了如何在U-Boot中"站稳脚跟", 进入命令行、使用帮助、操作环境变量。<BR>下一节（3.4.2）我们将学习**内存操作与烧录命令**，掌握如何查看内存、下载文件到RAM、以及将镜像烧录到Flash存储器。
