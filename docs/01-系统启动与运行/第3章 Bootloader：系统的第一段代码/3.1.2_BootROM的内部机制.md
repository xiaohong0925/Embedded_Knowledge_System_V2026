...
本节带你"解剖"BootROM: 这颗藏在芯片内部的"第一行代码"究竟在哪里？上电后它怎样决定从哪个设备读取启动程序？以及为什么一块小小的SRAM会催生SPL这个关键角色。<br>学完本节，你将能看懂任何ARM SoC的启动手册中关于BootROM的章节。

---

## <span class="blue"> BootROM存储位置 [B] 
...
这块ROM的典型容量非常小——通常只有**32KB到128KB**。<br>为什么这么小？原因有三：
...
![图1：SoC芯片内部结构示意，突出BootROM和eFuse在芯片中的位置](images/soc_bootrom_efuse_diagram.png)

---

## <span class="blue"> 启动介质检测顺序 [I] 
...
![emmc_header](images/emmc_header.png)

> 💡 **提示**：对于Rockchip芯片，`0x8000`（32KB）偏移处是固定的IDB（ID Block）位置。如果这里没数据，BootROM会直接跳过这个介质。

---

## <span class="blue"> SPL加载地址与约束 [I] 

### 为什么BootROM不直接加载U-Boot？

BootROM的终极目标是把操作系统 bootloader（通常是U-Boot）加载到内存并执行。但中间隔着一个巨大的障碍: **SoC内部的SRAM太小了**。
