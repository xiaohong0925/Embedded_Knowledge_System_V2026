...
本节将带领你看清一颗ARM芯片从上电到Linux系统运行的完整"接力赛"路线。学完本节，你将能说出BootROM、SPL、U-Boot、内核四个阶段各自的任务，并在串口日志中准确辨认"现在跑到哪一棒了"。

---

## 知识点1：启动链的四个阶段 [B] ~1,500字
...
**SPL**（Secondary Program Loader）是U-Boot的"精简版分身"。芯片内部SRAM通常只有几十KB到几百KB，放不下完整的U-Boot，而外部DDR又还没初始化、无法使用——SPL就是为了解决这个尴尬而生。

SPL的任务：
1. **初始化DDR内存控制器**：让外部内存"通电可用"
2. **初始化基本的时钟和电源**
3. **把完整U-Boot加载到DDR**
4. **跳转到U-Boot入口**
...
⚠️ **陷阱**：初学者常在U-Boot命令行输入`saveenv`时遇到"无法保存"。环境变量需要持久化存储（如eMMC分区或SPI Flash），分区表配置不正确时`saveenv`会默默失败。检查方法：执行`env print`后重启，看变量是否还在。
...
⚠️ **陷阱**：不同芯片厂商的SPL输出格式差异很大。NXP i.MX系列输出`U-Boot SPL`，TI AM335x输出`spl`，Allwinner芯片则输出`SPL:`。更棘手的是，有些厂商的BootROM完全不输出任何字符，直到SPL运行后才有可见日志——这会让初学者误以为"上电无反应"。
