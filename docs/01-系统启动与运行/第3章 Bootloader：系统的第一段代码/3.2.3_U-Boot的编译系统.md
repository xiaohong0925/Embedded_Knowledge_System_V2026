...
## <span class="blue"> Makefile入口与编译目标 [I] 
...
![图1：终端编译过程截图，显示从C文件编译到最终链接生成u-boot.bin的滚动输出](images/uboot编译.png)

![图1：终端编译过程截图，显示从C文件编译到最终链接生成u-boot.bin的滚动输出](images/uboot编译产物.png)


### 常用编译目标速查

U-Boot Makefile预定义了几十个目标，覆盖了编译、打包、烧录辅助、清理等不同阶段。<br>下表列出嵌入式工程师最常用的一批：
...
`u-boot.img`是在`u-boot.bin`前面附加了一个由`mkimage`工具生成的**启动头**（boot header）。<br>这个头里通常包含：
...
![图2：u-boot.img文件结构示意图，展示Header + u-boot.bin的拼接关系](images/u-boot_img_structure.png)
