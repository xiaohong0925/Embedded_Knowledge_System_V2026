...
本节介绍U-Boot中最核心的三个启动命令, `boot`、`bootm`、`bootz`，以及控制自动启动行为的`bootcmd`环境变量和向内核传递参数的`bootargs`。<br>学完本节，你将能够手动从U-Boot引导Linux内核启动，并理解开发板"上电自动进系统"的背后原理。

---

## <span class="blue"> boot、bootm 与 bootz 命令 [B] 
...
## <span class="blue">  bootcmd 环境变量 [B] 
...
## <span class="blue">  bootargs 内核参数 [B] 
