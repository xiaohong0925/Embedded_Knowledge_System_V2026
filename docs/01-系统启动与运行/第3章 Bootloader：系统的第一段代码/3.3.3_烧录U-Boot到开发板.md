...
上一节我们编译出了U-Boot镜像文件，但编译完成只是"纸上谈兵", 只有把镜像真正写入开发板的存储介质，开发板才能启动运行。<br>本节介绍三种主流烧录方式（SD卡、USB、串口），手把手教你把U-Boot"装"进硬件，并掌握烧录失败时的排查思路。

---

## <span class="blue"> 烧录方法选择 [B] 
