...
本节讲述U-Boot从何而来、内部如何组织，以及源码目录各自的职责。学完本节，你能看懂U-Boot源码顶层目录，理解它的模块化设计思路，为后续编译和移植打下基础。

---

## 知识点1：U-Boot历史——从PPCBOOT到通用引导器 [B] ~500字
...
## 知识点3：源码目录组织——看懂U-Boot的地图 [B] ~700字
...
下一节（3.2.2）我们将动手实操：下载U-Boot源码、选择配置、交叉编译，并生成可烧录的镜像。有了本节对目录结构的理解，你会更清楚编译过程中每一步在做什么。
...
### 表格清单
- 表1：U-Boot核心源码目录功能表（知识点3）

### 图示清单
- 图1：U-Boot三层架构图（驱动层→命令层→启动逻辑层）[mermaid图]
- 图2：U-Boot目录协作关系与启动执行路径 [配图说明]

### 代码清单
- 代码1：查看U-Boot版本命令（`version`）
- 代码2：典型`bootcmd`启动脚本内容
- 代码3：用`grep`在源码中定位命令和配置的示例
- 代码4：选择板级defconfig的`make`命令示例
