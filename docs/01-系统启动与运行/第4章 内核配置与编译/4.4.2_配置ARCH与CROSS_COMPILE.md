...
本节教你告诉内核源码"我要为谁编译"，通过`ARCH`指定目标处理器架构，通过`CROSS_COMPILE`指定交叉编译器。读完即可正确配置任意嵌入式平台的编译参数，并验证配置是否生效。

---

##  <span class="blue"> ARCH变量，告诉内核目标架构 [B]
...
> ⚠️ 在Makefile中`ARCH`的默认值会自动检测当前主机的架构。如果你在一台x86电脑上不指定`ARCH`，内核会尝试编译x86版本，导致ARM设备上完全无法启动。交叉编译时**必须显式指定**`ARCH`。
...
> 💡 `CROSS_COMPILE`的值末尾**必须带连字符`-`**。内核Makefile会把前缀和工具名拼接，例如`$(CROSS_COMPILE)gcc`实际变成`aarch64-linux-gnu-gcc`。漏写连字符会导致拼接失败。
...
> ⚠️ 如果你先`export ARCH=arm`然后执行`make ARCH=arm64`，**命令行参数会覆盖环境变量**，最终生效的是`arm64`。这是Make的行为规则，优先级：`make命令行 > 环境变量 > Makefile内部默认值`。
...
> 🔴 千万不要混用架构的编译器！用`arm-linux-gnueabihf-gcc`编译`ARCH=arm64`的内核，或者反过来，编译过程会报错一堆诡异的汇编错误，而且错误信息极难定位。始终确保`ARCH`和`CROSS_COMPILE`前缀指向同一种架构。
...
> 💡 `V=1`参数让Make显示完整命令。你应该在输出中看到`aarch64-linux-gnu-gcc ... -D__KERNEL__ ... -c init/main.o`，而不是普通的`gcc`。
