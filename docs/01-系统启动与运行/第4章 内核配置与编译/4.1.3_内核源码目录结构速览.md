...
拿到内核源码压缩包后，解压出来的几千个文件和目录让人望而生畏。<BR>本节带你做一次"顶层设计图"级别的游览，快速搞清楚每个目录是干什么的、哪些文件最重要。学完本节，你将能像看地图一样阅读内核源码树，知道去哪里找自己需要的东西。

---

##  <span class="blue"> 顶层目录功能 [B][M] 

Linux内核源码采用清晰的树状结构组织，顶层大约包含20～30个目录。<BR>掌握这些目录的分工，相当于拿到了内核源码的"城市地图", 知道哪个"街区"住着什么样的人。

### 操作步骤：先亲眼看看全貌

拿到源码后，第一步不是急着阅读代码，而是用命令感受一下这个"代码城市"的规模。

- 进入内核源码目录，查看顶层有哪些目录和文件：
...
- 统计各目录的代码量，快速了解哪里"人多"（代码多）：

```bash
du -sh */ | sort -hr | head -15
```

![linux代码量](images/linux代码量.png)
...
> ⚠️ 初学时容易把 `arch/arm/` 和 `drivers/` 搞混。记住原则, `arch/` 管的是**CPU本身**（启动、中断、MMU、寄存器），`drivers/` 管的是**CPU外面的设备**（网卡、传感器、屏幕）。
...
> 💡 做嵌入式开发时，你80%的时间可能在 `drivers/` 和 `arch/` 两个目录之间来回穿梭。

![images](images/image%20copy.png)
...
![images](images/image%20copy%202.png)
...
> 🔴 修改 `mm/` 下的代码极易导致内核崩溃或内存泄漏。除非你有十足把握，否则只做阅读。
...
> 💡 `scripts/` 下的工具在编译内核时会自动调用，你通常不需要手动执行它们。
...
> ⚠️ 写内核模块时，`#include <linux/module.h>` 中的头文件就来自 `include/linux/`，而不是你系统的 `/usr/include`。
...
![gpio-kbuild](images/gpio-makefile.png)

![KBUILD3](images/KBUILD_3.png)
...
![gpio-kconfig](images/gpio-kconfig.png)
