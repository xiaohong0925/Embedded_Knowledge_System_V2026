...
本节教你编译Linux内核模块（ko文件）。读完即可编译全部模块、单独编译某个驱动模块，将模块安装到指定目录，并理解模块依赖的自动解决机制。

##  <span class="blue"> 模块编译 [B][M]
...
> 💡 编译模块前，必须确保你已经**完整编译过一次内核**（`make`或`make zImage`）。因为模块编译依赖内核构建系统生成的符号表和配置文件（如`.config`、`Module.symvers`、`include/generated`等）。
...
> ⚠️ `make M=drivers/net/usb`中的路径必须是**内核源码根目录下的相对路径**，而不是绝对路径，除非你同时指定了`-C <内核源码根目录>`。
...
> 💡 交叉编译时记得加上`ARCH=`和`CROSS_COMPILE=`前缀，例如：
...
> 🔴 单独编译模块时，如果内核源码树版本与目标运行的内核版本不一致（如`uname -r`不同），模块将无法加载，报"Invalid module format"错误。务必确保模块与目标内核版本一致。

##  <span class="blue"> 模块安装 [B]
...
> 💡 `INSTALL_MOD_PATH`只是路径前缀，不是最终根目录的完整路径。安装后的结构为`$INSTALL_MOD_PATH/lib/modules/<版本>/`。确保目标设备的根文件系统中`lib/modules/`路径正确。
...
> 💡 嵌入式设备启动时，`/etc/init.d/`或systemd服务中应使用`modprobe`而非`insmod`加载驱动，这样模块启动顺序由内核自动处理，避免人为出错。

> 📷 此处建议插图：insmod vs modprobe 加载方式对比示意图
...
下一节（4.5.2）介绍 驱动编译进内核 vs 编译成模块 的具体内容。
