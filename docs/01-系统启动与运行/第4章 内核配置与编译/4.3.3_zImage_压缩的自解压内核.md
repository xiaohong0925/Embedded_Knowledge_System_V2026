...
本节讲解嵌入式Linux中最经典的压缩内核映像，zImage。<BR>你将理解为什么内核需要被压缩、Bootloader如何把zImage搬到内存、以及zImage在启动时如何"自己把自己解开"。<br>学完本节，你能看懂启动日志中`Uncompressing Linux... done`的含义，也能根据存储空间大小选择合适的内核映像格式。
