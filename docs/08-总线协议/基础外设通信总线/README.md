...
## <strong>物理层对比与信号完整性</strong>

### <strong>I2C开漏结构的优势与局限</strong>

<span class="green">I2C</span>采用开漏输出+上拉电阻的电气结构，这是其多主仲裁能力的基础。
<br>
当多个设备同时驱动SDA时，任何设备输出低电平都会拉低总线——实现"线与"逻辑。
<br>
但这种结构的代价是上拉电阻限制了总线电容和速率：
<br>
| 模式 | 速率 | 最大总线电容 | 上拉电阻典型值 | 应用场景 |
|------|------|-------------|---------------|----------|
| 标准模式（Sm） | 100kbps | 400pF | 4.7kΩ | 传感器、EEPROM |
| 快速模式（Fm） | 400kbps | 400pF | 1.5kΩ | 一般外设 |
| 快速模式+（Fm+） | 1Mbps | 400pF | 1kΩ | 高速传感器 |
| 高速模式（Hs） | 3.4Mbps | 100pF | 外部电流源 | 高性能应用 |

<span class="blue">关键认知：I2C的总线电容限制（400pF）决定了最大设备数和走线长度。每增加一个设备，输入电容（通常5-10pF）就会累积，超过400pF就需要总线缓冲器（如PCA9515）分段隔离。
</span><br>

### <strong>SPI推挽输出的高速能力</strong>

<span class="green">SPI</span>采用推挽输出，不依赖上拉电阻，因此速率可达数十MHz甚至上百MHz。
<br>
SPI的速率瓶颈不是电气驱动能力，而是：
<br>
- PCB走线长度和信号完整性（反射、串扰）
...
| 速率 | 典型应用 | 最大走线长度 | 注意事项 |
|------|----------|-------------|----------|
| 1MHz | 低速传感器 | 50cm | 无需特殊处理 |
| 10MHz | Flash存储器 | 20cm | 注意地线回路 |
| 50MHz | 高速ADC/DAC | 10cm | 等长布线，阻抗匹配 |
| 100MHz+ | 显示屏接口 | 5cm | 差分对，严格等长 |
