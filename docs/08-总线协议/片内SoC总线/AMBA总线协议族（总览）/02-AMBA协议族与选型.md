...
<span class="red">为什么AMBA需要这么多总线协议？</span> SoC设计中没有"万能总线"——CPU与DDR之间需要数十GB/s的带宽和低延迟，UART寄存器访问只需要几MB/s的简单读写，而GPU与CPU之间又需要缓存一致性。AMBA协议族的设计哲学是"分层适配"：AXI承担高性能主干，AHB连接中速外设，APB服务低速寄存器，ACE/CHI解决多核一致性。理解各协议的适用域和权衡维度，是SoC架构师的核心能力。
...
flowchart TD
    subgraph AMBA_FAMILY["AMBA Protocol Family"]
        AXI["AXI4 / AXI5<br/>高性能主互连"]
        ACE["ACE / ACE5-Lite<br/>缓存一致性扩展"]
        CHI["CHI<br/>片上一致性互连"]
        AHB["AHB5 / AHB-Lite<br/>中速共享总线"]
        APB["APB4 / APB5<br/>低速外设总线"]
        ATB["ATB<br/>跟踪总线"]
        GIC["GIC Stream Protocol<br/>中断分发"]
    end
...
flowchart LR
    subgraph CRITERIA["Selection Criteria"]
        PERF["Bandwidth<br/>>1GB/s?"]
        AREA["Gate Count<br/><50K?"]
        POWER["Power Budget<br/>mW级?"]
        LATENCY["Latency Req<br/><100ns?"]
        COHERENCY["Cache Coherency<br/>Required?"]
        IP["IP Availability<br/>成熟生态?"]
    end
...
flowchart TD
    subgraph CLUSTER["Cortex-A53 Cluster"]
        CPU0["CPU0"] --> CCI["CCI-400<br/>Cache Coherent Interconnect"]
        CPU1["CPU1"] --> CCI
        CPU2["CPU2"] --> CCI
        CPU3["CPU3"] --> CCI
    end
    ...
    GPU["Mali-G31"] --> CCI
    CCI --> NIC["NIC-400<br/>AXI Network Interconnect"]
...
| 2 | 对比AXI4与CHI在4核CPU cluster中的优劣：一致性机制、拓扑灵活性、频率上限、门数开销。给出选型建议 | <span class="badge-i">[I]</span> |
| 3 | 在AXI+AHB+APB三层架构中，AXI-to-AHB Bridge和AHB-to-APB Bridge分别需要处理哪些协议转换细节？列出信号映射表 | <span class="badge-i">[I]</span> |
