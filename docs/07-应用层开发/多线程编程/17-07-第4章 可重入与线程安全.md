...
本章围绕可重入与线程安全的定义区别、非线程安全标准库函数的典型陷阱、线程安全errno与strtok_r的使用、TLS替代全局变量的策略展开，系统梳理了相关核心概念、API用法及嵌入式适配策略。关键要点包括：明确各机制的设计初衷与适用边界，掌握标准API的正确调用顺序与资源回收方式，理解并发场景下常见的陷阱（如竞态条件、优先级反转、内存泄漏）及其预防手段，最终能够根据具体嵌入式硬件资源与实时性要求，设计出稳定可靠的多线程应用架构。

---

### <strong>本章练习</strong>

1.  可重入函数一定是线程安全的吗？线程安全函数一定是可重入的吗？请分别举例说明。
2.  为什么strtok函数在多线程环境中会导致段错误？strtok_r如何通过保存上下文解决这个问题？
3.  嵌入式开发中为什么建议用TLS替代全局变量存储线程ID？请结合互斥锁开销与缓存局部性说明。
