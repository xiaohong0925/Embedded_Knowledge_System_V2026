...
## 需求分析：为什么需要 MIPI-CSI-2

---

### <strong>为什么 MIPI-CSI-2 成为行业刚需</strong>

<span class="red">MIPI-CSI-2</span>成为摄像头接口首选，源于移动设备对高像素、低功耗、小体积的刚性需求。传统并行摄像头接口引脚数随像素位宽线性增长，在 PCB 空间有限的手机中已无立足之地。
<br>

<span class="blue">为何选择 CSI-2：差分对串行传输将引脚数压缩至 4~10 根；D-PHY 的 HS/LP 双模式在传输图像时切换高速模式，空闲时进入低功耗模式，兼顾带宽与续航；虚拟通道机制使前置与后置摄像头可共享同一 CSI-2 控制器。</span>
<br>
...
<span class="red">MIPI-CSI-2</span>的演进与移动摄像头像素数增长直接相关。2005 年 CSI-1 基于并行接口，引脚多且速率受限。2009 年 CSI-2 发布，采用 D-PHY 差分对串行传输，单通道速率提升至 1Gbps 以上。2012 年 CSI-2 v1.1 引入虚拟通道（Virtual Channel），支持多摄像头共享同一物理链路。2017 年 CSI-2 v2.0 加入 C-PHY 支持，单通道速率突破 2.5Gsym/s，满足 4K@60fps 与多帧 HDR 摄像头的带宽需求。
<br>

<span class="blue">未来趋势：CSI-2 正向机器视觉与自动驾驶领域扩展；C-PHY 与 D-PHY 的双模支持将使 CSI-2 成为跨平台摄像头互连的首选协议。</span>
<br>